...
| PHASE3-B7 | Separate backup-settings structural schema, runtime defaults, historical seeding, exact legacy normalization, read/merge and Flask runtime application; remove the matching lazy edge | IMPLEMENTED / LOCALLY VERIFIED / AWAITING EXTERNAL SUPERVISOR REVIEW | Authorized subject `Separate backup schema from runtime configuration`; final identity resolves through Git history and publication proof | External supervisor review after publication | Binding 39; integrated ownership/schema/contract 124; backup/cloud/restore 54; release admin/route/RBAC 6; runtime isolation 15; recovered test-binding leakage 41; full hermetic 830 passed, 17 D73H deselected; post-governance 142; routes 131; RBAC unmapped 0; independent FREE review `ses_05b37cc64ffeUakdqRN7BkJ9pq` APPROVE | `docs/refactor/PHASE3_SCHEMA_STARTUP_TRANSACTION_CONTRACT.md` | Explicit module-level `main.app` binding is baseline compatibility debt / not target architecture; runtime side effects remain pre-commit; cloud/OAuth/retention/restore isolated; B8 unauthorized |
| Fase 3 | Data access consolidation | IN PROGRESS / PHASE3-A THROUGH PHASE3-B6 CLOSED / ACCEPTED / PHASE3-B7 IMPLEMENTED AND LOCALLY VERIFIED AWAITING EXTERNAL REVIEW | PHASE3-A, PHASE3-B1 through PHASE3-B7 above | PHASE3-B7 publication/review pending | Routes 131; RBAC unmapped 0; canonical database open count 0 | `docs/mapeamento/05_avaliacao_refactor.md`; `PROJECT_STATE.md` | B7 resolves only the backup-settings ownership/split leaf; B8/later production units remain separately gated |
