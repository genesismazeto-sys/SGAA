...
| PHASE-1-U2 | Proof for KRThinkpad parallel template | PROOF COMPLETE / CONFIRMED SAFE FOR IMPLEMENTATION / IMPLEMENTATION NOT STARTED | N/A (proof only — no implementation commit) | N/A | Scanner inputs 73 before/after; catalog SHA-256 ae40805d... identical; 0 key delta; 0 usage delta | `docs/mapeamento/05_avaliacao_refactor.md` | None safe for implementation; requires separate deletion order |
| Fase 1 | Safe cleanup: dead code, lixo, headers | OPEN / IN PROGRESS | N/A | N/A | U1 CLOSED / ACCEPTED; U2 proof complete/safe but not started | `docs/mapeamento/05_avaliacao_refactor.md` | Only U1 accepted at this time; U2 implementation requires separate explicit order |
