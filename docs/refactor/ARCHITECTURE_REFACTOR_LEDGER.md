...
| PHASE3-B6 | Separate access structural DDL, historical defaults, startup normalization and savepoint orchestration; extract sole ownership to `app.db_maintenance` | IMPLEMENTED / LOCALLY VERIFIED / AWAITING EXTERNAL SUPERVISOR REVIEW | Authorized subject `Separate access schema from normalization`; final identity resolved through Git history and publication proof | External supervisor review after publication | TDD RED confirmed missing owner/lazy/direct-import architecture; focused schema/ownership/contract 85; access/auth/admin/student/login/RBAC 130; isolation/clean/route/RBAC 19; full hermetic 791 passed, 17 D73H deselected; routes 131; RBAC unmapped 0 | `docs/refactor/PHASE3_SCHEMA_STARTUP_TRANSACTION_CONTRACT.md` | Runtime login normalization unchanged; dual init and other transaction debt remain; B7 unauthorized |
| Fase 3 | Data access consolidation | IN PROGRESS / PHASE3-A THROUGH PHASE3-B5-R1 CLOSED / ACCEPTED / PHASE3-B6 IMPLEMENTED AND LOCALLY VERIFIED AWAITING EXTERNAL REVIEW | PHASE3-A, PHASE3-B1 through PHASE3-B6 above | PHASE3-B6 publication/review pending | Routes 131; RBAC unmapped 0; canonical database open count 0 | `docs/mapeamento/05_avaliacao_refactor.md`; `PROJECT_STATE.md` | B6 resolves the access ownership/split leaf only; B7/later production units remain separately gated |
