...
| PHASE3-B1 | Extract `ensure_reportes_table` ownership to `app/db_maintenance.py` | IMPLEMENTED / LOCALLY VERIFIED / AWAITING SUPERVISOR REVIEW | Authorized subject `Extract reportes schema helper ownership`; identity resolved through Git history | External supervisor review pending | Helper 6; reportes/aluno/ownership 45; route/RBAC 3; Git-aware isolation 15; full hermetic 739 passed, 17 D73H deselected | `PROJECT_STATE.md`; `AGENT_HANDOFF.md` | No technical hard stop; later PHASE 3-B units remain separately gated |
| Fase 3 | Data access consolidation | IN PROGRESS / PHASE3-A CLOSED / ACCEPTED / PHASE3-B ASSESSMENT CLOSED / ACCEPTED / PHASE3-B1 LOCALLY VERIFIED | PHASE3-A and PHASE3-B1 above | PHASE3-B1 external review pending | Routes 131; RBAC unmapped 0; schema/migration delta 0; canonical database open count 0 | `docs/mapeamento/05_avaliacao_refactor.md`; `PROJECT_STATE.md` | Only B1 is implemented; migrations, access, backup, matrices, versioning, repository and final init cutover remain unauthorized |
