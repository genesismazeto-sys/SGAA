...
| PHASE4-B1 | Extract exactly eight Configurações/Mensagens admin routes and eight settings helpers into the first endpoint-preserving admin blueprint; establish reusable collision-safe registrar; add recursive message and CSRF canonical-owner discovery | IMPLEMENTED / AWAITING SUPERVISOR REVIEW | Authorized subject `Extract admin configuration blueprint`; commit/publication pending | Supervisor adjudication after publication | Original TDD RED 15 failed / 6 passed; focused GREEN 21 passed; B1-R1 RED/GREEN 4 failed then 4 passed; targeted 47 passed; full hermetic 939 passed / 17 deselected / 297.48s; routes 131; endpoints 130; governed pairs 134; RBAC unmapped 0; independent Flash FREE review PASS, cost 0, no fallback/mutation; exact 13-path candidate | `docs/refactor/PHASE4_ADMIN_BLUEPRINT_COMPATIBILITY_CONTRACT.md` | Transitional global legacy endpoints and `main` identity exports intentionally retained; 12 catalog source paths moved with zero key/default/semantic delta; B2 unauthorized; Phase 4 not closed |
| Fase 4 | Admin blueprint extraction | IN PROGRESS / B1 IMPLEMENTED / NOT CLOSED | B1 commit pending | N/A | B1 full hermetic 939/17 and independent review PASS; staging/publication/post-publication pending | `docs/mapeamento/05_avaliacao_refactor.md`; `docs/refactor/PHASE4_ADMIN_BLUEPRINT_COMPATIBILITY_CONTRACT.md` | B2 and later cohorts unauthorized; dashboard and admin_meus_dados ownership unresolved |
