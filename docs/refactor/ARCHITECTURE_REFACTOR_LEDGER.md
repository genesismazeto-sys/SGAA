...
| PHASE3-B10 | Extract activity-versioning core migration v3, retire self-transactional rebuild, move core owner to `app.db_maintenance`, reduce lazy bridge to two entries | IMPLEMENTED / LOCALLY VERIFIED / AWAITING SUPERVISOR REVIEW / PUBLICATION PENDING AT COMMIT-TREE TIME | Authorized subject `Version activity versioning core schema`; final identity resolves through Git history and publication proof | Supervisor review after publication | SCHEMA_VERSION 3; v3 registry entry `normalize_activity_versioning_core`; legacy self-transactional rebuild retired from main.py; core owner `ensure_activity_versioning_core_tables` in app.db_maintenance; lazy bridge exactly `get_preferred_matriz_for_curso` and `logger`; B8 leaf ownership preserved; B7 backup ownership preserved; B9 atividades v2 preserved; routes 131; RBAC unmapped 0 | `docs/refactor/PHASE3_SCHEMA_STARTUP_TRANSACTION_CONTRACT.md` | PHASE 3-B11 not authorized; preferred-matrix and logger remain as lazy dependencies |
| Fase 3 | Data access consolidation | IN PROGRESS / PHASE3-A THROUGH PHASE3-B9 CLOSED / ACCEPTED / PHASE3-B10 IMPLEMENTED, LOCALLY VERIFIED AND AWAITING SUPERVISOR REVIEW / PUBLICATION PENDING AT COMMIT-TREE TIME | PHASE3-A, PHASE3-B1 through PHASE3-B10 above | PHASE3-B10 publication/post-publication review pending | Routes 131; RBAC unmapped 0; canonical database open count 0 | `docs/mapeamento/05_avaliacao_refactor.md`; `PROJECT_STATE.md` | B10 resolves only the authorized activity-versioning core migration unit; B11/later production units remain separately gated |
