...
## Current authoritative state — PHASE 4-B7-P IMPLEMENTED / AWAITING SUPERVISOR REVIEW (2026-08-04)

PHASE 4-B6 is CLOSED / ACCEPTED (see `PHASE4-B6` row below). PHASE 4-B7-A (read-only
diagnosis of the Arquivos/Alertas/Reportes cohort) is accepted with classification
`SHARED_OWNER_PREREQUISITE_REQUIRED`. PHASE 4-B7-P implements exactly that
prerequisite: 4 shared symbols moved to neutral owners 2/1/1 —
`app.db_maintenance` (`ensure_admin_arquivos_table`, `ensure_admin_alertas_table`,
joining the pre-existing `ensure_reportes_table`), new `app.admin_files`
...
`mark_student_request_updates_seen`. Zero route movement. See the `PHASE4-B7-A` and
...
AUTHORIZED. Migration v4 remains PROHIBITED.
