...
| PHASE3-B8 | Extract four activity-versioning leaf tables, two transition triggers and eight leaf indexes into three pure caller-owned helpers while preserving the main core/rebuild orchestrator and four-entry lazy bridge | IMPLEMENTED / LOCALLY VERIFIED / INDEPENDENT REVIEW APPROVED / PUBLICATION PENDING | Authorized subject `Extract activity versioning leaf schema ownership`; final identity resolves through Git history and publication proof | External supervisor review after publication | TDD final RED 16 failed + 3 baseline guards; focused 19; integrated 126; Git-aware isolation 15; expanded versioning/matrix/requisition 309; routes/RBAC 3 with 131/0; full hermetic 849 passed, 17 D73H deselected; Stage-A FREE `ses_05af8e352ffetO5DM4E9LJ4n3d`; independent FREE review `ses_05adebcf2ffe4xZqeQFq1LX6Fy` APPROVE; both cost 0, no fallback/mutation | `docs/refactor/PHASE3_SCHEMA_STARTUP_TRANSACTION_CONTRACT.md` | Core schema, migration/rebuild, requisicoes compatibility and `app.db` remain isolated; PHASE 3-B9 unauthorized |
| Fase 3 | Data access consolidation | IN PROGRESS / PHASE3-A THROUGH PHASE3-B7 CLOSED / ACCEPTED / PHASE3-B8 IMPLEMENTED, LOCALLY VERIFIED AND INDEPENDENTLY APPROVED / PUBLICATION PENDING | PHASE3-A, PHASE3-B1 through PHASE3-B8 above | PHASE3-B8 publication pending | Routes 131; RBAC unmapped 0; canonical database open count 0 | `docs/mapeamento/05_avaliacao_refactor.md`; `PROJECT_STATE.md` | B8 resolves only the authorized activity-versioning leaf cluster; B9/later production units remain separately gated |
