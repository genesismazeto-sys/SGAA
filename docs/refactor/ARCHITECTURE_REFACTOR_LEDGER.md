...
| PHASE-1-U1 | Remove accidental tracked VS Code workspace artifact | VALIDATED / AWAITING EXTERNAL REVIEW | Subject `Remove accidental VS Code workspace artifact`; identity resolved through Git history | NO SEPARATE COMMIT (docs share the six-path unit commit) | 654 passed, 17 deselected; focused gate 7 passed; 0 failed, 0 errors; aggregate invariant e4bee85... pre/post identical | `docs/mapeamento/05_avaliacao_refactor.md` | None |
| Fase 1 | Safe cleanup: dead code, lixo, headers | OPEN / IN PROGRESS | N/A | N/A | U1 validated; remaining candidates unauthorized without separate order | `docs/mapeamento/05_avaliacao_refactor.md` | Only U1 authorized at this time; subsequent candidates require separate authorization |
