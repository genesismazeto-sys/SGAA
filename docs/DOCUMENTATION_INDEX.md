...
`docs/refactor/PHASE4_MATRIZES_BLUEPRINT_CONTRACT.md` for PHASE 4-B5
      ...
      IMPLEMENTED / AWAITING SUPERVISOR REVIEW, must never become CLOSED / ACCEPTED;
      publication permitted only after independent review PASS, bounded gates,
      selective per-path staging and a live-remote race check; PHASE 4-B6 NOT AUTHORIZED).
...
8. `docs/refactor/PHASE4_ATIVIDADES_BLUEPRINT_CONTRACT.md` — accepted B3 exact
 ...
 13. `docs/refactor/PHASE4_ALUNOS_TURMAS_CURSOS_SHARED_OWNER_CONTRACT.md` — PHASE 4-B6-P
      Alunos/Turmas/Cursos neutral shared-owner prerequisite contract; IMPLEMENTED /
      AWAITING SUPERVISOR REVIEW (must never become CLOSED / ACCEPTED; publication
      permitted only after independent review PASS, bounded gates, selective per-path
      staging and a live-remote race check; PHASE 4-B6 NOT AUTHORIZED).
 14. `PROJECT_STATE.md` — canonical current state (top block).
 15. `docs/refactor/HISTORICAL_DATABASE_SNAPSHOT_CUSTODY.md` — historical snapshot custody governance track.
 16. `AGENT_HANDOFF.md` — current operational handoff.
 16. All `docs/refactor/REF_*.md` / `docs/refactor/PHASE_0_*.md` files in dependency order: REF-0TF →
...
- **Fase 4 — Blueprint extraction**: OPEN / INCREMENTAL IMPLEMENTATION / B1 CLOSED /
  ...
  `main`) /
  PHASE 4-B6-P IMPLEMENTED / AWAITING SUPERVISOR REVIEW (must never become CLOSED /
  ACCEPTED; publication permitted only after independent review PASS, bounded gates,
  selective per-path staging and a live-remote race check; neutral Alunos/Turmas/Cursos
  shared-owner prerequisite; PHASE 4-B6 NOT AUTHORIZED) /
...
| `PHASE4_ALUNOS_TURMAS_CURSOS_SHARED_OWNER_CONTRACT.md` | PHASE 4-B6-P | PHASE 4-B6-A DIAGNOSIS COMPLETE / SHARED_OWNER_PREREQUISITE REQUIRED (read-only diagnosis; supervisor correction excluded `periodo_corrente`, which remains local in `main`); PHASE 4-B6-P IMPLEMENTED / AWAITING SUPERVISOR REVIEW at parent `1bf5173949b0bf0bdce15d1b87d6ed15d535158c` (must never become CLOSED / ACCEPTED; publication permitted only after independent review PASS, bounded gates, selective per-path staging and a live-remote race check; technical publication, when completed, does not constitute external supervisor acceptance; post-publication next action is external supervisor review). Neutral Alunos/Turmas/Cursos shared-owner prerequisite: exactly 9 shared symbols moved to neutral owners 3/5/1 — `app.academics` (3), `app.user_accounts` (5), `app.web.request` (1); `main` identity re-exports with zero local bodies; `app.views.core` direct import of `normalize_usuario_access_for_user_type`, residual lazy map exactly `aluno_url`/`get_db_connection`/`logger`; `periodo_corrente` local AST-identical; owner imports isolated from `main`, no new owner→`main` edge/cycle. R1 literal `PRE_REVIEW_SCOPE_EXPANSION / EXPLICITLY_AUTHORIZED / MESSAGE_SCANNER_OWNER_RECONCILIATION / NO_RETROACTIVE_GENERIC_AUTHORITY` (scanner provenance literal: moved `ValueError` message usage/source owner changed from `main.py` to `app/academics.py` while key `msg_4642b1608cf6a126` and exact default text remained unchanged — the message itself did not change); R3 literal `PRE_REVIEW_SCOPE_EXPANSION / ONE_FOR_ONE_TEST_POOL_SUBSTITUTION_PLUS_FOUR_ADDITIONS / STALE_GOVERNANCE_TEST_RECONCILIATION / MESSAGE_SCANNER_ALLOWLIST_RECONCILIATION / NO_RETROACTIVE_GENERIC_AUTHORITY`. Fresh full 1066/17/0/0/328.77s; routes 131; endpoints 130; message catalog 536; route/CSRF/database/residual byte-identical; canonical SQLite opens 0. Zero B6 route movement; no blueprint/factory/endpoint/RBAC/CSRF ownership migration; **PHASE 4-B6 NOT AUTHORIZED; B6 route extraction NOT AUTHORIZED**. Scope ceiling 18 (6 production + 6 tests + 6 governance), path 19 hard stop. |
