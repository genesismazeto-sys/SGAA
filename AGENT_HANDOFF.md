...
## Current state — PHASE 3-B10 ACTIVITY-VERSIONING CORE SCHEMA VERSIONING IMPLEMENTED / LOCALLY VERIFIED / AWAITING SUPERVISOR REVIEW / PUBLICATION PENDING AT COMMIT-TREE TIME — PHASE 3-B9 CLOSED / ACCEPTED — PHASE 3-B8 CLOSED / ACCEPTED — PHASE 3-B7 CLOSED / ACCEPTED — PHASE 3-B6 CLOSED / ACCEPTED — PHASE 3-B5-R1 CLOSED / ACCEPTED — PHASE 3-B5 CLOSED / ACCEPTED — PHASE 3-B4 CLOSED / ACCEPTED — PHASE 3-B3 CLOSED / ACCEPTED — PHASE 3-B2 CLOSED / ACCEPTED — PHASE 3-B1 CLOSED / ACCEPTED — PHASE 3-B ASSESSMENT CLOSED / ACCEPTED — PHASE 3-A CLOSED / ACCEPTED — PHASE 2 CLOSED / ACCEPTED — Phase 1 CLOSED / ACCEPTED — Historical snapshot custody OPEN / SOURCE PRESERVED / LEVEL 3 NOT EXECUTED / SECURITY-COMPLETE CUSTODY NOT CLAIMED

### Current operational handoff — PHASE 3-B10 activity-versioning core migration v3 (2026-07-30)

- **Status/baseline:** IMPLEMENTED / LOCALLY VERIFIED / AWAITING SUPERVISOR REVIEW / PUBLICATION PENDING AT COMMIT-TREE TIME; B9 CLOSED / ACCEPTED; B11 unauthorized. Workspace `D:\OneDrive\Programação\SGAA_clean_baseline`, branch `refactor/architecture-safety-net`, baseline HEAD/upstream/live remote `37c8757e4ef41647c971ad8a974c853dec6ce4e7`, parent `42ad0b500fe26fb2a4f49a2f8655d0217233af75` (B8 parent / B9 head), divergence 0/0 and clean before mutation. Authorized commit subject: `Version activity versioning core schema`.
- **Core changes:** SCHEMA_VERSION 3; v3 registry entry `normalize_activity_versioning_core` in `app.db_maintenance`. Migration v3 normalizes five legacy `atividade_versao` variants, preserves all parent/child/self-reference data, and handles deterministic `numero_versao` assignment and `sqlite_sequence`. The early runner `apply_early_schema_migrations` is the sole destructive migration boundary with one isolated `BEGIN IMMEDIATE` transaction.
- **Retired from main.py:** `_VERSAO_NEW_DDL`, `_needs_atividade_versao_migration`, `_needs_atividade_versao_default_fix`, `_needs_index_hardening`, `_recreate_atividade_versao`, `_migrate_atividade_versao_to_numero_versao`, `_fix_atividade_versao_default`, and the legacy public orchestrator `ensure_atividade_versioning_schema`. Core DDL, triggers, indexes, leaf tables/triggers/indexes and requisicoes compatibility are now solely in `app.db_maintenance`.
- **Lazy bridge:** exactly two entries remain: `get_preferred_matriz_for_curso` and `logger`. `ensure_atividade_versioning_schema` is a direct import in both `app.db` and `main.py`.
- **Isolation:** B8 leaf ownership (four tables, two triggers, eight indexes) unchanged. B9 atividades v2 unchanged. B7 backup ownership unchanged. No B11 work exists.
- **Test evidence:** Focused v3+v2: 49 passed in 2.25s. Six-test contract gate: 163 passed in 10.36s. Comprehensive regression: 352 passed, 17 D73H deselected, 0 failures. Routes 131; RBAC unmapped 0.
- **Authorized manifest:** 14 paths: `app/db_maintenance.py`, `app/db.py`, `main.py`, `tests/test_activity_versioning_core_migration_v3.py` (new, tracked after staging), `tests/test_atividades_schema_migration_v2.py`, `tests/test_activity_versioning_leaf_schema_ownership.py`, `tests/test_backup_settings_ownership.py`, `tests/test_db_schema_maintenance.py`, `tests/test_phase3_schema_startup_transaction_contract.py`, canonical contract, Documentation Index, Architecture Ledger, `PROJECT_STATE.md`, this handoff.
- **Next action:** selective staging, bounded commit, feature-branch push, post-publication verification. Do not begin PHASE 3-B11.
