...
### Operational handoff — PHASE 4-B7-P acceptance closeout (2026-08-04)

- **Authority:** PHASE 4-B4.2: CLOSED / ACCEPTED. PHASE 4-B5 CLOSED / ACCEPTED. PHASE 4-B6-P is CLOSED / ACCEPTED. PHASE 4-B6-R1 and PHASE 4-B6 are CLOSED / ACCEPTED. PHASE 4-B7-A: **CLOSED / ACCEPTED**. PHASE 4-B7-P: **CLOSED / ACCEPTED**. PHASE 4-B7 route extraction: **NOT AUTHORIZED**. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION. Phase 5 and Phase 6 are NOT AUTHORIZED. Migration v4 is PROHIBITED.
- **Baseline:** `refactor/architecture-safety-net` at accepted technical commit `1c82a1954250aa5e6654349ce77a50d60f03fe8f` (`Extract B7 shared neutral owners`), parent `b6d6e2295e2beeba046cfe1f4c1614f667261ad2` (`Record acceptance of Phase 4-B6`); protected `main` `340fc7c91c6bc9b50e884adcb5915f9e29a0bfe1`. Technical publication: **COMPLETE**. Post-publication bounded verification: **COMPLETE**. External supervisor technical acceptance: **GRANTED**. Governance closeout: published by this R3 commit.
- **Accepted candidate:** canonical partition 2/1/1 — `app.db_maintenance` owns `ensure_admin_arquivos_table` + `ensure_admin_alertas_table` (joining pre-existing `ensure_reportes_table`); new `app.admin_files` owns `get_admin_arquivo`; new `app.admin_alerts` owns `list_active_admin_alertas`. `main` re-exports all four by identity, zero local bodies. `aluno._get_main_helpers()` reduced 5 → exactly 2 (`get_student_request_update_alert`, `mark_student_request_updates_seen`). Zero route movement (12 handlers / 13 route-method pairs remain main-local); `uploaded_file` and `admin_dashboard` AST-unchanged; Reportes ownership unchanged.
- **Preserved exceptions (kept truthful, not rewritten):** full default suite `1103 passed / 3 failed / 17 deselected` — **NOT GREEN**; the exact three failures remain classified `PRE_EXISTING_BASELINE_REPRODUCED / ENVIRONMENTAL_ENCODING_FAILURE / UNRELATED_TO_B7_P / ACCEPTED_NONBLOCKING_RESIDUAL / NO_RETROACTIVE_GREEN_CLAIM`. Phantom `sgaa_canonical_db_guard` debt and `_best_effort_remove_admin_arquivo_file` containment residual remain recorded, unrepaired. Routing/reviewer deviation preserved: the ordered IAsup/IAexec router was not invocable in the implementing harness; implementation was performed by Claude, disclosed rather than substituted silently; the external supervisor subsequently performed an independent repository review as part of this closeout; deviation accepted as nonblocking.
- **Governance closeout:** exactly six governance paths; authorized subject `Record acceptance of Phase 4-B7-P`; identity resolves through Git history. No production/test/snapshot/database path is changed by this closeout.
- **Next exact action:** none for B7-P. Governance closeout complete. **PHASE 4-B7 (route/blueprint extraction) remains NOT AUTHORIZED.** Do not begin another cohort.

## Historical state — PHASE 4-B7-P IMPLEMENTED / AWAITING SUPERVISOR REVIEW (historical phase-time record)

### Operational handoff — PHASE 4-B7-P Arquivos/Alertas neutral shared-owner prerequisite (2026-08-04)

- **Authority (historical phase-time):** PHASE 4-B4.2: CLOSED / ACCEPTED. PHASE 4-B5 CLOSED / ACCEPTED. PHASE 4-B6-P is CLOSED / ACCEPTED. PHASE 4-B6-R1 and PHASE 4-B6 are CLOSED / ACCEPTED. PHASE 4-B7-A: DIAGNOSIS COMPLETE / SHARED_OWNER_PREREQUISITE_REQUIRED (accepted). PHASE 4-B7-P: IMPLEMENTED / AWAITING SUPERVISOR REVIEW (phase-time; superseded by CLOSED / ACCEPTED above). PHASE 4-B7 route extraction: NOT AUTHORIZED. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION. Phase 5 and Phase 6 are NOT AUTHORIZED. Migration v4 is PROHIBITED.
- **Baseline:** `refactor/architecture-safety-net` at required entry `HEAD` `b6d6e2295e2beeba046cfe1f4c1614f667261ad2` (`Record acceptance of Phase 4-B6`); protected `main` `340fc7c91c6bc9b50e884adcb5915f9e29a0bfe1`; remote branch confirmed at the same entry HEAD before mutation; worktree/staging confirmed clean.
- **Implemented candidate:** canonical owner partition 2/1/1 — `app.db_maintenance` owns `ensure_admin_arquivos_table` + `ensure_admin_alertas_table` (joining the pre-existing `ensure_reportes_table`); new `app.admin_files` owns `get_admin_arquivo`; new `app.admin_alerts` owns `list_active_admin_alertas`. `main` re-exports all four by identity, zero local bodies, zero wrappers, zero dynamic import, zero `sys.modules` bridge. `aluno._get_main_helpers()` reduced from 5 keys to exactly `{get_student_request_update_alert, mark_student_request_updates_seen}`. Zero route movement (12 handlers / 13 route-method pairs remain main-local). `uploaded_file` and `admin_dashboard` AST-unchanged. Reportes ownership unchanged.
- **Body equivalence:** all four moved bodies (and their `def`/arg signatures) are AST-equivalent to entry baseline `b6d6e2295e2beeba046cfe1f4c1614f667261ad2:main.py`, verified via `ast.dump` comparison in `tests/test_phase4_arquivos_alertas_shared_owners.py::test_b7p_body_equivalence_against_entry_baseline`. No SQL/index/transaction/schema/return-shape change.
- **RED evidence:** new `tests/test_phase4_arquivos_alertas_shared_owners.py`, 11 collected: 7 failed / 4 passed, exit 1, captured before any production mutation. All seven failures attributable solely to the absent prerequisite.
- **GREEN evidence:** same file, 11 passed, exit 0, after implementation.
- **PHASE 4-B7-P-R2 — scope-expansion correction (supervisor-authorized in session):** full default suite exposed two more frozen assertions of the identical pre-B7-P 5-key aluno lazy map outside the named 13-path pool — `tests/test_db_schema_maintenance.py::EXPECTED_ALUNO_LAZY_KEYS_AFTER_VERSIONING_EXTRACTION` (B2 versioning-subsystem residue) and `tests/test_phase4_versioning_subsystem.py::REMAINING_ALUNO_MAIN_HELPERS`. Both received the identical one-for-one 5-key → 2-key correction after explicit supervisor authorization; no other change to either file. Classification: `PRE_REVIEW_SCOPE_EXPANSION / EXPLICITLY_AUTHORIZED / ALUNO_LAZY_MAP_INVARIANT_RECONCILIATION / NO_RETROACTIVE_GENERIC_AUTHORITY`.
- **PHASE 4-B7-P-R2 — pre-existing environmental failure waiver (supervisor-authorized in session):** full default suite: `1103 passed / 3 failed / 17 deselected`, 329.53s, exit 1. **NOT GREEN; NOT 0 failed.** Exact three failing node IDs: `tests/test_phase3_final_init_cutover.py::test_seed_tool_uses_factory_owner_without_main_and_is_idempotent` (assertion on a mis-decoded `"Seed demo concluído."` subprocess stdout string); `tests/test_pytest_runtime_isolation.py::TestSubprocessImportMain::test_import_main_uses_runtime_root` and `::TestMainNoOverwrite::test_import_main_preserves_upload_folder` (both `SyntaxError: Non-UTF-8 code starting with '\xe7' ... no encoding declared` on a generated temp `.py` file). Independently reproduced with identical fingerprints on a disposable `git worktree` of unmodified entry baseline `b6d6e2295e2beeba046cfe1f4c1614f667261ad2` under the same interpreter (worktree removed after verification, no residual). Neither failing file references `admin_arquivos`/`admin_alertas`, or any of the four B7-P symbols; grep-confirmed zero causal coupling. Classification: `PRE_EXISTING_BASELINE_REPRODUCED / ENVIRONMENTAL_ENCODING_FAILURE / UNRELATED_TO_B7_P / ACCEPTED_NONBLOCKING_RESIDUAL / NO_RETROACTIVE_GREEN_CLAIM`. Neither file modified; neither added to the mutable pool; the environmental debt remains separate and unrepaired.
- **B7-P-specific/affected focused gates — all GREEN:** targeted lane (`test_phase4_arquivos_alertas_shared_owners.py` + `test_phase4_requisicoes_shared_owners.py` + `test_residual_shared_helpers.py` + `test_admin_arquivos.py` + `test_admin_reportes.py` + `test_admin_aux_sections.py` + `test_csrf_inventory_audit.py`): 69 passed, 0 failed, 0 errors, 19.97s, exit 0. Global-invariant lane (`test_route_inventory_snapshot.py` + `test_rbac_requirement_coverage.py` + `test_ref_0c_d_r1_route_complete_actor_matrix.py`): 36 passed, 0 failed, 0 errors, 29.72s, exit 0.
- **Invariants:** routes 131; endpoints 130; business route-method pairs 160; RBAC unmapped 0 (baseline artifact byte-identical, zero `git diff`); route inventory byte-identical (baseline artifact zero `git diff`); message catalog 536. `database.db` 544768 bytes / SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9` unchanged before/after; WAL/SHM/journal absent both times.
- **Residual debt recorded, not repaired in this phase:** `_best_effort_remove_admin_arquivo_file` path-containment observation (main-local, low-risk, server-generated filenames only); phantom `sgaa_canonical_db_guard` documentary-only governance debt (not invoked, not created); the pre-existing environmental encoding defect in `test_phase3_final_init_cutover.py`/`test_pytest_runtime_isolation.py`.
- **Agent routing disclosure:** ordered IAsup `openai-codex/gpt-5.6-sol` / IAexec DeepSeek V4 Flash (FREE-first) were not invocable in this harness (no router tool available); the acting Claude Code session performed diagnosis and implementation directly. No Pro/Luna/GPT escalation occurred.
- **Scope:** exact production/test manifest is 9 paths — production `app/db_maintenance.py`, `app/admin_files.py` (new), `app/admin_alerts.py` (new), `app/views/aluno.py`, `main.py` (5); tests `tests/test_phase4_arquivos_alertas_shared_owners.py` (new), `tests/test_phase4_requisicoes_shared_owners.py`, `tests/test_db_schema_maintenance.py`, `tests/test_phase4_versioning_subsystem.py` (4, the latter two under the R2 supplemental-scope grant). Adding the six governance paths below yields exact 15 changed paths; path 16 is a hard stop absent further authorization.
- **Governance synchronization:** exactly six governance paths: `PROJECT_STATE.md`, `AGENT_HANDOFF.md`, `docs/DOCUMENTATION_INDEX.md`, `docs/mapeamento/05_avaliacao_refactor.md`, `docs/refactor/ARCHITECTURE_REFACTOR_LEDGER.md`, `docs/refactor/PHASE4_ARQUIVOS_ALERTAS_SHARED_OWNER_CONTRACT.md` (new).
- **Next exact action:** freeze the candidate, run independent review (disclosed as not invocable via the ordered router in this harness), commit as `Extract B7 shared neutral owners` with required parent `b6d6e2295e2beeba046cfe1f4c1614f667261ad2`, verify the race gate, and push by normal fast-forward only. PHASE 4-B7 (route/blueprint extraction) remains NOT AUTHORIZED by this phase.

## Historical state — PHASE 4-B6-R3 CLOSED / ACCEPTED — GOVERNANCE CLOSEOUT PUBLISHED

### Operational handoff — PHASE 4-B6 acceptance closeout (2026-08-04)

- **Authority:** PHASE 4-B4.2: CLOSED / ACCEPTED. PHASE 4-B5 CLOSED / ACCEPTED. PHASE 4-B6-P is CLOSED / ACCEPTED. PHASE 4-B6-R1 and PHASE 4-B6 are **CLOSED / ACCEPTED**. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION. Phase 5 and Phase 6 are NOT AUTHORIZED. Migration v4 is PROHIBITED. Arquivos / Alertas / Reportes are not authorized.
- **Baseline:** `refactor/architecture-safety-net` at accepted technical commit `3d9660a99e6944ff94a3991a353ecf3aaf300987` (`Extract admin alunos turmas cursos blueprint`); protected `main` `340fc7c91c6bc9b50e884adcb5915f9e29a0bfe1`. Technical publication: **COMPLETE**. Post-publication verification: **COMPLETE**. External supervisor acceptance: **GRANTED**. Governance closeout: published by this R3 commit.
- **Implemented candidate:** canonical owner `app.views.admin.alunos_turmas_cursos`; exact 17 endpoints / 24 route-method pairs / 10 helpers; main zero local B6 bodies and 27 identity re-exports; accepted registrar; factory default/exact opt-out; RBAC 6 view / 13 edit / 5 full; B6-P neutral owners preserved; zero `app -> main`.
- **R1 correction:** the original ownership contradiction over `periodo_corrente` was resolved only by removing dead template kwargs from `admin_detalhes_curso`, `admin_turmas` and `admin_detalhes_turma`. Exactly three removals, no fourth. `periodo_corrente` remains main-local, byte/AST unchanged, and called by `_build_admin_dashboard_turma_cards`; no B6 binding. Ten helpers remain AST-identical; 17 handlers differ only by route decorators and those exact removals.
- **Chronology:** RED 25 collected / 6 passed / 19 failed / 0 errors; primary GREEN 28; CSRF regeneration 2 with second generation byte-identical and B6 11 owner-only; focused 254; initial full 1093 passed / 2 stale cumulative-CSRF failures / 17 deselected / opens 0. The history is preserved. R2 direct nodes 2 passed in 1.08s; structural/global lane 34 passed in 1.29s; pre-freeze full collected 1112 = 1095 passed + 17 deselected, 0 failed/errors, 433.87s, exit 0, opens 0. The selective-staging whitespace gate exposed five copied trailing-space lines; the post-review/full gate also exposed the B1 ledger-row compatibility regression. The first recovery full truthfully yielded 1093 passed / 2 failed / 17 deselected (AST literal equivalence plus B1 governance). The bounded correction retained the three SQL-string spaces via source `\x20`, removed only two syntactic trailing spaces and restored canonical `| Fase 4 |`; the two focused nodes passed and the repeated final full collected 1112 = 1095 passed + 17 deselected / 0 failed/errors / 318.06s / exit 0 / opens 0.
- **CSRF:** exact exhaustive B6 `11`; Matrizes `8 + 11 = 19`; Requisições `5 + 8 + 11 = 24`; zero non-owner/unexpected deltas and no weakened assertion.
- **Invariants/custody:** routes 131; endpoints 130; business pairs 160; governed pairs 134; unmapped 0; actor 402 = 263 + 139; catalog 536; route inventory 20814 bytes / `6e32148cd1988d5e405c9d11bdbda285359f72d5fc7eca8bd5ef8da9b83049fa`. `database.db` 544768 bytes / `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; sidecars absent. Residual 17420 bytes / `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`.
- **Scope:** exact 10 technical/test + 6 governance paths = 16; path 17 hard stop. Canonical contract: `docs/refactor/PHASE4_ALUNOS_TURMAS_CURSOS_BLUEPRINT_CONTRACT.md`.
- **Next exact action:** none for B6. PHASE 4-B6 is CLOSED / ACCEPTED. Governance closeout complete. Do not begin another cohort.

## Historical state — PHASE 4-B6-P CLOSED / ACCEPTED

### Operational handoff — PHASE 4-B6-P Alunos/Turmas/Cursos neutral shared-owner prerequisite (published, recovered and externally accepted) (2026-08-04)

- **Current authority/status:** PHASE 4-B4.2: CLOSED / ACCEPTED. PHASE 4-B5-P: CLOSED / ACCEPTED. PHASE 4-B5: CLOSED / ACCEPTED. Compatibility literal: PHASE 4-B5 CLOSED / ACCEPTED. PHASE 4-B6-A: DIAGNOSIS COMPLETE / SHARED_OWNER_PREREQUISITE REQUIRED (historical prerequisite diagnosis). PHASE 4-B6-P: **CLOSED / ACCEPTED**. PHASE 4-B6: **NOT AUTHORIZED**. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION; it is not closed. Phase 5: NOT AUTHORIZED. Phase 6: NOT AUTHORIZED. Migration v4: PROHIBITED. This accepted unit is only the neutral shared-owner prerequisite; no Alunos/Turmas/Cursos route moved and `app/views/admin/alunos_turmas_cursos.py` remains absent.
- **Accepted publication and recovery state:** branch `refactor/architecture-safety-net`; accepted technical commit `0b003c5f00e5e6181c9b9a5f96d8572b8488323f`, subject `Extract B6 shared neutral owners`, parent `1bf5173949b0bf0bdce15d1b87d6ed15d535158c`; accepted recovery commit `5af7fc70547fccbfed42ebaf5ca353365371c892`, subject `Repair B6-P post-publication governance state`. Technical publication: **COMPLETE**. Post-publication recovery: **COMPLETE**. Post-publication verification: **COMPLETE**. External supervisor acceptance: **GRANTED**. The R6 governance closeout commit identity resolves through Git history. Protected `main` remains `340fc7c91c6bc9b50e884adcb5915f9e29a0bfe1`.
- **Ownership:** exactly 9 shared symbols moved to neutral owners 3/5/1. `app.academics` owns `build_turma_aluno_matricula`, `resequence_turma_aluno_matriculas`, `resequence_turma_aluno_matriculas_for_ids`; `app.user_accounts` owns `_access_defaults_map`, `_default_password_for_user_type`, `create_usuario_with_default_access`, `create_usuario_with_default_password`, `normalize_usuario_access_for_user_type`; `app.web.request` owns `_is_ajax_request`. `main` re-exports all nine by identity with zero local bodies/wrappers/duplicates. All nine moved bodies are AST-equivalent to parent `1bf5173949b0bf0bdce15d1b87d6ed15d535158c:main.py`.
- **B6-A diagnosis (read-only prerequisite diagnosis, excluding `periodo_corrente`):** DIAGNOSIS COMPLETE / SHARED_OWNER_PREREQUISITE REQUIRED. The diagnosis and supervisor correction identified exactly the nine shared symbols below and **excluded `periodo_corrente`** — it remains local in `main` and is AST-identical to parent. No route, factory, endpoint, RBAC, CSRF or schema movement was diagnosed for B6-P.
- **Consumer resolution:** `app.views.core` imports `normalize_usuario_access_for_user_type` directly from `app.user_accounts`; the residual lazy map in `core._get_main_helpers` remains exactly `aluno_url` / `get_db_connection` / `logger`. The `login` flow `normalize_usuario_access_for_user_type(...) -> conn.commit() -> refreshed SELECT` is preserved. Acesso consumers and `_admin_access_denied_response` (uses the canonical `_is_ajax_request`) are preserved. `periodo_corrente` remains local in `main`, AST-identical to parent.
- **Import isolation:** isolated imports of the three owners do not import `main`; no new owner → `main` edge or cycle; `app.auth`/`app.db_maintenance` do not import `app.user_accounts`.
- **Zero B6 route movement:** no blueprint/factory/endpoint/RBAC/CSRF ownership migration. **B6 route extraction remains NOT AUTHORIZED.**
- **B6-P-R1 literal record —** `PRE_REVIEW_SCOPE_EXPANSION / EXPLICITLY_AUTHORIZED / MESSAGE_SCANNER_OWNER_RECONCILIATION / NO_RETROACTIVE_GENERIC_AUTHORITY`. R1 added `utils/messages.py` to production scope and exactly `PROJECT_ROOT / "app" / "academics.py"` to `_iter_backend_files()`; no recursive `app/**/*.py`, no `app/user_accounts.py`, no `app/web/request.py`, no other message-scanner scope change. Regression RED: catalog 535, key `msg_4642b1608cf6a126` absent; GREEN: catalog 536, key present, exact default `"Turma sem código para gerar matrícula."`, source `app/academics.py`, no duplicate. Scanner provenance preserved literally: the moved `ValueError` message usage/source owner changed from `main.py` to `app/academics.py` (because the raising `build_turma_aluno_matricula` body moved there), while key `msg_4642b1608cf6a126` and the exact default text remained unchanged — the message itself did not change. Discrimination was an in-memory monkeypatch filtering `app/academics.py`, proving 535/key absent, then restoring the real callable/cache; final bytes retain the entry.
- **B6-P-R3 literal record —** `PRE_REVIEW_SCOPE_EXPANSION / ONE_FOR_ONE_TEST_POOL_SUBSTITUTION_PLUS_FOUR_ADDITIONS / STALE_GOVERNANCE_TEST_RECONCILIATION / MESSAGE_SCANNER_ALLOWLIST_RECONCILIATION / NO_RETROACTIVE_GENERIC_AUTHORITY`. `tests/test_residual_shared_helpers.py` lost mutation authority and remained unchanged gate-only; five paths were explicitly added; final exact mutable test pool is the six test paths in the actual dirty manifest. First full: 1061 passed, 5 failed, 17 deselected, 410.35s, canonical opens 0, physical state identical (four preexisting stale governance assertions + one R1-caused scanner allowlist assertion). Exact five-node recovery 5/5 in 1.06s; coupled five-file lane 77/77 in 11.26s; fresh final full collected 1083 = 1066 passed + exactly 17 deselected, 0 failed, 0 errors, 328.77s, exit 0, `CANONICAL_SQLITE_OPENS=0`, physical before/after candidate snapshot identical.
- **Earlier focused qualification:** 206 passed, failed 0, errors 0, 117.65s, exit 0. The first failed harness attempt (206 passed but exit 1) was a recoverable basetemp path translation defect (MSYS `/c` → native Windows `D:/c`); rerun with native `C:/` path passed with identical physical state. Exact R1 CSRF nodes passed 2/2, snapshot bytes unchanged, B6-P CSRF delta `[0,0]`.
- **Historical TDD / scanner-owner chronology (preserved, not rewritten):** initial structural RED `13 failed / 31 passed` (owner modules absent; `main` still owned the nine bodies); initial structural GREEN `44 passed` (exact nine shared symbols moved, neutral owners 3/5/1). Historical focused lane `203 passed / 2 failed`, both failures scanner-owner expectations (message catalog RED at 535, key `msg_4642b1608cf6a126` absent); the R1 scanner-owner reconciliation added exactly `PROJECT_ROOT / "app" / "academics.py"` to `_iter_backend_files()` and brought the catalog to 536 with the exact default `"Turma sem código para gerar matrícula."`. These historical failures are retained as phase-time evidence and are not claimed to have never happened.
- **Fresh invariants:** routes 131; endpoints 130; business pairs 160; governed pairs 134; RBAC unmapped 0; actor 402 = 263 allowed + 139 denied; message catalog 536. Route inventory byte-identical: 20814 bytes, SHA-256 `6e32148cd1988d5e405c9d11bdbda285359f72d5fc7eca8bd5ef8da9b83049fa`. CSRF shadow-off/on byte-identical to each other: each 288509 bytes, SHA-256 `4b16f1b4a1e672807398f33fb626f073e29ada5e9453675eabe5c824a7eec769`. Canonical `database.db` 544768 bytes / `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent; canonical SQLite opens 0. Protected residual 17420 bytes / `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`.
- **Scope ceiling after R3:** production 6, tests 6, governance 6, total 18; path 19 hard stop. Actual non-governance dirty manifest exactly 12 paths: production `app/academics.py`, `app/user_accounts.py`, `app/web/request.py`, `app/views/core.py`, `main.py`, `utils/messages.py`; tests `tests/test_phase4_alunos_turmas_cursos_shared_owners.py`, `tests/test_phase3_schema_startup_transaction_contract.py`, `tests/test_phase4_atividades_blueprint.py`, `tests/test_phase4_configuracoes_blueprint.py`, `tests/test_phase4_requisicoes_shared_owners.py`, `tests/test_phase4_versioning_subsystem.py`. Adding the exact six governance documents yields exact 18 changed paths. `tests/test_residual_shared_helpers.py` remains unchanged/read-only gate.
- **Governance synchronization:** exactly six governance paths; canonical contract `docs/refactor/PHASE4_ALUNOS_TURMAS_CURSOS_SHARED_OWNER_CONTRACT.md`. This documentary delta changes no production, test, snapshot or database path.
- **Final accepted prerequisite state:** `PHASE4_B6_P_CLOSED / ACCEPTED / NEUTRAL_SHARED_OWNER_PREREQUISITE_ESTABLISHED / EXACT_9_SHARED_SYMBOLS / OWNERS_3_5_1 / MESSAGE_SCANNER_RECONCILED / POST_PUBLICATION_RECOVERY_COMPLETE / PREPUBLICATION_GATE_INVOCATION_ERROR_RECORDED / NO_RETROACTIVE_GREEN_CLAIM / GOVERNANCE_CLOSEOUT_AUTHORIZED`. Accepted evidence: main identity re-exports with zero local bodies for the nine; `periodo_corrente` unchanged; zero B6 route movement; message catalog 536; routes 131; endpoints 130; business pairs 160; governed pairs 134; RBAC unmapped 0; actor 402 = 263 allowed + 139 denied; route inventory and CSRF snapshots byte-identical; full hermetic 1066 passed / 17 deselected / 0 failed / 0 errors / `CANONICAL_SQLITE_OPENS=0`; independent review PASS with 0 blocking findings; recovery node 1 passed / 0 failed; bounded post-publication 162 passed / 0 failed / 0 errors; published-tree bounded verification 162 passed / 0 failed / 0 errors. PHASE 4 remains OPEN / INCREMENTAL IMPLEMENTATION. PHASE 4-B6 is NOT AUTHORIZED; Phase 5 and Phase 6 are NOT AUTHORIZED; Migration v4 is PROHIBITED.
- **Historical pre-publication freeze and invocation defect:** before commit/publication, the 18-path candidate was frozen at parent/live feature `1bf5173949b0bf0bdce15d1b87d6ed15d535158c`, NOT committed and NOT pushed. The intended bounded pre-publication pytest lane was invoked incorrectly as `python -m sgaa_canonical_db_guard -m pytest ...`; that command did **not** execute pytest. Its exit 0 / empty stdout is invalid as qualification and is not represented as GREEN. Classification: `PREPUBLICATION_BOUNDED_GATE_INVOCATION_ERROR / DISCOVERED_POST_PUBLICATION / NO_RETROACTIVE_GREEN_CLAIM`. Valid evidence remains the focused `206 passed`, full hermetic `1066 passed / 17 deselected / 0 failed / 0 errors`, and independent review PASS.
- **Historical publication and first real post-publication lane:** the first normal push was rejected with GitHub permission HTTP 403; a later authorized retry published technical commit `0b003c5f00e5e6181c9b9a5f96d8572b8488323f` by normal fast-forward. The first real bounded post-publication lane, using `python -m pytest -p sgaa_canonical_db_guard ...`, produced `161 passed / 1 failed / 0 errors / 48.20s`, exit 1, `CANONICAL_SQLITE_OPENS=0`. Exact failure: `tests/test_phase4_requisicoes_shared_owners.py::test_b41_governance_records_exact_scope_and_preserves_later_prohibitions`; exact defect: the current authoritative prefixes omitted `PHASE 4-B4.2: CLOSED / ACCEPTED`. This is a documentary current-state defect only, with no production/test defect established and no retroactive pre-publication GREEN claim.
- **R5 recovery completion and R6 acceptance:** R5/R5.1 changed only `AGENT_HANDOFF.md` and `PROJECT_STATE.md`; recovery commit `5af7fc70547fccbfed42ebaf5ca353365371c892` was published by normal fast-forward after correct post-publication gates. R6 external supervisor acceptance is GRANTED for technical commit `0b003c5f00e5e6181c9b9a5f96d8572b8488323f` plus recovery commit `5af7fc70547fccbfed42ebaf5ca353365371c892`. No production, test, snapshot, database, schema or migration mutation occurred in recovery/closeout. B6 remains NOT AUTHORIZED.

## Historical state — PHASE 4-B5 CLOSED / ACCEPTED

### Operational handoff — PHASE 4-B5-R6 Matrizes admin blueprint acceptance closeout (2026-08-03)

- **Authority/status:** PHASE 4-B4.2: CLOSED / ACCEPTED. PHASE 4-B5-A: DIAGNOSIS COMPLETE / SHARED_OWNER_PREREQUISITE REQUIRED. PHASE 4-B5-P: CLOSED / ACCEPTED and immutable prerequisite history. PHASE 4-B5: **CLOSED / ACCEPTED** — technical publication COMPLETE, post-publication verification COMPLETE, external supervisor acceptance GRANTED. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION; it is not closed. Phase 5 and Phase 6 are NOT AUTHORIZED. Migration v4 is PROHIBITED. No later cohort is named or authorized as current work by this block.
- **Baseline / parent (required):** branch `refactor/architecture-safety-net`; parent `ef874b9d14b02656a0f26ea885024a280d49682e`, subject `Record acceptance of Phase 4-B5-P`; protected `main` `340fc7c91c6bc9b50e884adcb5915f9e29a0bfe1`.
- **Published technical commit:** `2a122357a79080fa66aa19c00ed5ff8533308f41`, subject `Extract admin matrices blueprint`, parent `ef874b9d14b02656a0f26ea885024a280d49682e`. Publication: COMPLETE. Post-publication verification: COMPLETE. Published manifest: exactly 17 paths. The authorized governance closeout subject is `Record acceptance of Phase 4-B5`; its identity resolves through Git history. No future closeout SHA is claimed here.
- **Canonical owner:** `app.views.admin.matrizes` is the canonical owner of exactly **10 global legacy endpoints / 12 route-method pairs** and the **21 route-private corrected helpers**. `main.py` has zero local bodies/decorators for the 10 handlers and zero local bodies for the 21 helpers, re-exporting all by identity. All 31 moved bodies are **AST-identical to the baseline modulo removal of `@app.route` decorators**; `@admin_required` is preserved. `bp_admin_matrizes` plus the immutable tuple `LEGACY_ROUTE_SPECS` use the accepted `LegacyRouteSpec` / `configure_legacy_routes` registrar; no `@bp.route`, namespaced alias, duplicate rule, compatibility wrapper, `import main`, dynamic import or `sys.modules` edge exists. Zero `app -> main` edge.
- **10 endpoints:** `admin_matrizes` GET; `admin_adicionar_matriz` GET/POST; `admin_editar_matriz` GET/POST; `admin_excluir_matrizes` POST; `admin_excluir_matriz` POST; `admin_matriz_nova_atividade` POST; `admin_matriz_nova_versao_card` POST; `admin_matriz_versoes` GET; `admin_matriz_versoes_definir` POST; `admin_matriz_versoes_remover` POST.
- **21 corrected helpers:** `get_bases_escopo_matriz`; `get_versoes_ativas_por_base_na_matriz`; `get_vinculo_versao_da_matriz`; `_set_versao_da_matriz_para_base`; `_remover_versao_da_matriz_para_base`; `get_card_version_menu_data`; `_matriz_status_badge_type`; `_matriz_vigencia_label`; `_matriz_activity_type_for_tab`; `_matriz_axis_for_tab`; `_get_grupos_por_tipo`; `_get_matriz_active_normas_for_axis`; `_build_matriz_new_activity_modal_context`; `_matriz_transfer_meta`; `_matriz_activity_rule_summary`; `_matriz_transfer_lists`; `_matriz_counts`; `_render_matriz_form`; `_matriz_payload_from_request`; `_ensure_default_versao_link`; `_save_matriz_activity_links`.
- **Void names:** `_get_grupos_atividade` and `_get_matriz_active_norma_ids` remain absent and are never created/exported in `main` or the new module. Classification: `SUPERVISOR CONTRACT CORRECTION / NOT A PATH-POOL EXPANSION / NOT A DOMAIN-SCOPE EXPANSION`.
- **Factory:** keyword-only `register_admin_matrizes_blueprint=True` registers the exact 10/12 cohort deterministically after the accepted admin cohorts; explicit `False` removes exactly the cohort; independent apps remain isolated; endpoint and rule/method collisions fail atomically through the accepted registrar.
- **RBAC:** exact 3 view / 7 edit / 2 full; `app/auth.py` is unchanged.
- **B5-P preservation:** `app.admin_access` remains protected and unchanged; `admin_editar_matriz` and `admin_matriz_nova_atividade` resolve the same canonical `_get_current_admin_access_context` and `_admin_can` objects through their `__globals__`; `_persist_user_access_overrides` and `_parse_access_overrides_from_form` remain main-local.
- **Direct owners consumed:** `app.matrix_scope`, `app.activity_catalog`, `app.db`, `app.db_maintenance`, `app.web.filters`, `app.web.pagination`, `app.auth`, `utils.messages`, and only `_build_grupo_label` / `_canonicalize_tipo_limitacao` from `app.views.admin.atividades`.
- **Freezes:** ensures (`ensure_matrizes_atividades_table`, `ensure_matriz_atividade_links_table`, `ensure_atividade_versioning_schema`), GET timing, SQL, transactions, commits/rollbacks/exceptions, version/matrix links, templates, `url_for`, redirects, flashes/messages, CSRF and HTTP are preserved; zero redesign/hardening.
- **Evidence:** preimplementation ownership/governance nodes 3 passed; corrected TDD RED `20 failed / 4 passed / 0 collection errors` (owner/factory/CSRF absent); primary B5 GREEN `24 passed / 3.74s`; focused expanded (44 files, full identified matriz/versioning coverage) `578 passed / 249.33s`; first full `1049 passed / 1 failed / 17 deselected / 370.60s` with the sole failure at `tests/test_phase4_configuracoes_blueprint.py` package membership — HARD STOP before any out-of-pool change; B5-R3 one-for-one mutable-pool substitution removed `tests/test_ref_0c_b1_p0_access_context_transactions.py` mutation authorization (remains read-only focused gate) and added `tests/test_phase4_configuracoes_blueprint.py` only for `matrizes.py` package membership (node 1 passed / 0.64s; affected focused 143 passed / 24.30s); final full hermetic fresh `1050 passed / 17 deselected / 0 failed / 0 errors / 317.65s` (collected 1067); canonical SQLite opens 0. Accepted post-publication bounded lane: `59 passed / 0 failed / 0 errors / 22.59s`. No other full run is claimed.
- **Invariants:** routes 131; endpoints 130; business pairs 160; governed pairs 134; RBAC unmapped 0; actor 402 = 263 allowed + 139 denied; message catalog 536. Route inventory byte-identical: 20814 bytes, SHA-256 `6e32148cd1988d5e405c9d11bdbda285359f72d5fc7eca8bd5ef8da9b83049fa`. CSRF exactly `[8, 8]` owner-only deltas, 78 rows each, summaries equal, no non-owner delta. Canonical database: 544768 bytes, SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent; canonical SQLite opens 0. Protected residual: 17420 bytes, SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`. `app/admin_access.py` byte-identical: 4546 bytes, SHA-256 `b2dc2592637b1268c001ef6153c0e9ad619b1336654c976e328224c6da1d9814`.
- **Execution / routing:** IAsup Hermes `openai-codex/gpt-5.6-sol` direct adjudication/deterministic gates. RED IAexec logical/effective `flash_free`, `opencode` / `opencode/deepseek-v4-flash-free`, session `ses_03b57ca6bffenOnPYH3mt5WdX0`, exit 0, cost 0, fallback none. Implementation IAexec logical `flash_free` → selected `flash_normal` by `FALLBACK_FREE_CONTEXT_TOO_LARGE`, actual `opencode-go` / `opencode-go/deepseek-v4-flash`, same session `ses_03b57ca6bffenOnPYH3mt5WdX0`, exit 0, cost `0.0010985912`, no silent escalation. Corrections IAsup direct: two deterministic typos in the new RED and the B5-R3 membership expressly authorized (rationale: lower risk/cost; preserve the second FREE for independent review). **Independent technical read-only review — COMPLETE / PASS:** original request logical `flash_free`, `opencode` / `opencode/deepseek-v4-flash-free`; the wrapper created session `ses_03adb7f27ffeJ1feNSLhrFij5R`, cost 0, exit 1 — FREE technical failure, no usable verdict, no mutation. Single explicit fallback R4 `FALLBACK_FREE_EXECUTION_FAILURE`: selected `flash_normal`, actual `opencode-go` / `opencode-go/deepseek-v4-flash`, session `ses_03ad0a15dffeR53I9BqGU6a4tl`, exit 0, cost `0.001000188`, status COMPLETED, no further escalation. READ-ONLY; mutation 0; index empty before/after; manifest/identities initial/final identical. Isolated reviewer gates `51 passed` in `6.19s`; supplemental requisicoes csrf/route/owners `3 passed`. Findings: BLOCKING 0; NONBLOCKING 4 (some routing/cost documentary facts are IAsup evidence not re-executed; `test_phase4_atividades_blueprint` owner assertion is a precise mechanical correction, not weakening; malformed/preexisting protected residual outside the manifest, do not clean; preexisting ignored `__pycache__`, not created). Additional risks recorded: full/focused not re-executed by the reviewer per order; CSRF off/on byte-identical is a pre-existing state not introduced by B5. Verdict: **PASS / SCOPE EXACT / BEHAVIOR PRESERVED / MUTATION 0**. Reviewer confirmed 31 AST-exact bodies, 10/12, 21 helpers, RBAC 3/7/2, zero `app -> main`, 8 owner-only CSRF deltas, exact package-membership change, coherent governance, shared-owner contract unchanged. Frozen/reconfirmed identities (initial=final): full WORKTREE raw `73706a7bf10b10d96e273489bee2824aab57cd91197f23a37f59f1e6f2e33e60`; full GIT-canonical `aed7ef95144deb711ec45ca018ec33a8dd0da97b4ec4ab48753ad652215dcf33`; production/test raw `9e8c0bbc8e7421a160a0386ee357c4f2062fbfa7a3d4bf06717d5ab8043e1cd4`; production/test git-canonical `0b4caa7bf50a706cc68f7ef6d6557aea09dd40eb3c342282fc4a04edbe860b88`; governance raw pre-review-doc-update `769e75d33df559133befcc6b244e920a9ec230cff7486bbffdaae40abf2a95a8`; governance git-canonical pre-review-doc-update `2d9429ea76eed4fb0b2f68d5055bfb7134b759440cdbf70753a54c44b0b32cde`. The two full hashes and the two governance hashes identify the candidate actually reviewed before this documentary addendum and are naturally superseded by this documentation update; the production/test hashes remain valid and are the stable technical link. Do not present the pre-update hashes as post-addendum identity.
- **Governance synchronization:** exactly six governance paths; canonical contract `docs/refactor/PHASE4_MATRIZES_BLUEPRINT_CONTRACT.md` (new). `docs/refactor/PHASE4_MATRIZES_SHARED_OWNER_CONTRACT.md` is NOT modified. This documentary delta changes no production, test, snapshot or database path.
- **Final canonical state:** `PHASE4_B5_CLOSED / ACCEPTED / MATRIZES_BLUEPRINT_ESTABLISHED / EXACT_10_LEGACY_ENDPOINTS / EXACT_12_ROUTE_METHOD_PAIRS / EXACT_21_CORRECTED_HELPERS / RBAC_3_VIEW_7_EDIT_2_FULL / B5_P_ADMIN_ACCESS_OWNER_PRESERVED / ZERO_APP_TO_MAIN_EDGE / ROUTES_131 / ENDPOINTS_130 / BUSINESS_PAIRS_160 / GOVERNED_PAIRS_134 / RBAC_UNMAPPED_ZERO / ACTOR_402_263_139 / MESSAGE_CATALOG_536 / ROUTE_INVENTORY_BYTE_IDENTICAL / CSRF_EXACT_8_OWNER_ONLY_DELTAS / FULL_HERMETIC_1050_17 / INDEPENDENT_REVIEW_PASS / TECHNICAL_COMMIT_2A122357 / PUBLICATION_VERIFIED / SECTION_22_PROCESS_DEVIATION_RECORDED / GOVERNANCE_CLOSEOUT_PUBLISHED`. PHASE 4 remains OPEN / INCREMENTAL IMPLEMENTATION. PHASE 4-B5 is CLOSED / ACCEPTED. Phase 5 and Phase 6 are NOT AUTHORIZED. Migration v4 is PROHIBITED. No later cohort is named or authorized as current work.

## Section 22 mandatory permanent history

The Section-22 pre-implementation reconciliation gate was **NOT executed before the first B5 production mutation** as required by the original order. This is a historical process deviation recorded literally and unambiguously.
...
This closeout resolves the governance debt by recording it truthfully, not by rewriting history. Section 22 was **not** satisfied retroactively, the deviation **did** happen, and the original ordering requirement was **not** met. The B5-R4 hard stop correctly detected the missing required Section-22 record in the frozen technical commit before publication and withheld publication at that time; the B5-R5 external-supervisor waiver superseded only the pre-push documentary blocker, did not claim retroactive compliance, and authorized publication of the frozen technical commit unchanged. Publication was a single normal fast-forward; no force, main/tag/PR activity occurred.

## Historical state — PHASE 4-B5-P CLOSED / ACCEPTED

### Operational handoff — PHASE 4-B5-P-R3 Matrizes neutral admin-access shared-owner acceptance closeout (2026-08-02)

- **Authority/status:** PHASE 4-B4.2: CLOSED / ACCEPTED. PHASE 4-B5-A: DIAGNOSIS COMPLETE / SHARED_OWNER_PREREQUISITE REQUIRED. PHASE 4-B5-P: CLOSED / ACCEPTED. PHASE 4-B5: NOT AUTHORIZED FOR IMPLEMENTATION. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION; it is not closed. Phase 5 and Phase 6 are NOT AUTHORIZED. Migration v4 is PROHIBITED. This is only the neutral shared owner prerequisite; no Matrizes route moved and `app/views/admin/matrizes.py` remains absent.
- **Published technical identity:** branch `refactor/architecture-safety-net`; technical commit `92486f87ea15697282a265cb7a9941678cb9138f` (`Extract admin access context shared owner`), parent `a0b56896252a276e562da3842d3d61b078bd9f27` (`Record acceptance of Phase 4-B4.2`); protected `main` `340fc7c91c6bc9b50e884adcb5915f9e29a0bfe1`. Publication: COMPLETE. Post-publication verification: COMPLETE.
- **Ownership:** `app/admin_access.py` is the canonical owner of exactly `_fetch_user_access_overrides`, `_build_access_scope_groups_for_level`, `_load_admin_access_context`, `_get_current_admin_access_context` and `_admin_can`. `main.py` has zero local bodies and directly imports/re-exports all five by identity. All five moved bodies are AST-identical to parent `a0b56896252a276e562da3842d3d61b078bd9f27:main.py` with no redesign. `_persist_user_access_overrides` and `_parse_access_overrides_from_form` remain local in `main.py`.
- **Consumers:** six exact current main consumers: `enforce_admin_access_control`, `inject_admin_access_helpers`, `admin_editar_matriz`, `admin_matriz_nova_atividade`, `admin_acesso`, `uploaded_file`. Existing Flask `admin_required` wrappers are unwrapped only by test introspection; no compatibility wrapper was added.
- **Import graph:** `app.admin_access -> app.auth, app.db, app.db_maintenance` and Flask `g`/`session`; `app.admin_access -> main = 0`; `app.auth -> app.admin_access = 0`; no cycle among these modules. Existing unrelated lazy `app.views.core`/`aluno -> main` edges remain baseline. Isolated `app.admin_access` import creates no Flask app/file/network/SQLite/migration side effect. Transaction neutrality preserved.
- **Candidate manifest (11 paths):** production (2) `app/admin_access.py`, `main.py`; tests (3) `tests/test_phase4_matrizes_shared_owners.py` (new), `tests/test_phase4_requisicoes_blueprint.py` (mechanical fixed-baseline correction), `tests/test_phase4_requisicoes_shared_owners.py` (B5-P-R2 supplemental scope authorization); governance (6). The authorized but unchanged `tests/test_ref_0c_b1_p0_access_context_transactions.py` remains a gate, not a changed path. Updated authorized ceiling is 12 (2 production + pool of 4 tests + 6 governance). Do not claim 12 changed paths.
- **Accepted evidence:** final pre-publication hermetic `1025 passed / 17 deselected / 0 failed / 0 errors / 326.74s`; independent-review rerun `1025 / 17 / 0 / 0 / 367.27s`; post-publication focused `132 passed / 0 failed / 0 errors / 39.23s`. Historical first full hermetic `1 failed / 1024 passed / 17 deselected / 399.46s` (stale B4.2 CSRF baseline) was corrected by B5-P-R1 via `B4_2_BASELINE_COMMIT=c587098152e97d125f41a2d26f2f414c10ae5676` under classification `PRE_REVIEW_SCOPE_EXPANSION / NOW_EXPLICITLY_RATIFIED / NO_RETROACTIVE_GENERIC_AUTHORITY`; R1 exact node `1 passed / 0.68s`; a later interrupted run is not a final result. B5-P-R2 SUPPLEMENTAL SCOPE AUTHORIZATION added and modified only `tests/test_phase4_requisicoes_shared_owners.py` (classification `PRE_REVIEW_SCOPE_EXPANSION / EXPLICITLY_AUTHORIZED_B5_P_R2 / FIXED_ACCEPTED_B4_1_BASELINE / NO_RETROACTIVE_GENERIC_AUTHORITY`); R2 exact node `1 passed / 1.02s`; affected governance aggregate `118 passed / 16.70s`. The earlier `1025 passed / 17 deselected / 306.41s` run is only pre-R2 historical evidence.
- **Invariants:** routes 131; endpoints 130; business pairs 160; governed pairs 134; RBAC unmapped 0; actor 402 = 263 allowed + 139 denied; message catalog 536. Route inventory byte-identical: 20814 bytes, SHA-256 `6e32148cd1988d5e405c9d11bdbda285359f72d5fc7eca8bd5ef8da9b83049fa`. CSRF shadow-off/on byte-identical to HEAD and each other: each 288349 bytes, SHA-256 `3a94e2e1ca587ffe966262717e2cdcde1251ef90b61202d81d4b681ae79a0056`. Canonical database: 544768 bytes, SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent; canonical SQLite opens 0. Protected residual: 17420 bytes, SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`.
- **Execution evidence:** Supervisor/IAsup Hermes `openai-codex/gpt-5.6-sol`. TDD/implementation IAexec continuity session `ses_03ce1c465ffeb31XEcAo57hbC4`; initial route `flash_free`; explicit fallback `opencode-go/deepseek-v4-flash`, trigger `FALLBACK_FREE_CONTEXT_TOO_LARGE`; recorded router costs RED correction `0.0008019144 USD`, GREEN `0.0009146312 USD`, test correction `0.0007943432 USD`; no silent escalation. B5-P-R1 direct IAsup correction (no fourth delegated continuation). Earlier B5-P governance authoring succeeded via `flash_normal`, `opencode-go/deepseek-v4-flash`, session `ses_03cb9cee8ffeyk6hJFuMcmRrgD`, exit 0, cost `0.0013047696 USD`, no fallback, exact six governance mutations. A later continuation attempt to that expired/nonexistent session failed with `FLASH_NORMAL_FAILED / Session not found / no session / no cost / no mutation`; recorded as a recoverable transport/session failure. The fresh explicit delegation then succeeded via `flash_normal`, `opencode-go/deepseek-v4-flash`, session `ses_03c9ae6a7ffe4XeucPqy9Iyrre`, exit 0, cost `0.0005911248 USD`, with no fallback and mutations limited to the same six governance paths. The R2 test correction was a direct IAsup Sol correction because it was deterministic, exact, tiny, and safer/cheaper than another context export.
- **Accepted independent review:** provider `opencode`, model `opencode/deepseek-v4-flash-free`, session `ses_03c92c10affegAmZLZ63tmTjjA`, exit 0, cost 0, fallback none, mutation 0, blocking findings 0, nonblocking findings 3 (R2 no literal `NO ROUTE MOVEMENT` token; `B4_2_BASELINE_COMMIT` naming; runtime duration changed). External acceptance GRANTED by the PHASE 4-B5-P-R3 order.
- **Accepted content identities:** raw candidate `bf67fcaa7a44d27ae70adc4b2ab38842a9e808d31eb66a7907f56dad50aefaaf`; technical raw `932793aaac94862cac81310dcfca17aa53a5ce52a69283f82e7c2f2d5fde574c`; Git-normalized `068bf70d5434685980c8268c12de5c550c7f6f2f4ed07666c8d802ff8334e9bf`.
- **Governance synchronization:** exactly six governance paths; canonical contract `docs/refactor/PHASE4_MATRIZES_SHARED_OWNER_CONTRACT.md`; the B5-P-R2 supplemental scope authorization changed only `tests/test_phase4_requisicoes_shared_owners.py` (governance-test path), reading accepted B4.1 governance from fixed commit `c587098152e97d125f41a2d26f2f414c10ae5676`; the B5-P-R3 documentary delta changes no production, test, snapshot or database path.
- **Final canonical state:** `PHASE4_B5_P_CLOSED / ACCEPTED / ADMIN_ACCESS_NEUTRAL_OWNER_ESTABLISHED / EXACT_FIVE_SHARED_SYMBOLS / MAIN_IDENTITY_REEXPORTS_PRESERVED / ZERO_LOCAL_MAIN_BODIES_FOR_FIVE / TRANSACTION_NEUTRALITY_PRESERVED / ZERO_ROUTE_MOVEMENT / ROUTES_131 / ENDPOINTS_130 / BUSINESS_PAIRS_160 / GOVERNED_PAIRS_134 / RBAC_UNMAPPED_ZERO / ACTOR_402_263_139 / MESSAGE_CATALOG_536 / ROUTE_INVENTORY_BYTE_IDENTICAL / CSRF_SHADOWS_BYTE_IDENTICAL / FULL_HERMETIC_1025_17 / INDEPENDENT_REVIEW_PASS / TECHNICAL_COMMIT_92486F87 / PUBLICATION_VERIFIED / GOVERNANCE_CLOSEOUT_PUBLISHED`. This B5-P block is **historical and superseded** by the current PHASE 4-B5 CLOSED / ACCEPTED block above. Its phase-time statements that PHASE 4-B5 was NOT AUTHORIZED and that `app/views/admin/matrizes.py` should not be begun are B5-P closeout-time history only; PHASE 4-B5 blueprint implementation is now technically published at `2a122357a79080fa66aa19c00ed5ff8533308f41` and CLOSED / ACCEPTED.

## Historical / superseded pre-publication state — PHASE 4-B5-P IMPLEMENTED / AWAITING SUPERVISOR REVIEW

Historical phase-time token: `PHASE 4-B5-P: IMPLEMENTED / AWAITING SUPERVISOR REVIEW`.
The former pre-publication state of this unit (NEVER CLOSED / ACCEPTED; technical commit/push/review PENDING; `INDEPENDENT_REVIEW_PENDING`) is phase-time history only. It is preserved as the audit trail and is superseded by the accepted B5-P closeout above. The RED/GREEN chronology, B5-P-R1 fixed-baseline classification, B5-P-R2 supplemental scope authorization and all routing/fallback history remain preserved under the accepted state block. This pre-publication history is further superseded by the current PHASE 4-B5 CLOSED / ACCEPTED block at the top of this document.

## Historical state — PHASE 4-B4.2 CLOSED / ACCEPTED

### Operational handoff — PHASE 4-B4.2-R2 external supervisor acceptance closeout (2026-08-02)

- **Authority/status:** PHASE 4-B4-A: CLOSED / ACCEPTED. PHASE 4-B4.1: CLOSED / ACCEPTED. PHASE 4-B4.2: CLOSED / ACCEPTED. B1, B2 and B3 remain CLOSED / ACCEPTED. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION; it is not closed. Matrizes is NOT STARTED / NOT AUTHORIZED BY THIS CLOSEOUT. Phase 5 and Phase 6 are NOT AUTHORIZED. Migration v4 is PROHIBITED.
- **Published technical identity:** commit `3231dbd2ff9759d8f855f2a4118102783aedea83` (`Extract admin requisitions blueprint`), parent `c587098152e97d125f41a2d26f2f414c10ae5676`; exact artifact 16 paths = 3 production + 7 tests/snapshots + 6 governance. Publication: COMPLETE. Post-publication verification: COMPLETE.
- **Accepted owner and contract:** `app.views.admin.requisicoes` canonically owns exactly nine legacy global endpoints / 12 route-method pairs and the five-item route-private closure (`ALLOWED_EXCEL` plus four helpers). `main` preserves identity exports. RBAC remains `view=4`, `edit=5`, `full=3`; the B4.1 neutral owners remain preserved; no direct or dynamic `app -> main` edge exists. Route inventory is unchanged.
- **Accepted evidence:** final hermetic `1005 passed / 17 deselected / 0 failed / 0 errors / 362.33s`; index-visible `57 passed / 0 failed / 0 errors`; post-publication `56 passed / 1 deselected / 0 failed / 0 errors`. Routes 131; endpoints 130; governed pairs 134; RBAC unmapped 0; actor matrix 402 = 263 allowed + 139 denied; message catalog 536. The post-commit `HEAD^ -> HEAD` CSRF comparison is `[5, 5]`, owner-only deltas. Canonical SQLite opens: 0.
- **Authorized compatibility correction:** the supervisor expressly added `tests/test_admin_snapshot_diagnostics.py` for the canonical monkeypatch owner change. The B1/B3 blueprint tests were mechanically advanced to recognize `requisicoes.py`; assertions were not weakened.
- **Accepted technical review:** logical route `flash_free`; provider/model `opencode` / `opencode/deepseek-v4-flash-free`; session `ses_03ff5695bffeEBNIZRiSVZonYz`; exit 0; cost 0; fallback none; mutation 0; PASS / blocking NONE / scope EXACT / behavior PRESERVED. The hash-method reproducibility finding was VALID / NONBLOCKING / DOCUMENTARILY CORRECTED / NO TECHNICAL IMPACT.
- **Accepted documentary addendum:** logical requested route `flash_free`; selected route `flash_normal`; provider/model `opencode-go` / `opencode-go/deepseek-v4-flash`; session `ses_03fce2ebbffejkf6IUdfPrsxF3`; exit 0; fallback `FALLBACK_FREE_BUDGET_EXHAUSTED`; router cost `0.0011911032 USD`; observed step-finish aggregate `0.0180531456 USD`; mutation 0; outside-scope reads 0; PASS / blocking NONE.
- **Accepted identities:** `WORKTREE_RAW_CONTENT_MANIFEST_SHA256_V1` `1b8435a9db10f8a2ae680f60c17a9ad0a723eed88066a834ca59255bf7b8cc0e`; `GIT_CANONICAL_CONTENT_MANIFEST_SHA256_V1` `c362566627667ba684765ad3ea8fdeb9abf7678dd52e185cedd3ed8b08a891b4`; governance raw/canonical six-path hashes `19519783e02bf983f820d357e6c2b250db541581fae57f6988e64cf1900f544d` / `b44ae4231aaaeb022c2cfc2ca94f20be76a94dd7e82c51e97966866d294d0ceb`; production/test raw identity `f60ebdab5cd1e7aa2d98d9ade66534925c5a077326dff2e70b24c72e6390c037`. Raw worktree and Git-canonical identities are intentionally distinct.
...
- **Historical pre-publication state (superseded):** RED/GREEN chronology, the first failed full lane (`1001 passed / 4 failed / 17 deselected`), review/fallback history, hash normalization history, and the explicit `tests/test_admin_snapshot_diagnostics.py` scope expansion remain phase-time evidence. The former prepublication candidate state and its outstanding documentary, Git-publication and external-acceptance gates are not current.
- **Governance closeout:** authorized subject `Record acceptance of Phase 4-B4.2`; identity resolves through Git history. No production, test/snapshot, route, database, schema or migration path is changed by this closeout.
- **Final canonical state:** `PHASE4_B4_2_CLOSED / ACCEPTED / REQUISICOES_BLUEPRINT_ESTABLISHED / EXACT_9_LEGACY_ENDPOINTS_PRESERVED / EXACT_12_ROUTE_METHOD_PAIRS_PRESERVED / RBAC_4_VIEW_5_EDIT_3_FULL_PRESERVED / B4_1_NEUTRAL_OWNERS_PRESERVED / ZERO_APP_TO_MAIN_EDGE / ROUTES_131 / ENDPOINTS_130 / GOVERNED_PAIRS_134 / RBAC_UNMAPPED_ZERO / ACTOR_402_263_139 / MESSAGE_CATALOG_536 / CSRF_EXACT_FIVE_OWNER_ONLY_DELTAS / FULL_HERMETIC_1005_17 / INDEPENDENTLY_REVIEWED / TECHNICAL_COMMIT_3231DBD2 / PUBLICATION_VERIFIED / GOVERNANCE_CLOSEOUT_PUBLISHED`. Do not begin the next cohort.
...
- **Authority/status:** PHASE 4-B4-A: CLOSED / ACCEPTED. PHASE 4-B4.1: CLOSED / ACCEPTED. B1, B2 and B3 remain CLOSED / ACCEPTED. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION. PHASE 4-B4.2: NOT AUTHORIZED. Phase 5: NOT AUTHORIZED. Phase 6: NOT AUTHORIZED. Migration v4: PROHIBITED. No B4.1 technical correction remains open.
- **Published technical identity:** commit `73ebf0dc34681e74e778759af476e1cd2f981444` (`Extract requisition shared owners`), parent `185426daccc9f0eb0dba4497248100c1a88d15fa`; exact 20-path artifact = 7 production + 7 tests + 6 governance. Publication: COMPLETE. Post-publication verification: COMPLETE.
- **Accepted owners:** `app.settings` owns the eight app-settings helpers; `app.requisitions` owns `auto_indefer_devolvidas`; `app.matrix_scope` owns exactly six matrix-scope symbols. Configurações and `main` preserve identity exports. Aluno imports `get_effective_matriz_for_turma` directly from its neutral owner.
- **Accepted reverse-edge history:** B2 established six Aluno lazy-main edges at B2-time. B4.1 removes only `get_effective_matriz_for_turma`; the accepted current five are `ensure_admin_arquivos_table`, `get_admin_arquivo`, `get_student_request_update_alert`, `list_active_admin_alertas`, and `mark_student_request_updates_seen`.
- **No-route boundary:** NO ROUTE MOVEMENT. EXACT 9 REQUISICOES ROUTES REMAIN IN MAIN.PY / 12 route-method pairs. `app/views/admin/requisicoes.py` is absent and requires separate B4.2 authority.
- **Accepted gates:** final hermetic 984 passed / 17 deselected; index-visible 170 passed; post-publication 170 passed. Routes 131; endpoints 130; governed pairs 134; RBAC unmapped 0; actor matrix 402 = 263 allowed + 139 denied; message catalog 536.
- **Accepted reviews:** technical review `flash_free`, provider/model `opencode` / `opencode/deepseek-v4-flash-free`, session `ses_040d538bfffegBsAJQbLJrnqSV`, exit 0, cost 0, no fallback, mutation 0, PASS / findings NONE. Documentary addendum `flash_free`, same provider/model, session `ses_040ae7625ffeHCGZxyXoMzoQmw`, exit 0, cost 0, no fallback, mutation 0, PASS / blocking and nonblocking findings NONE. Its read-scope deviation remains `READ_SCOPE_DEVIATION / NON_MUTATING / PHYSICALLY_RECONCILED / NO_CANDIDATE_IMPACT`.
- **Accepted hashes:** technical reviewed tracked diff `f4b6cb00b4365cc7c20af5fcba1ac736ece1bab0ab9c6e0f89b19084799727f9`; complete pre-addendum `ddc73cb94786899595f3cce9d577bdbcca961a082c83866498a36ebd4687a23f`; non-governance `74649089be0699dff4440260bdb11793b4e5793550f17456893f4a281bd6096b`; final publication `bdd947a18df900aac691ce12683f393a82d8c8efc3e28a8c54c55226d7bf2d4a`; governance `301a6a891936338bd1a752ff0be65dd7855302d1e3a8e81f9bcbd94964b66a71`. Reviewer “8 tests” wording remains `REJECTED_NON_MATERIAL / STALE_NARRATIVE_COUNT / NO_TECHNICAL_CONSEQUENCE`; exact count is seven.
- **Physical invariants:** `database.db` remains 544768 bytes / SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent. Protected residual remains 17420 bytes / SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`.
- **Next exact action:** none for B4.1. B4.2 is not authorized and requires a new separate order.

## Historical state — PHASE 4-B3 CLOSED / ACCEPTED

### Current operational handoff — PHASE 4-B3-R3 external acceptance closeout (2026-08-01)

- **Current decision:** PHASE 4-B3: CLOSED / ACCEPTED. PHASE 4-B1 and PHASE 4-B2 remain CLOSED / ACCEPTED; B2 commit `17e468ad938e873e1f9e9c303808ad31b9f3806b` remains published with post-publication verification COMPLETE. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION. PHASE 4-B4, Phase 5 and Phase 6: NOT AUTHORIZED. Migration v4: PROHIBITED. No B3 technical correction remains open; B4 requires separate authority.
- **Published technical identity:** commit `50801b6bdddc4d2772853c13f4905c49e8c996cf` (`Extract admin activities blueprint`), parent `81cc6b10b893f1d34bd211a527e9fd12c3b6bbbe`; exact 16-path artifact = 6 production + 4 tests/snapshots + 6 governance. Publication: COMPLETE. Post-publication verification: COMPLETE.
- **Exact accepted manifest:** `AGENT_HANDOFF.md`, `PROJECT_STATE.md`, `app/__init__.py`, `app/activity_catalog.py`, `app/uploads.py`, `app/views/admin/atividades.py`, `docs/DOCUMENTATION_INDEX.md`, `docs/mapeamento/05_avaliacao_refactor.md`, `docs/refactor/ARCHITECTURE_REFACTOR_LEDGER.md`, `docs/refactor/PHASE4_ATIVIDADES_BLUEPRINT_CONTRACT.md`, `main.py`, `tests/_artifacts/csrf_inventory_shadow_off.json`, `tests/_artifacts/csrf_inventory_shadow_on.json`, `tests/test_phase4_atividades_blueprint.py`, `tests/test_phase4_configuracoes_blueprint.py`, `utils/messages.py`.
- **Accepted owners and boundaries:** `app.views.admin.atividades` owns exactly 22 legacy global endpoints / 29 route-method combinations; `app.activity_catalog` owns 14 neutral shared helpers; `app.uploads` owns neutral `save_upload`; `main` preserves compatibility identities. Matrizes/Requisições route cohorts remain unmoved. No B2 helper, schema, migration, RBAC, template or static ownership changed.
- **Accepted reconciliations:** message inventory 536→536 with zero key/default/kind/semantic delta; both CSRF snapshots contain exactly 15 owner-only deltas each; route inventory 131 routes / 130 endpoints; governed pairs 134; RBAC unmapped 0; actor matrix 402 = 263 allowed + 139 denied.
- **Accepted gates:** final hermetic 974 passed / 17 deselected; index-visible 300 passed; post-publication 300 passed. Technical review PASS / findings NONE. Documentary review addendum PASS / findings NONE.
- **Accepted hashes:** reviewed technical hash `ec96796d3541710a36ac8121e40ffd888737c7c926f191a28034482cedbfd556`; final publication hash `c41ffe5b7328b6d5a986dbdc28f054fe89641496589003b4e5649ff88463cc19`; governance hash `af2906ef0fa9fef7fdd469dd4e967cd1c914b4bfb21fc2a132b8d74c2d8dfd27`; non-governance hash `13b0af13e653641d75d2466d7d8d69090e655a18e28bb678a7090dbe0e2ecab0`.
- **Review history:** provisional FREE `opencode/deepseek-v4-flash-free`, session `ses_0425bf1cbffeIxMwsDQn0etSEC`, cost 0: PROVISIONAL / UNUSABLE DELIVERY / NO VERDICT because the external bundle was inaccessible. Final FREE session `ses_0422f0e1cffepCXyNpg47dWpoq`, cost 0: TIMEOUT / NO ACCEPTED DELIVERY. Accepted technical fallback: logical `flash_free` → selected `flash_normal`, router trigger `FALLBACK_FREE_BUDGET_EXHAUSTED`, task antecedent the immediately preceding FREE timeout; effective `opencode-go/deepseek-v4-flash`, session `ses_04224ca47ffe5qAwwHGtxlR7i7`, router cost `0.0010424344 USD`, observed step-finish aggregate `0.0206550400 USD`, mutation 0, PASS / findings NONE; IAsup PASS ACCEPTED.
- **Documentary addendum history:** first verdict REJECT because of stale historical Documentation Index wording; correction was limited to governance. Final addendum used `opencode-go/deepseek-v4-flash`, session `ses_04203dca3ffe8OM83rhbHyjqYI`, reviewed final publication hash `c41ffe5b7328b6d5a986dbdc28f054fe89641496589003b4e5649ff88463cc19`, governance hash `af2906ef0fa9fef7fdd469dd4e967cd1c914b4bfb21fc2a132b8d74c2d8dfd27`, non-governance hash `13b0af13e653641d75d2466d7d8d69090e655a18e28bb678a7090dbe0e2ecab0`; final verdict PASS.
- **Historical pre-publication state (superseded):** PHASE 4-B3 IMPLEMENTED / AWAITING SUPERVISOR REVIEW; review, staging, commit, push and publication were pending at that phase-time. These pending states are not current.
- **Historical B2-time boundary (superseded):** PHASE 4-B3: NOT AUTHORIZED. The later separate B3 order and accepted technical publication superseded only that phase-time prohibition.
- **Scope-expansion time reconciliation:** historical pre-publication classification `PRE_REVIEW_SCOPE_EXPANSION / MECHANICALLY_CAUSED_BY_CANONICAL_OWNER_MOVE / NOT_STAGED / NOT_COMMITTED / PUBLICATION_WITHHELD`. Current disposition: `EXACT_DELTA_PROVED / SUPERVISOR_RECONCILED / INCLUDED_IN_ACCEPTED_B3_COMMIT / NO_GENERIC_RETROACTIVE_AUTHORITY`.
- **Physical invariants:** `database.db` 544768 bytes / SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent. Protected ignored residual 17420 bytes / SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`; preserved unchanged and outside the index.
- **Next exact action:** none for B3. A PHASE 4-B4 order is the only next architectural action and is not authorized by this closeout.

## Historical state — PHASE 4-B2 CLOSED / ACCEPTED — PHASE 4 OPEN / INCREMENTAL IMPLEMENTATION

### Current operational handoff — PHASE 4-B2-R3 external acceptance closeout (2026-08-01)

- **Baseline/physical state:** workspace `D:\OneDrive\Programação\SGAA_clean_baseline`, branch `refactor/architecture-safety-net`. Accepted technical commit `17e468ad938e873e1f9e9c303808ad31b9f3806b` (`Extract versioning subsystem ownership`), parent `2fbe4954106dc8d410f6495ca8bd4b1956b326d2`, is committed and pushed. Post-publication verification: COMPLETE. Its exact artifact is 24 paths = 8 production + 10 tests + 6 governance.
- **Status:** PHASE 4-B2 CLOSED / ACCEPTED. There is no open B2 correction. PHASE 4 remains OPEN / INCREMENTAL IMPLEMENTATION; B1 remains CLOSED / ACCEPTED.
...
- **Accepted gates:** final full hermetic 954 passed / 17 deselected / 343.75s / exit 0; index-visible 271 passed; post-publication 282 passed. Routes 131; endpoints 130; governed pairs 134; RBAC unmapped 0; actor matrix 402 = 263 allowed + 139 denied. The recovered first full 949 passed / 2 stale lazy-map failures / 17 deselected / 384.10s remains historical evidence.
- **Manifest:** exact published artifact 24 paths = 8 production + 3 original-pool tests + 7 R1-expanded tests + 6 governance. `tests/test_activity_versioning_resolver.py` is the only authorized original-pool omission because no semantic delta was needed. All seven expanded tests are retained for candidate sandbox, ownership, lazy-map, B1 and RBAC compatibility. No 25th/26th path.
...
- **Final documentary review addendum:** route `flash_free`; provider/model `opencode` / `opencode/deepseek-v4-flash-free`; session `ses_0431aa4d7ffev4hTZImBDA86Ca`; cost 0; verdict PASS; no fallback.
- **Scratch nonconformity:** external `baseline_main.py`, SHA-256 `2652d1213d7f0b5ac577ebddb528341448e9eb0afb8b41d051e5826a56d4af48`: EXTERNAL_REVIEW_SCRATCH / OUTSIDE_REPOSITORY / NOT_STAGED / NOT_COMMITTED / SELECTIVELY_REMOVED / NO_CANDIDATE_OR_INDEX_IMPACT; do not recreate.
- **Published identities:** final published diff SHA-256 `2a98b4a4ff9747745335259d0e5aad2c18eb9a8c0bc4762c1ea2681bc7571eec`; published technical diff SHA-256 `76f8089b577f04c7d9f4b2090627c1767ec2f2f4b9b3abcb6e973c1e41959e50`; technical physical aggregate `d54bec738de05934906e03c610f513dac18269206fdf3a6de9b0a79d64cd99a3`.
- **Formal decision / next action:** PHASE 4-B2: CLOSED / ACCEPTED. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION. PHASE 4-B3: NOT AUTHORIZED. Phase 5: NOT AUTHORIZED. Phase 6: NOT AUTHORIZED. Migration v4: PROHIBITED. B3 requires new authority; no B2 technical action remains.
...
- **Status/accepted technical artifact:** PHASE 4-B1 CLOSED / ACCEPTED. Accepted commit `cd8a76b2484abc376174332578ecd8be4b8206ea`, subject `Extract admin configuration blueprint`, parent `7f393c72ad3e9d70eae4c06ee41e0d74881e40f2`; exact 13-path artifact committed, published and post-publication verified.
- **Ownership:** `app/views/admin/configuracoes.py` solely defines exactly eight Configurações/Mensagens views and the eight settings helpers. `app/views/admin/__init__.py` solely owns the reusable immutable legacy registrar. `main` retains identity imports for all sixteen compatibility exports. No `app -> main` import exists.
- **Registration:** `create_app(register_admin_configuracoes_blueprint=True)` registers the unique blueprint by default after existing core registration; explicit `False` provides factory isolation. `Blueprint.record_once` callbacks use `state.app.add_url_rule` with exact global legacy endpoints. Duplicate blueprint registration, endpoint collision, and rule/method collision fail explicitly before B1 route mutation.
- **Accepted validation/review:** original B1 RED 15 failed / 6 passed and focused GREEN 21 passed; B1-R1 recursive inventory/owner RED 4 failed and GREEN 4 passed; the two formerly failing CSRF nodes pass 2/2; required targeted matrix 47/47; full hermetic 939 passed / 17 deselected; post-publication 91 passed. Routes 131, endpoints 130, governed pairs 134 and RBAC unmapped 0. Canonical database, route/CSRF snapshots and protected residual remained byte-identical, with WAL/SHM absent. Independent read-only review used `opencode/deepseek-v4-flash-free`, session `ses_0453cc57fffexsyJU43Oyi1wwz`, cost 0, no fallback/mutation, verdict PASS; no functional residual was demonstrated.
- **Exact accepted 13-path manifest:** `AGENT_HANDOFF.md`, `PROJECT_STATE.md`, `app/__init__.py`, `app/views/admin/__init__.py`, `app/views/admin/configuracoes.py`, `docs/DOCUMENTATION_INDEX.md`, `docs/mapeamento/05_avaliacao_refactor.md`, `docs/refactor/ARCHITECTURE_REFACTOR_LEDGER.md`, `docs/refactor/PHASE4_ADMIN_BLUEPRINT_COMPATIBILITY_CONTRACT.md`, `main.py`, `tests/test_csrf_inventory_audit.py`, `tests/test_phase4_configuracoes_blueprint.py`, `utils/messages.py`.
- **Accepted discovery correction:** B1-R1 uses deterministic filesystem-recursive repository-tree discovery under `app/views/**/*.py` for messages and canonical CSRF owners. The implementation does not query or filter through the Git index; clean-worktree and repository-boundary assumptions remain execution-protocol gates.
- **Preserved Phase 3 closeout tokens:** `MACRO PHASE 3 CLOSED / ACCEPTED`, `PHASE 3-B11 CLOSED / ACCEPTED`, and `PHASE 3-B11-R1 CLOSED / ACCEPTED` remain true. The historical sentence `Phase 4 remains NOT AUTHORIZED` is retained only as the superseded Phase 3 boundary. `migration v4 remains PROHIBITED` remains active.
- **Formal decision:** `PHASE 4-B1: CLOSED / ACCEPTED`; `PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION`; `PHASE 4-B2: NOT AUTHORIZED`; `PHASE 5: NOT AUTHORIZED`; `PHASE 6: NOT AUTHORIZED`; `Migration v4: PROHIBITED`. Phase 4 is not closed. `dashboard.py` and `admin_meus_dados` ownership remain unresolved. No template, JavaScript, database, migration, RBAC, snapshot or runtime delta is accepted.
- **Next exact action:** none for B1; there is no open B1 correction. PHASE 4-B2 requires a new separate order and is not authorized by this closeout.

### Current operational handoff — Macro Phase 3 final acceptance closeout (2026-07-31)

- **Status and accepted identities:** Macro Phase 3, PHASE 3-B11, and PHASE 3-B11-R1 are CLOSED / ACCEPTED by external supervisor decision. The accepted B11 technical commit is `c9009bf3d68950ad4e0499b65928603e84bee341`, subject `Unify database initialization ownership`, parent `e63e1a66b9d2ebad7253a0efd2e0a367b89b8b8a`. The accepted B11-R1 governance commit is `630d4eb448b992bdc3beb28752c30717989312bb`, subject `Record B11 publication and review closeout`, parent `c9009bf3d68950ad4e0499b65928603e84bee341`. B11 technical commit: COMMITTED AND PUSHED. B11 post-commit verification: COMPLETE. The feature-branch baseline before this acceptance closeout is `630d4eb448b992bdc3beb28752c30717989312bb`.
- **Final accepted architecture:** `app.db.init_db` is the sole defining initializer; `main.init_db is app.db.init_db`; defining bodies 1 (`app.db` 1, `main` 0); `app.db → main` dependencies 0; lazy bridge entries 0; `_init_db_impl`, `_get_main_db_helpers`, and `_sync_database_from_main` absent. Canonical connection ownership belongs to `app.db`. `app.db` directly owns preferred-matrix selection, app/cloud/turma schema helpers, related defaults, and a module-local logger without a handler. Factory/seed remains independent from `main`; restore and development startup resolve the canonical initializer.
- **Accepted migration and safety contract:** qualified calls remain 72 `main.init_db(...)` plus 5 `app.db`/`app_db.init_db(...)`, with three bare imported-owner calls tracked separately. `SCHEMA_VERSION == 3`; registry is exactly v1 `baseline_schema_management`, v2 `normalize_atividades_schema`, and v3 `normalize_activity_versioning_core`, with no gap, duplicate, or migration v4. Deterministic transaction/failure postconditions are accepted unchanged.
- **Mandatory Phase 3 objectives satisfied:** canonical database connection ownership, single defining initialization, versioned schema maintenance, reverse-dependency/lazy-bridge retirement, and the test/safety contract are complete. The optional repository-layer extraction is OPTIONAL / NOT IMPLEMENTED / NOT A CLOSURE BLOCKER and is not silently assigned to Phase 4.
- **Accepted test evidence:** final hermetic `913 passed / 17 deselected / 416.66s / exit 0`; index-visible 67 passed; route/RBAC `3 passed / routes 131 / unmapped 0`; B11 post-publication `212 passed / 42.37s`; latest governance/route/RBAC lane 12 passed.
- **Accepted independent review:** verdict `APPROVE`; Material/Critical/High findings 0. Accepted LOW: the `main` compatibility import creates intentional, tested compatibility coupling. The prior routing/session/cost record remains permanently preserved in the canonical Phase 3 contract. Do not repeat the independent review.
- **Reviewer nonconformity 1 — accepted with declaration:** `SAME_SESSION_OUTPUT_RECOVERY / SECOND_INVOCATION_PROCESS_NONCONFORMITY / SAME_PROVIDER_MODEL_AND_SESSION / NO_NEW_TOOLS / NO_DIFF_CHANGE / NO_REVIEWER_SUBSTITUTION / USABLE_VERDICT_RECOVERED / ACCEPTED_WITH_DECLARATION`. The first post-opt-in inference produced no final text; textual continuation in the same provider/model/session recovered the verdict. No fallback, repository mutation, reviewer substitution, tool expansion, or candidate change occurred.
- **Reviewer nonconformity 2 — accepted with declaration:** `EXTERNAL_TEMPORARY_FILE_MUTATION / OUTSIDE_REPOSITORY / NOT_STAGED / NOT_COMMITTED / NO_REPOSITORY_OR_INDEX_IMPACT / UNAUTHORIZED_BY_REVIEW_CONTRACT / ACCEPTED_WITH_DECLARATION`. Declared artifact `/tmp/candidate.diff`, reported size 144049 bytes and reported SHA-256 `fcd1b62e141434dccaa89dabe9b604afe61977c96490674503b9410185627771`; preserved without inspection, alteration, or deletion under B11-R1.
- **Physical safety:** canonical `database.db` remains 544768 bytes with SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM absent. Protected ignored residual remains 17420 bytes with SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`, index-absent. No functional residual or production delta was demonstrated.
- **Formal decision/boundaries:** PHASE 4-B1 CLOSED / ACCEPTED. PHASE 4 OPEN / INCREMENTAL IMPLEMENTATION and is not closed. PHASE 4-B2, Phase 5 and Phase 6 are NOT AUTHORIZED; migration v4 is PROHIBITED. `dashboard.py` and `admin_meus_dados` ownership remain unresolved. The prior statement `Phase 4 remains NOT AUTHORIZED` is historical and superseded only for the accepted B1 scope.
- **Next exact action:** none for B1; there is no open B1 correction. PHASE 4-B2 requires a new separate order and is not authorized by this closeout.

### Historical / superseded recovery narrative — B11 pre-publication state (2026-07-30)

After the first FREE delivery timed out without usable output, the phase stopped before staging and withheld commit/publication. That phase-time restriction was correct for the then-observed state. A later accepted same-session recovery and explicit paid-Flash review record produced the usable `APPROVE` verdict above; Git now proves the B11 technical artifact published at `c9009bf3d68950ad4e0499b65928603e84bee341`. The former pre-publication restriction and request for another review attempt are historical only and do not govern B11-R1.

### Historical operational handoff — PHASE 3-B10-R1 governance correction (2026-07-30)

- **Status/baseline:** B10-R1 GOVERNANCE CORRECTION IMPLEMENTED / AWAITING SUPERVISOR REVIEW. B10 commit `8fe0345eab08e312f7e015730f70d02327e7eb5f` (`Version activity versioning core schema`, parent `37c8757e4ef41647c971ad8a974c853dec6ce4e7`) is published and technically valid; B9 CLOSED / ACCEPTED; B11 unauthorized. Workspace `D:\OneDrive\Programação\SGAA_clean_baseline`, branch `refactor/architecture-safety-net`, HEAD/upstream/live remote `8fe0345eab08e312f7e015730f70d02327e7eb5f`, divergence 0/0 and clean.
- **B10-R1 purpose:** Correct the published B10 governance record to match the actual committed artifact. The B10 governance documents described a 14-path manifest, but the actual commit contains 15 paths. The undocumented fifteenth path is `tests/test_ref_0c_b1_p0_access_context_transactions.py`. This R1 governance-only correction records the true manifest, classifies the nonconformity, corrects the lazy-bridge description in the canonical contract, and synchronizes all governance documents.
- **Actual B10 commit artifact:** `8fe0345eab08e312f7e015730f70d02327e7eb5f`; 15 files, 1726 insertions, 464 deletions. Subject `Version activity versioning core schema`. Parent `37c8757e4ef41647c971ad8a974c853dec6ce4e7` (`Version atividades schema migration`).
- **Fifteenth path:** `tests/test_ref_0c_b1_p0_access_context_transactions.py`. Delta: only the final expected migration metadata changed from `(2, "normalize_atividades_schema")` / `user_version == 2` to `(3, "normalize_activity_versioning_core")` / `user_version == 3`. Classification: `MECHANICALLY_REQUIRED_V3_EXPECTATION_UPDATE / CONTENT_CORRECT / NOT_PREAUTHORIZED / PROCESS_NONCONFORMITY / NO_PRODUCTION_SCOPE_EXPANSION`. The path is not reverted; its content is accepted as mechanically required by the SCHEMA_VERSION change.
- **Actual B10 15-path manifest:** `app/db_maintenance.py`, `app/db.py`, `main.py`; `tests/test_activity_versioning_core_migration_v3.py` (new), `tests/test_atividades_schema_migration_v2.py`, `tests/test_activity_versioning_leaf_schema_ownership.py`, `tests/test_backup_settings_ownership.py`, `tests/test_db_schema_maintenance.py`, `tests/test_phase3_schema_startup_transaction_contract.py`, `tests/test_ref_0c_b1_p0_access_context_transactions.py` (undocumented fifteenth path); canonical contract, Documentation Index, Architecture Ledger, `PROJECT_STATE.md`, this handoff.
- **Lazy-bridge contract corrected:** The canonical contract section 3 incorrectly stated that `app.db._init_db_impl` obtains three dependencies through the lazy bridge. After B10, the lazy bridge contains exactly two entries: `get_preferred_matriz_for_curso` and `logger`. `ensure_atividade_versioning_schema` is a direct import from `app.db_maintenance` in both `app.db` and `main.py`. This correction is applied to the canonical contract.
- **B10-R1 authorized manifest:** Exactly 6 governance paths: `AGENT_HANDOFF.md`, `PROJECT_STATE.md`, `docs/DOCUMENTATION_INDEX.md`, `docs/refactor/ARCHITECTURE_REFACTOR_LEDGER.md`, `docs/refactor/PHASE3_SCHEMA_STARTUP_TRANSACTION_CONTRACT.md`, `tests/test_phase3_schema_startup_transaction_contract.py`. No production delta. No B11 work.
- **Next action:** supervisor review of the B10-R1 governance correction. PHASE 3-B11 remains unauthorized.

### Historical operational handoff — PHASE 3-B10 activity-versioning core migration v3 (2026-07-30)

- **Status:** COMMITTED AND PUSHED / POST-COMMIT VERIFIED / AWAITING SUPERVISOR REVIEW. B10 governance record contained a process nonconformity (documented 14-path manifest vs actual 15-path artifact) corrected by B10-R1. B9 CLOSED / ACCEPTED. B11 unauthorized.
- **Commit:** `8fe0345eab08e312f7e015730f70d02327e7eb5f`, subject `Version activity versioning core schema`, parent `37c8757e4ef41647c971ad8a974c853dec6ce4e7`. Actual manifest: 15 paths, 1726 insertions, 464 deletions.
- **Undocumented fifteenth path:** `tests/test_ref_0c_b1_p0_access_context_transactions.py` (mechanically required v3 expectation update; content correct; not preauthorized; process nonconformity; no production scope expansion).
- **Core changes:** SCHEMA_VERSION 3; v3 registry `normalize_activity_versioning_core`; legacy self-transactional rebuild retired from main.py; core owner in `app.db_maintenance`; lazy bridge exactly `get_preferred_matriz_for_curso` and `logger`.
- **Test evidence:** Focused v3+v2: 49 passed; 6-test gate: 163 passed; regression: 352 passed, 17 deselected; full hermetic: 898 passed, 17 deselected; post-commit: 231 passed. Routes 131; RBAC unmapped 0.

### Historical operational handoff — PHASE 3-B9 versioned atividades schema rebuild (2026-07-27)

- **Status/baseline:** IMPLEMENTED / LOCALLY VERIFIED / INDEPENDENTLY REVIEWED / PUBLICATION PENDING AT COMMIT-TREE TIME; B8 CLOSED / ACCEPTED; B10 unauthorized. Workspace `D:\OneDrive\Programação\SGAA_clean_baseline`, branch `refactor/architecture-safety-net`, baseline HEAD/upstream/live remote `42ad0b500fe26fb2a4f49a2f8655d0217233af75`, parent `d1947b0c11506045b8d52bd235bc7381a2ca22c9`, divergence 0/0 and clean before mutation. Authorized commit subject: `Version atividades schema migration`.
- **Root cause/decision:** PATH A is rejected: deferred FK checking does not defer `ON DELETE CASCADE`, so dropping the parent can erase children with a clean FK check. PATH B is selected: disable and verify FK before an isolated early `BEGIN IMMEDIATE` migration transaction.
- **Owner/target:** `app.db_maintenance` solely owns registry v2 `normalize_atividades_schema`, the exact target `(id, grupo, nome, descricao, limite_horas, tipo_atividade, tem_limitacao, tipo_limitacao, limite_horas_total, limite_horas_semestral, documentos_json)`, accepted-shape predicate/copy/rebuild, physical validation, and early transaction runner. V1 remains byte-semantically unchanged.
- **Atomicity/data proof:** existing caller transaction is rejected before mutation. CREATE/copy/DROP/rename/v2-row/user-version failures roll back the whole bounded migration; FK state is restored; no false v2, false user-version or orphan temp remains. Exact `requisicoes`, `matrizes_atividades_itens`, `atividade_legacy_map`, activities IDs/values/JSON and sequence state survive.
- **Init/route/lazy delta:** `app.db._init_db_impl` and `main.init_db` invoke the early runner immediately after connection acquisition and before dependent DDL, create identical fresh eleven-column tables, then retain final unrestricted migration application and one bootstrap commit. The recurring helper, mapped-route trigger, six defensive ALTERs/normalization, and only its lazy key/retrieval are retired. Remaining lazy order: versioning, preferred-matrix, logger.
- **Isolation:** B8 leaf table/trigger/index helper SQL and ownership are unchanged; activity-versioning core/rebuild/requisicoes compatibility remain in `main.py`. No access, backup, route response, RBAC, harness encoding or B10 behavior changed.
- **TDD/evidence:** initial RED was 20 expected failures; a supplemental RED proved that `user_version=2` with legacy physical schema did not hard-stop (`1 failed`, expected `RuntimeError`). The first complete hermetic run was 869 passed, 2 failed and 17 canonical D73H deselected: both failures were CSRF inventory/message-catalog collection, localized to literal infrastructure messages passed directly to `RuntimeError`, not to route, CSRF or snapshot drift. The bounded repair added the pre-mutation contradiction gate and `SchemaMigrationStateError(RuntimeError)`, keeping migration failures outside the editable UI catalog. Focused v2 GREEN is 23 passed; isolated CSRF is 2 passed; critical aggregate is 173 passed; the pre-staging Windows `PYTHONUTF8=1`/`-B` hermetic suite was 872 passed / 17 deselected. Selective staging made the new B9 test visible to the Git-aware caller inventory and exposed one localized contract failure after 78 passes: the exact `main.init_db` manifest still declared 73 callers and omitted `_run_init_on_temp_database`. The authorized test/contract paths were corrected to 74 callers; the node passed 1, the post-staging lane passed 79, and the required full rerun passed 873 with 17 deselected and zero failures/errors in 357.56s. All 14 paths and physical invariants remained stable; no migration operation was repeated.
- **Routing/review:** Stage A FREE actual `opencode` / `opencode/deepseek-v4-flash-free`, session `ses_05ac5d405ffenqwkRrh7QeBAIo`, cost 0, rejected — unusable delivery. Explicit Stage-A fallback `FALLBACK_FREE_UNUSABLE_DELIVERY`: `opencode-go` / `opencode-go/deepseek-v4-flash`, session `ses_05abb3bd1ffeut3C2R2SSaIuw7`, cost `0.0021259168`, accepted after IAsup adjudication. Independent read-only review attempted FREE first; the FREE transport was technically unusable/incomplete and the final delta re-review used explicit `FALLBACK_FREE_BUDGET_EXHAUSTED` after two recorded FREE uses: `opencode-go` / `opencode-go/deepseek-v4-flash`, session `ses_05a18eb14ffejH0B4Z7zROy427`, reported final-call cost `0.0005707464`, exit 0, zero mutation, verdict APPROVE / findings NONE. IAsup independently checked the transaction boundary, FK restoration, contradiction timing, `documentos_json`, all three FK-child sets and the exact lazy delta, and accepted the verdict.
- **Changed manifest:** exactly 14 authorized paths: three production, six test, canonical contract, Documentation Index, Architecture Ledger, `PROJECT_STATE.md`, and this handoff. The three-path waiver is used only for the named historical guards; no fourth additional path is required.
- **Next exact action:** selectively stage the exact 14-path manifest, run index-visible gates, create the bounded commit, publish only the feature branch and execute post-publication gates. This commit tree intentionally records publication as pending and contains no future/self-referential commit SHA. Do not begin PHASE 3-B10.

### Historical operational handoff — PHASE 3-B8 activity-versioning leaf extraction (2026-07-27)

- **Status/baseline:** IMPLEMENTED / LOCALLY VERIFIED / INDEPENDENT REVIEW APPROVED / PUBLICATION PENDING; B7 CLOSED / ACCEPTED; B9 unauthorized. Workspace `D:\OneDrive\Programação\SGAA_clean_baseline`, branch `refactor/architecture-safety-net`, baseline HEAD/upstream/live remote `d1947b0c11506045b8d52bd235bc7381a2ca22c9`, subject `Separate backup schema from runtime configuration`, parent `b328789f16cc4db0173e58d2d2454902565d0610`, divergence 0/0 and clean before mutation. Authorized subject: `Extract activity versioning leaf schema ownership`; final SHA resolves only after the bounded commit and publication.
- **Exact block classification/owners:** self-transactional migration/default-fix/index-hardening predicates and delegates remain in `main`; matrix-link ensure plus `atividade_base`, `norma_atividade`, `atividade_versao`, six core triggers and seven core indexes remain in `main`; three new `app.db_maintenance` helpers own exactly the four leaf tables, two AAC→AEU transition triggers and eight leaf indexes; requisicoes three-column compatibility block and its index remain in `main`.
- **Call-order proof:** migration/fix/hardening → matrix links → core tables → `ensure_atividade_versioning_leaf_tables` → core triggers → `ensure_atividade_versioning_leaf_triggers` → requisicoes ALTERs → core indexes → `ensure_atividade_versioning_leaf_indexes` → requisicoes index. Frozen baseline AST/SQL comparison proves only those three contiguous leaf blocks were replaced and the public orchestrator retains one `conn` argument.
- **Dependencies/imports:** transition leaf depends twice on `atividade_versao`; matrix/norma depends on `matrizes_atividades` and `norma_atividade`; matrix/version item depends on `matrizes_atividades` and `atividade_versao`; legacy map depends on `atividades` and `atividade_base`; no leaf-to-leaf cycle exists. Both triggers depend only on the transition and version tables. Import direction is `main → app.db_maintenance`; subprocess isolation proves no reverse import of `main`.
- **Transactions:** each extracted helper consists only of 4/2/8 ordered `conn.execute(CREATE ...)` calls. Static and dynamic tests prove no commit, rollback, savepoint, BEGIN, PRAGMA, executescript, ALTER or DML ownership; outer rollback removes uncommitted helper DDL, and injected failures in every helper propagate while the outer transaction remains active. The `_recreate_atividade_versao` internal transaction remains isolated and AST-identical in `main`.
- **Lazy/core isolation:** `app/db.py` is byte-identical to baseline, with exact four keys and retrievals in the accepted order. `_VERSAO_NEW_DDL`, six migration/rebuild functions/predicates, `SCHEMA_VERSION`, `SCHEMA_MIGRATIONS`, core/requisicoes DDL and existing functional versioning tests remain unchanged.
- **Files read/changed:** Stage A read `main.py`, `app/db.py`, `app/db_maintenance.py`, all direct test consumers of the named tables/triggers/indexes, requisition/matrix/versioning/schema/residual/contract tests, canonical contract and five governance files. Current exact changed manifest is nine authorized paths: two production files; `tests/test_activity_versioning_leaf_schema_ownership.py`; `tests/test_phase3_schema_startup_transaction_contract.py`; canonical contract; Documentation Index; Architecture Ledger; `PROJECT_STATE.md`; this handoff. No other path is authorized or required.
- **TDD/tests:** final RED `16 failed, 3 passed`; focused GREEN `19 passed in 2.05s`; integrated ownership/Phase-B/idempotence/matrix/maintenance/residual/contract `126 passed in 15.24s`; Git-aware isolation `15 passed in 8.69s`; expanded 25-file versioning/matrix/requisition lane `309 passed in 113.34s`; route/RBAC `3 passed in 0.53s`, routes 131, unmapped 0; full hermetic `849 passed, 17 deselected in 287.05s`. Post-commit counts remain pending.
- **Routing/review:** Stage A route `flash_free`, actual `opencode` / `opencode/deepseek-v4-flash-free`, session `ses_05af8e352ffetO5DM4E9LJ4n3d`, cost 0, exit 0, no fallback, mutation none. IAsup rejected its out-of-scope recommendation to move `_needs_*` predicates. Independent review used the second FREE call, same provider/model, session `ses_05adebcf2ffe4xZqeQFq1LX6Fy`, cost 0, exit 0, no fallback/mutation; verdict APPROVE, accepted by IAsup.
- **Database/residual:** canonical SQLite open count 0. Post-full filesystem proof: `database.db` 544768 bytes, SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`, WAL/SHM absent. Accepted ignored residual `DOneDriveProgramaçãoSGAA_clean_baselined76g1_full_run.log` remains 17420 bytes, SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`, ignored/untracked; creation `2026-06-13 23:08:22.102134100 -0300` and last-write `2026-06-13 23:14:37.000000000 -0300` are preserved. Read/hash probes affected only access/NTFS change metadata.
- **Next action:** restage this evidence-only closeout, create the one bounded commit, push only `refactor/architecture-safety-net`, and run post-publication verification. **Do not begin PHASE 3-B9.**

### Current operational handoff — PHASE 3-B7 backup-settings separation (2026-07-27)

- **Historical status/baseline:** CLOSED / ACCEPTED by the explicit B8 order; the earlier B8 prohibition is superseded. Workspace `D:\OneDrive\Programação\SGAA_clean_baseline`, branch `refactor/architecture-safety-net`, baseline HEAD/upstream/live remote `b328789f16cc4db0173e58d2d2454902565d0610`, subject `Separate access schema from normalization`, parent `b29688839ea2105782fbf889550bbbd047e2a3f7`, divergence 0/0 and clean before mutation. Published commit `d1947b0c11506045b8d52bd235bc7381a2ca22c9`, subject `Separate backup schema from runtime configuration`.
- **Callers/runtime target:** exact direct one-argument orchestrator callers are `main.save_backup_settings`, `main.save_retention_policy`, `main._save_drive_config`, `main.init_db`, and `app.db._init_db_impl`. `tools/seed_demo_data.seed` enters a distinct `create_app()` context, but the baseline helper reads/mutates module-level `main.app`; therefore `current_app` is explicitly not equivalent. `main` binds its exact configured app into the runtime owner. The module fails clearly if unbound and never silently uses the active context app.
- **Classification/owners:** exact table CREATE is solely `app.db_maintenance.ensure_backup_settings_structural_schema`. `app.backup_settings` solely defines the explicit binding plus `_backup_settings_defaults`, seed, exact legacy normalization, read/merge, runtime apply, getter and orchestrator. Order is structural → six bound-app defaults → six `INSERT OR IGNORE` rows → only stripped literal `"300"` normalization → defaults-base persisted override/unknown-key merge → six effects on that same bound app. `main` re-exports the four historical names by identity and binds its app after configuring all six keys; `app.db` directly imports the orchestrator. Classification: `EXPLICIT_LEGACY_RUNTIME_APP_BINDING / BASELINE_COMPATIBILITY_SHIM / NOT_TARGET_ARCHITECTURE`.
- **Transactions/timing:** no extracted function commits, rolls back or owns a savepoint. Clean and outer-`BEGIN` calls, repeated execution, caller rollback, and structural/seed/normalization/read/runtime-apply failures are covered. Partial database work and exception propagation remain baseline-equivalent. Runtime config is applied before caller commit; later DB rollback does not undo it and apply failure may leave partial runtime config.
- **Exact lazy delta/import graph:** only `ensure_backup_settings_schema` and its same-name `helpers[...]` retrieval were removed. Remaining exact order: `ensure_atividades_schema_current`, `ensure_atividade_versioning_schema`, `get_preferred_matriz_for_curso`, `logger`. Bootstrap call count/position remain after `ensure_usuario_profile_schema` and before `alunos`. Binding direction is `main → app.backup_settings → app.db_maintenance`; the runtime module imports neither `main` nor `app.db`; deterministic subprocess import proves no cycle.
- **Cloud/downstream isolation:** `save_backup_settings`, retention, drive config, `_get_runtime_backup_settings`, `ensure_cloud_backup_schema`, `cloud_accounts`, `backup_logs`, `cloud_drive_settings`, OAuth/token/provider behavior, directory creation, snapshot and restore remain in `main.py` unchanged. Extracted modules contain no directory/network/cloud/restore effect.
- **Files read:** `main.py`, `app/db.py`, `app/db_maintenance.py`, `app/__init__.py`, backup/cloud service consumers found by tracked search, all tests referencing target symbols/config/table, canonical contract/executable manifest, Project State, handoff, index and ledger. Stage A also listed the workspace root outside its bounded read contract; IAsup rejected that evidence expansion and independently reconciled the unchanged repository.
- **Authorized changed manifest:** `app/backup_settings.py`, `app/db.py`, `app/db_maintenance.py`, `main.py`, `tests/test_backup_settings_ownership.py`, `tests/test_db_schema_maintenance.py`, `tests/test_residual_shared_helpers.py`, `tests/test_phase3_schema_startup_transaction_contract.py`, canonical contract, Documentation Index, Architecture Refactor Ledger, `PROJECT_STATE.md`, and `AGENT_HANDOFF.md`—exactly 13 paths.
- **TDD/tests:** initial RED: missing `app.backup_settings`, pytest exit 2; focused pre-waiver ownership then passed 37. The seed-app divergence triggered hard stop before review/staging/commit/push. After the explicit waiver, binding-specific RED was the missing owner (1 failed) and GREEN was 39 passed in 3.35s. Final pre-documentation lanes: integrated ownership/schema/contract 124 in 13.85s; backup/cloud/restore isolation 54 in 21.17s; release admin/route/RBAC 6 in 6.35s; runtime isolation 15 in 13.91s. The first full run exposed test-owned binding leakage only: 828 passed, 17 deselected, 2 CSRF inventory failures in 308.97s. An autouse fixture now restores the prior binding after each ownership test; the exact contaminating order passed 41 in 14.42s. The final hermetic suite passed 830 with 17 canonical D73H deselected, zero failures/errors, in 375.82s; no material warning was reported. The post-governance aggregate passed 142 in 21.43s; direct canonical builders proved routes 131 and RBAC unmapped 0.
- **Routing/recovery/review:** Stage A route `flash_free`, effective `opencode` / `opencode/deepseek-v4-flash-free`, session `ses_05b5aa2a5ffeasQzu1zrYMXvoY`, cost 0, exit 0, no fallback, mutation none. IAsup later found the Stage-A-missed seed-app divergence, froze mutation, reconciled all residues and requested the specific waiver. Independent final review used the second FREE call, same effective provider/model, session `ses_05b37cc64ffeUakdqRN7BkJ9pq`, cost 0, exit 0, no fallback and no mutation; verdict APPROVE. IAsup accepted the material verdict but retained binding, dual-init and non-atomic runtime/DB timing as explicit debt.
- **Database/residual:** canonical SQLite opens 0; size 544768; SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM absent. Accepted ignored log remains 17420 bytes with SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`, creation `2026-06-13 23:08:22.102134100 -0300`, last-write `2026-06-13 23:14:37.000000000 -0300`, ignored/untracked and absent from the index. Mandatory read/hash probes advanced only last-access/NTFS change metadata, not content, creation or last-write. Classification remains `PREEXISTING_IGNORED_WORKSPACE_RESIDUAL / SUPERVISOR_ACCEPTED / CONTENT_PRESERVED / OUTSIDE_COMMIT_MANIFEST`.
- **Historical risks/next action:** preserved explicit binding shim, non-atomic runtime-vs-DB timing, dual init and cloud residual ownership remain technical debt. B7 publication/review is complete; the current action is only the B8 block above.

### Historical accepted operational handoff — PHASE 3-B6 access responsibility separation (2026-07-27)

- **Status:** CLOSED / ACCEPTED by the explicit B7 order. B5/B5-R1 are CLOSED / ACCEPTED.
...
- **Baseline/final identity:** workspace `D:\OneDrive\Programação\SGAA_clean_baseline`, branch
  ...
  `Extract requisition alert receipts schema ownership`. Final bounded commit uses authorized
  subject `Extract matrix schema helper ownership`; identity is resolved through Git history
  and must be copied from post-commit/post-push proof, never guessed inside its own commit.
- **Owner/compatibility:** `app/db_maintenance.py` solely defines
  `ensure_matrizes_atividades_table` and `ensure_matriz_atividade_links_table`; immutable-base
  AST equivalence passes for both. `main.py` imports both compatibility exports and preserves
  object identity. `app/db.py` is the only other `app/` consumer and imports both directly.
- **Schema/transactions:** exact matrix and link-table DDL, defaults, foreign keys, named
  indexes, composite unique key, nested call, conditional ALTERs, created-at backfill,
  AAC/AEU normalization and valid-value preservation pass. Duplicate link rejection, both
  deletion cascades, idempotence and caller transaction rollback/no-commit behavior pass using
  only `:memory:` SQLite.
- **Edges/order:** `app.db._get_main_db_helpers` removed exactly the two matrix keys and
  `_init_db_impl` removed exactly the two corresponding retrievals. All other lazy keys and
  order are unchanged. The existing bootstrap calls remain once each and in exact baseline
  order before activity versioning.
- **Files read directly:** `app/db.py`, `app/db_maintenance.py`, `main.py`,
  `tests/test_db_schema_maintenance.py`, `tests/conftest.py`,
  `tests/test_ref_0c_d_r1_route_complete_actor_matrix.py`, `PROJECT_STATE.md`,
  `AGENT_HANDOFF.md`. IAexec read exactly the first four. Focused pytest additionally loaded
  the exact 32 matrix/activity/requisition/versioning paths recorded in the validation command;
  route/RBAC loaded the four exact nodes recorded below; the hermetic lane collected the full
  repository suite with D73H excluded by the canonical collection hook.
- **Files changed:** `app/db.py`, `app/db_maintenance.py`, `main.py`,
  `tests/test_db_schema_maintenance.py`, `PROJECT_STATE.md`, `AGENT_HANDOFF.md`; no other path.
- **Tests:** `.venv/Scripts/python.exe -B -m pytest tests/test_db_schema_maintenance.py -q`
  returned 39 passed in 3.66s. The exact 32-path affected matrix/activity/requisition/versioning
  command returned 334 passed in 143.48s. Route/RBAC exact-node command returned 4 passed in
  0.77s and proves live inventory 131 and unmapped 0. Runtime isolation returned 15 passed in
  9.43s. Full `.venv/Scripts/python.exe -B -m pytest -q` returned 773 passed, 17 deselected in
  305.58s. Focused file hashes and dirty manifest remained stable across lanes.
- **Database safety:** canonical SQLite opens 0; `tests/conftest.py` sets `APP_DATABASE` to a
  session-owned external runtime root before module import and the new schema probes use only
  `:memory:`. Canonical size remains 544768 and SHA-256 remains
  `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM absent after
  every lane.
- **Mode/routing:** implementation IAexec via R4 `flash_free`, effective provider/model
  `opencode` / `opencode/deepseek-v4-flash-free`, session
  `ses_05efda84cffeXU17BeYq5MJU6N`, cost 0, exit 0, no fallback. A prior transport-only dispatch
  ended `BLOCKED_PROMPT_SOURCE_INVALID` before model/session creation, exit 64, cost 0. IAsup
  `openai-codex/gpt-5.6-sol` adjudicated all diffs and gates and made one test-only deterministic
  correction for current-map naming plus explicit sole-owner AST counts; code behavior was not
  re-mutated. A separate review model was unnecessary because immutable AST, normalized module
  comparison, exact lazy delta, SQLite contracts and the full suite are deterministic.
- **Recovered verifier defect:** the first auxiliary normalized-AST script compared the two
  authorized direct imports and exited 1 after all physical work was stable. It was corrected
  read-only and then proved whole-module normalized delta zero, exact two-key lazy removal and
  baseline-identical bootstrap order. No product mutation was repeated.
- **Risks/next action:** no known technical hard stop or unauthorized residue. After selective
  commit and exact feature-branch push, external supervisor review of PHASE 3-B4 is the only
  next action. PHASE 3-B5, canonical DB opening, migrations, cleanup/custody and production
  remain prohibited without a new explicit order.

### Historical accepted operational handoff — PHASE 3-B3 requisition alert-receipts schema-helper ownership extraction (2026-07-26)
...
- **Ownership change:** `app/db.py` now solely defines `DATABASE`, `get_db_connection` and
  `close_db_connection`; `main.py` imports all three compatibility exports. Duplicate main
  implementations and its duplicate teardown registration were removed. The connection
  owner preserves environment/default path resolution, `sqlite3.Row`, foreign keys, WAL,
  synchronous NORMAL, `PTBR_NOACCENT`, Flask `g` reuse and close lifecycle.
- **Compatibility/runtime boundary:** main callable exports are object-identical to
  `app.db`; all 21 dual-assignment test fixtures remain compatible. The old
  `app.db`→`sys.modules["main"]`→`main.DATABASE` edge is zero. `init_db` and `_init_db_impl`
  remain unchanged; the retained pre-call is a pure local no-op and does not read `main`.
  Existing lazy `main` access for out-of-scope `init_db`/`ensure_*` helpers is unchanged.
- **Tests:** focused connection/configuration/collation/monkeypatch/import/route/RBAC lane
  38 passed; Git-aware runtime-isolation 15 passed; full hermetic suite 734 passed,
  17 D73H deselected, zero failures/errors in 446.67s.
- **Invariants:** routes 131→131; RBAC unmapped 0→0; behavior/schema/database delta 0;
  canonical SQLite opens 0; `database.db` remained 544768 bytes with unchanged mtime and
  SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`;
  no sidecars.
- **Mode/model:** direct IAsup `openai-codex/gpt-5.6-sol`, provider `openai-codex`, profile
  `ia-sup-sgaa`, high. Optional read-only mapping used
  `opencode/deepseek-v4-flash-free`, session `ses_05fca25ebffeyu5VanMC7CJ7MV`, cost 0,
  exit 0, no fallback, but exceeded its read allowlist; its compliance claim was rejected
  and accepted findings were independently re-proven. Direct mutation minimized context
  export and revalidation risk.
- **Manifest:** `app/db.py`, `main.py`, `tests/test_db_connection_ownership.py`,
  `PROJECT_STATE.md`, `AGENT_HANDOFF.md`. Subject:
  `Consolidate canonical database connection ownership`; identity via Git history.
- **Recovered defects:** the global Python lacked `flask_compress`; its collection failure
...
baseline above and remains only as phase-time evidence.
...
- **Mode/model:** direct IAsup `openai-codex/gpt-5.6-sol`, profile `ia-sup-sgaa`, medium;
  no delegation or fallback. Incremental source/AST inspection was the lowest-risk path for
  this deterministic three-function unit.
- **Selected unit:** `app/text.py` owns `normalize_header`, `ptbr_text_sort_key` and
  `ptbr_sqlite_collation`; all three bodies are AST-identical to starting HEAD. Date,
  presentation, message and unrelated normalization helpers remain untouched.
- **Consumers/imports:** ten `main.py` functions retain the three shared imports and
  compatibility exports; `tests/test_admin_alunos_sorting.py` now imports the collation
  directly. No matching lazy `main` edge existed, so none was removed. `app.text` imports
  without loading `main`. The old `main.py` `unidecode` import was removed only after all
  four loads were proven exclusive to the moved functions.
- **Behavior:** transliteration, lower/casefold selection, whitespace collapse, None/empty/
  non-string coercion, empty-last sort key, collation comparison results and SQLite
  registration/query behavior remain unchanged.
- **Validation:** RED expected `ModuleNotFoundError: app.text`; helper/sorting GREEN 24
  passed; expanded text/import/turma/route/RBAC lane 37 passed; Git-aware runtime-isolation
  gate 15 passed; full hermetic suite 700 passed, 17 D73H deselected, zero failures/errors
  in 347.43s.
- **Invariants:** routes 131 -> 131; unmapped RBAC 0 -> 0; behavior delta 0; schema/database
  mutation 0; canonical database SQLite open count 0. `database.db` remained at SHA-256
  `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; the only
  collation test database was `:memory:`. Candidate and Git state remained stable.
- **Exact manifest:** `app/text.py`, `main.py`, `tests/test_admin_alunos_sorting.py`,
...
- **U3 remains CLOSED / ACCEPTED.**
- **U4 remains CLOSED / ACCEPTED.**
- **U5 is CLOSED / ACCEPTED.**
- **PHASE-1-U6: CLOSED / ACCEPTED.**
  U6 was read-only, no implementation, no tests, no commit, no physical mutation.
...
- **HISTORICAL-DATABASE-SNAPSHOT-CUSTODY-R2: CLOSED / ACCEPTED.**
  ...
  Provisioning status: SELECTED / PARENT PATH NOT YET PROVISIONED — neither
  `D:\programas` nor the custodial directory exists; verified read-only; zero conflicts.
  ...
  no container created, no runtime verified, no volume mounted, no database opened.
...
- **HISTORICAL-DATABASE-SNAPSHOT-CUSTODY-R3: CLOSED / ACCEPTED.**
  ...
  withheld in the same decision that approved the contract, and released later by a separate
  explicit authorization scoped to R4 only. Move, delete, compress, SQLite open, restoration
...
- **HISTORICAL-DATABASE-SNAPSHOT-CUSTODY-R4: EXECUTED / PHYSICAL PROVISIONING COMPLETE /
  ...
  Explicit human instruction issued in the Claude Code session immediately before execution
  and before the point of no return; enumerated ten authorized actions, named the executor
  and SID, required a stop at the first error or divergence, restated the prohibitions and
  declared itself non-permanent. Medium: session record, not a repository file; this closeout
  is its durable repository record.
  Destination `D:\programas\SGAA_Historical_Custody`: PROVISIONED.
  ...
  Per-file destination SHA-256 = source = canon for all 17; zero unexpected files at the
  destination; manifest and evidence outside `artifacts\`; source remains 17/17, ignored,
  untracked and physically unchanged.
  Gates 1, P1, P2, 2, 3 and 5: all PASS.
  Final ACL: custody root protected — SYSTEM FullControl, Administrators FullControl,
  executor Modify; `artifacts\` protected — executor ReadAndExecute; `manifests\` and
  ...
  SQLite: NOT OPENED. Restoration Level 2 and Level 3: NOT EXECUTED. Source removal: NOT
  AUTHORIZED. Phase 2–6: UNAUTHORIZED.
- **R4 operational nonconformities — DECLARED / CONTAINED / NO ARTIFACT INTEGRITY IMPACT /
  NOT AN AUTHORIZED PRECEDENT:**
  (1) `New-Item -LiteralPath` incompatible with PowerShell 5.1; failure before any creation;
  residue absence verified; replaced by `[System.IO.Directory]::CreateDirectory`.
  (2) `Set-Acl` failed with `PrivilegeNotHeldException` (`SeSecurityPrivilege`) on the
  post-verification downgrade because it attempted to write the SACL; no artifact altered;
  corrected by `DirectoryInfo.SetAccessControl` writing the DACL exclusively.
  (3) First evidence-report write failed on shell quoting; no partial file created; directory
  verified empty; second explicit write completed.
  R4 must not be described as a flawless execution.
- **R5 — read-only parent ACL assessment and hardening decision: CLOSED / ACCEPTED.**
  R5 was a strict read-only Windows ACL assessment followed by this documentary
  human-decision closeout. No ACL or physical mutation occurred in R5 or this closeout.
  ...
  - `D:\programas` currently has only immediate child `SGAA_Historical_Custody` — physically
    dedicated to custody track. Human-declared exclusively dedicated.
  - `D:\programas` and `D:\Programação` are distinct literal directories; no inspected path
    was a reparse point.
  - Inherited `Authenticated Users` parent-object mask `0x001301BF`: lacks
    `FILE_DELETE_CHILD` (`0x00000040`), `WRITE_DAC`, `WRITE_OWNER`.
  - Includes `DELETE` (`0x00010000`), `FILE_ADD_FILE` (`0x00000002`),
    `FILE_ADD_SUBDIRECTORY` (`0x00000004`) on `D:\programas`.
  - Common `Authenticated Users` + `BUILTIN\Users` cannot directly delete/rename custody
    root (no `DELETE` on protected root, no `FILE_DELETE_CHILD` on parent).
  - Can create sibling files; `DELETE` on parent plus `ADD_SUBDIRECTORY` on `D:` permits
    parent-namespace rename/recreation risk (no destructive test performed).
  - Current non-elevated executor token: `Administrators` SID deny-only; executor/owner
    can alter DACL; elevated administrators retain authority.
  - `FILE_DELETE_CHILD_NOT_GRANTED_CONFIRMED` confirmed independently from SDDL/raw-mask
    decoding plus `icacls` and `Get-Acl`/.NET representations.
  - Human-approved strict hardening policy: disable DACL inheritance; SYSTEM FullControl
    OI/CI; Administrators FullControl OI/CI; `KR-IDEAPAD\klebe` ReadAndExecute OI/CI;
    remove Authenticated Users; remove BUILTIN\Users; keep current owner.
  ...
  - Accepted residuals: owner can alter DACL; elevated administrators retain authority;
    ACL is not immutability; source and destination remain on physical D:; no independent
    redundancy created.
  - Preserved R5 phase-time physical state: `D:\programas` SDDL remained inherited
    R4-era SDDL and the target was NOT APPLIED; custody-root ACL unchanged from R4;
    source and destination 17/17 intact,
    4,808,704 bytes each, all hashes matched; manifest 16,872 bytes SHA-256
  ...
  - No SQLite, restoration, recopy, source removal, ACL/owner change, file/directory
    creation/deletion/rename, tests, app, or Phase 2-6 work.
  See `docs/refactor/HISTORICAL_DATABASE_SNAPSHOT_CUSTODY.md`.
- **HISTORICAL-DATABASE-SNAPSHOT-CUSTODY-R6: CLOSED / ACCEPTED WITH DECLARED
  ...
  - Post-application error: `PropertyNotFoundStrict — property 'Value' not found` in the
    POST-MUTATION VERIFICATION / SERIALIZATION PATH.
  - Retry: NOT PERFORMED / PROHIBITED. Rollback: NOT PERFORMED / PROHIBITED.
  - `D:\programas`: protected DACL, owner/group preserved, exactly three approved Allow
    ACEs, zero inherited/Deny ACEs, no Authenticated Users, BUILTIN\Users or Everyone.
  - Descendant ACLs: zero drift. Source and destination: 17/17, 4,808,704 bytes each,
    SHA-256 source = destination = manifest for all 17.
  ...
  - This documentary closeout performs zero new physical mutation and does not edit the
    preserved R6 script/report or R4 evidence.
  See `docs/refactor/HISTORICAL_DATABASE_SNAPSHOT_CUSTODY.md`.
...
- **LEVEL 2 PHYSICAL RESTORATION (execution round R3): COMPLETE / LOCALLY VERIFIED /
  ...
  SHA-256 `92627ded44c9094e74f01da5718c995cd3fdd5ac467ef79298541a75b777cd8c`; the R7
  primary candidate. Fallback not used (fallback uses 0). `sealed\` and `working\`
  copies both remain 544768 bytes at that SHA-256.
  ...
  recovery was documentary only, with 0 additional openings.
  Evidence 7/7 complete: `authorization.txt`, `tool-hashes.txt`, `preflight.json`,
  `copy-hashes.json`, `sqlite-result.json` (43682 bytes, SHA-256 `71bb40e4…5866`),
  `postflight.json` (2318 bytes, SHA-256 `e498af58…6176`) and `level2-report.md`
  (SHA-256 `efaa34bd…3622`).
  ...
  missing names 0, candidate sidecars 0. R8 register: 11 entries, digest `75c00810…1023`.
  Package and qualification hashes unchanged and matched the R3 execution contract.
  Source artifacts remain intact, ignored and untracked; no source removal occurred.
  Carried-forward boundaries for the next executor: the restore root and its `sealed\`,
...
- **Preserved historical/superseded — pre-R2 wording:** statements that R2 was "NOT STARTED", that the specific canonical destination was "UNRESOLVED" or "NOT YET SELECTED", and the R30 state `DESTINATION_OPTIONS_READY_AWAITING_HUMAN_SELECTION` are superseded by this closeout. Such claims in historical blocks below are preserved only as historical record.
- **Preserved historical/superseded — pre-R7 wording:** statements that R7 was "NOT STARTED", "not authorized", "requires a separate explicit order", or that R7's contract was undrafted or pending are superseded by this closeout. Such claims in historical blocks below are preserved only as historical record.
- **Preserved historical/superseded — pre-Level-2 wording:** statements that physical Level 2 was "NOT AUTHORIZED", "NOT EXECUTED", pending or blocked, and that the timestamped root `D:\tmp\sgaa_restore_<UTC>` was unresolved, are superseded by the Level 2 block above. Such claims in historical blocks below are preserved only as historical record.
...
A note on the Level 2 environment: NATIVE WINDOWS is selected for R7. The R3-era
...
- **R9A** was externally accepted as part of the overall Phase-0 progression.
- **Status:** CLOSED / ACCEPTED.
- Smoke flows (R9) were not implemented in R9A; they are the subject of the block above.
- All R9A technical and documentary facts remain true as historical record (see historical block below).
