...
## Current state — PHASE 4-B4.1 IMPLEMENTED / AWAITING SUPERVISOR REVIEW

### Operational handoff — PHASE 4-B4.1 neutral shared-owner extraction (2026-08-01)

- **Authority/status:** PHASE 4-B4-A: CLOSED / ACCEPTED. PHASE 4-B4.1: IMPLEMENTED / AWAITING SUPERVISOR REVIEW. B1, B2 and B3 remain CLOSED / ACCEPTED. PHASE 4 remains OPEN / INCREMENTAL IMPLEMENTATION. PHASE 4-B4.2: NOT AUTHORIZED. Phase 5: NOT AUTHORIZED. Phase 6: NOT AUTHORIZED. Migration v4: PROHIBITED.
- **Baseline/publication:** parent `185426daccc9f0eb0dba4497248100c1a88d15fa` (`Record acceptance of Phase 4-B3`), branch `refactor/architecture-safety-net`, baseline/live/upstream equal with divergence `0/0`; one child with subject `Extract requisition shared owners` is the authorized publication unit.
- **Expansion adjudication:** `app/settings.py` is accepted as the neutral owner of all eight B1 settings helpers; `app/matrix_scope.py` is accepted as the dedicated neutral owner of exactly six matrix-scope symbols; `app/views/aluno.py` contains only the direct-import reverse-edge reduction; the Phase-3 test delta is governance-reader-only. Classification: `B4_1_SCOPE_EXPANSION / COHERENT_NEUTRALIZATION / SUPERVISOR_RECONCILED`.
- **Exact owner cut:** `app.settings` solely defines `_normalize_optional_iso_date`, `get_app_settings`, `get_response_time_settings`, `save_app_settings`, `reset_response_time_metrics`, `save_return_response_settings`, `get_horas_settings`, and `save_horas_settings`; `app.requisitions` solely defines `auto_indefer_devolvidas`; `app.matrix_scope` solely defines `MATRIZ_STATUS_META`, `_matriz_status_label`, `_matriz_option_label`, `get_effective_matriz_for_turma`, `is_activity_allowed_for_turma_matrix`, and `get_allowed_activity_ids_for_turma_matrix`. `main` and Configurações preserve callable identity.
- **No-route boundary:** NO ROUTE MOVEMENT. EXACT 9 REQUISICOES ROUTES REMAIN IN MAIN.PY; their ASTs and global endpoint bindings are baseline-identical. `app/views/admin/requisicoes.py` does not exist. B4.2 is not started.
- **Compatibility:** settings, matrix, requisition helper and nine route AST equivalence PASS; message catalog remains 536 keys with six settings usage owners moved to `app/settings.py`; conditional helper-owned auto-indefer commit remains exact; no hardening, schema, migration, RBAC, CSRF, snapshot, template or filesystem behavior change.
- **Exact manifest:** 20 paths = 7 production + 7 tests + 6 governance, enumerated in `docs/refactor/PHASE4_REQUISICOES_SHARED_OWNER_CONTRACT.md`. The twentieth path is the mechanically required stale Macro Phase 3 governance reader; no production scope expansion occurred.
- **Evidence:** baseline focused 125 passed / canonical SQLite opens 0; technical RED 7 failed + 1 passed as expected; governance RED 1 failed as expected; owner GREEN 8 passed; affected technical lane 247 passed / canonical SQLite opens 0. First full: 981 passed / 3 failed / 17 deselected; two stale governance-reader failures were repaired and one randomized pre-existing turma-code collision was isolated; recovered nodes 3 passed. Final full: 984 passed / 17 deselected / 419.95s / canonical SQLite opens 0.
- **Technical review:** route `flash_free`; requested/effective `opencode/deepseek-v4-flash-free`; session `ses_040d538bfffegBsAJQbLJrnqSV`; exit 0; cost 0; fallback none; mutation 0; PASS / findings NONE. Reviewed tracked-diff SHA-256 `f4b6cb00b4365cc7c20af5fcba1ac736ece1bab0ab9c6e0f89b19084799727f9`; complete pre-addendum candidate SHA-256 `ddc73cb94786899595f3cce9d577bdbcca961a082c83866498a36ebd4687a23f`; non-governance SHA-256 `74649089be0699dff4440260bdb11793b4e5793550f17456893f4a281bd6096b`. Reviewer “8 tests” wording rejected as stale/non-material; exact count is seven.
- **Historical reconciliation:** B1-time ownership of all eight settings helpers was Configurações; B4.1-time ownership is `app.settings`, with Configurações/`main` identity exports and accepted B1 route behavior preserved. B2 established six Aluno lazy-main edges at B2-time; B4.1 later removes only `get_effective_matriz_for_turma`, leaving the exact five keys `ensure_admin_arquivos_table`, `get_admin_arquivo`, `get_student_request_update_alert`, `list_active_admin_alertas`, and `mark_student_request_updates_seen`.
...
- **Next exact action:** documentary Flash FREE addendum, exact selective publication and post-publication gates in this execution; then external supervisor review. Do not begin B4.2.
