...
# ``APP_ACCEPTANCE_PUBLIC_BASE_URL`` and its override lived here.  It existed
# for exactly one reason: the acceptance runtime listened on a different PORT
# than the canonical one, so the base URL held by the machine-local secret store
# pointed at the wrong port and the links in issued e-mails were unusable.
#
# SGAA now runs on a single port (5000) and isolates acceptance by DATABASE, so
# the stored base URL is already correct for both runtimes and the override had
# nothing left to reconcile.  Removed rather than left dormant: an environment
# variable that can silently replace the trusted link authority is not something
# to keep around for a scheme that no longer exists.
