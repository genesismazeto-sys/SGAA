...
- **Status/baseline:** IMPLEMENTED / LOCALLY VERIFIED / AWAITING SUPERVISOR REVIEW / PUBLICATION PENDING AT COMMIT-TREE TIME. B9 is CLOSED / ACCEPTED by the explicit B10 order. Baseline workspace `D:\OneDrive\Programação\SGAA_clean_baseline`, branch `refactor/architecture-safety-net`, baseline HEAD/upstream/live remote `37c8757e4ef41647c971ad8a974c853dec6ce4e7`, subject `Version atividades schema migration`, divergence 0/0 and clean before mutation.
- **Core migration v3:** `SCHEMA_VERSION == 3`; v3 registry entry `normalize_activity_versioning_core` owned by `app.db_maintenance`. Migration v3 normalizes every supported legacy `atividade_versao` variant (`missing_numero`, `old_unique_base_norma`, `default_zero`, `partial_index`, canonical), preserves parent and child FK data, assigns deterministic `numero_versao` via `ROW_NUMBER() OVER (PARTITION BY atividade_base_id ORDER BY id)` for the `missing_numero` variant, and handles `sqlite_sequence`. The early runner `apply_early_schema_migrations` is the sole destructive migration boundary: it rejects existing transactions, records/disables/verifies FK state, runs the bounded `BEGIN IMMEDIATE` migration, rolls back every failure atomically, and restores original FK state.
- **Self-transactional rebuild retired:** All seven legacy symbols removed from `main.py`: `_VERSAO_NEW_DDL`, `_needs_atividade_versao_migration`, `_needs_atividade_versao_default_fix`, `_needs_index_hardening`, `_recreate_atividade_versao`, `_migrate_atividade_versao_to_numero_versao`, `_fix_atividade_versao_default`. The public orchestrator `ensure_atividade_versioning_schema` is now solely defined in `app.db_maintenance`; it calls ordered non-destructive core/leaf/triggers/indexes/requisicoes-compatibility helpers under caller transaction ownership. `main.py` imports it by identity.
- **Lazy bridge delta:** `app.db._get_main_db_helpers` now contains exactly two entries: `get_preferred_matriz_for_curso` and `logger`. `ensure_atividade_versioning_schema` is a direct import from `app.db_maintenance` in both `app.db` and `main.py`.
- **B8/B9/B7 isolation:** B8 four leaf tables, two leaf triggers, eight leaf indexes, and their three pure helpers remain unchanged in `app.db_maintenance`. B9 atividades v2 migration and eleven-column contract unchanged. B7 backup-settings ownership unchanged. No B11 work exists.
- **TDD/validation:** Focused v3 + v2 tests: 49 passed in 2.25s. Six-test contract gate: 163 passed in 10.36s. Comprehensive regression: 352 passed, 17 deselected in 82.32s. Routes 131; RBAC unmapped 0. Canonical database opens 0.
- **Authorized manifest:** 14 paths: three production (`app/db_maintenance.py`, `app/db.py`, `main.py`), one new v3 test, one modified v2 test, four historical guard tests, canonical contract, Documentation Index, Architecture Ledger, this state, and `AGENT_HANDOFF.md`. `tests/test_activity_versioning_core_migration_v3.py` is tracked only after staging.
- **Canonical safety:** `database.db` remains 544768 bytes with SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`, WAL/SHM absent. Protected residual remains 17420 bytes with SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`.
- **Next action:** selective staging, bounded commit, feature-branch publication and post-publication verification. PHASE 3-B11 is not authorized.

### Historical PHASE 3-B10 — predecessor state (pre-B10 baseline)
