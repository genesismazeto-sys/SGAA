## Current authoritative state — PHASE 4-B3 IMPLEMENTED / AWAITING SUPERVISOR REVIEW (2026-08-01)

### PHASE 4-B3-R2 — final qualification, technical review and publication preparation

- **Status/authority:** PHASE 4-B3 is IMPLEMENTED / AWAITING SUPERVISOR REVIEW. It is not CLOSED / ACCEPTED. Starting HEAD, upstream and live feature-branch remote were `81cc6b10b893f1d34bd211a527e9fd12c3b6bbbe` (`Record acceptance of Phase 4-B2`), divergence `0/0`, empty index and no Git operation. PHASE 4-B1 and PHASE 4-B2 remain CLOSED / ACCEPTED; B2 commit `17e468ad938e873e1f9e9c303808ad31b9f3806b` remains published with post-publication verification COMPLETE. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION.
- **Owners and exact route scope:** `app.views.admin.atividades` owns exactly 22 legacy global endpoints and 29 route/method combinations for Atividades CRUD, CSV import, groups, versioned catalog, normas and legacy mapping. `app.activity_catalog` is the view-neutral shared owner of 14 catalog/parsing helpers. `main` re-exports moved objects by identity. No twenty-third endpoint, namespaced alias or duplicate rule exists.
- **Preserved boundaries:** URLs, endpoints, methods, `request.endpoint`, `url_for`, RBAC requirements, CSRF behavior, templates, redirects, flashes and transaction paths remain compatible. Matrizes and Requisições route bodies remain in `main.py`; their shared helper consumers resolve through `app.activity_catalog`. B2 helpers were not duplicated; no schema, migration, template, static asset or `app/auth.py` changed.
- **Scope-expansion reconciliation:** four previously unstaged/uncommitted paths are accepted only under B3-R1's exact proof. `app/uploads.py` neutrally owns `save_upload` because current non-B3 consumers remain in `main.admin_reportes` and `main.admin_meus_dados`; `main.save_upload is app.uploads.save_upload`; valid-call filename/path behavior and all three extension sets are preserved, `current_app.config["UPLOAD_FOLDER"]` replaces `main.app`, and an escaped internal subdirectory now fails before filesystem mutation. `utils/messages.py` adds only `app/uploads.py` to the existing explicit scanner roots: 536→536 keys, zero added/removed keys, zero default/kind/semantic-usage deltas; 292 source path/line metadata deltas are mechanically caused by owner moves and line relocation. Each CSRF snapshot has exactly 15 deltas, all and only `view_function: main.<handler> → app.views.admin.atividades.<handler>` with identical handler suffix; all other fields and all 78 rows are byte-semantically unchanged.
- **Process nonconformity:** the early `utils/messages.py` and CSRF snapshot changes are recorded as `PRE_REVIEW_SCOPE_EXPANSION / MECHANICALLY_CAUSED_BY_CANONICAL_OWNER_MOVE / NOT_STAGED / NOT_COMMITTED / PUBLICATION_WITHHELD / REQUIRES_SUPERVISOR_RECONCILIATION / RECONCILED_BY_B3_R1_EXACT_DELTA_PROVED`. This is not generic retroactive authorization.
- **Tests/evidence:** B3 RED was 12 failed / 2 passed; B3 owner and upload contract is 19 passed. The mandatory affected lane passed 353 in 119.56s. The 974 passed / 17 deselected / 292.86s run is `GREEN HISTORICAL PRE-FINAL-TEST-DELTA / NOT FINAL QUALIFICATION`. The exact current 16-path candidate was finally requalified by `.venv/Scripts/python.exe -B -m pytest -q --tb=short --durations=25`: 974 passed / 17 deselected / zero failures/errors / 399.95s / exit 0. No pytest process, repository artifact, candidate/protected/status/index mutation or sidecar remained. Routes 131; endpoints 130; governed pairs 134; RBAC unmapped 0; actor matrix 402 = 263 allowed + 139 denied. Route snapshot Git blob/worktree hash is byte-identical.
- **Physical safety:** canonical `database.db` remains 544768 bytes / SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal are absent. Protected ignored residual remains 17420 bytes / SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`, ignored and index-absent. Canonical SQLite opening count remains zero; pytest used isolated runtime databases.
- **Exact authorized manifest:** 16 paths = 6 production (`app/__init__.py`, `app/uploads.py`, `app/activity_catalog.py`, `app/views/admin/atividades.py`, `main.py`, `utils/messages.py`) + 4 tests/snapshots (`tests/test_phase4_atividades_blueprint.py`, `tests/test_phase4_configuracoes_blueprint.py`, both CSRF inventory snapshots) + 6 governance (`AGENT_HANDOFF.md`, `PROJECT_STATE.md`, Documentation Index, master plan, Architecture Ledger and the B3 contract). Path 17 remains a hard stop.
- **Provisional review:** logical route `flash_free`; requested/effective provider/model `opencode` / `opencode/deepseek-v4-flash-free`; session `ses_0425bf1cbffeIxMwsDQn0etSEC`; router exit 0; cost 0; `free-uses-used=0`; no fallback. It produced no technical verdict after external-review-directory permission errors, so IAsup classified it `PROVISIONAL / UNUSABLE DELIVERY / NO VERDICT`, not PASS. Physical post-review SHA-256 remained `ec96796d3541710a36ac8121e40ffd888737c7c926f191a28034482cedbfd556`; mutation count 0.
- **Final technical review:** the final FREE attempt used `opencode/deepseek-v4-flash-free`, session `ses_0422f0e1cffepCXyNpg47dWpoq`, cost 0, but timed out after 600s without an accepted delivery. After the two FREE sessions were consumed, the router recorded logical `flash_free` → selected `flash_normal`, trigger `FALLBACK_FREE_BUDGET_EXHAUSTED`; task-level technical cause was the immediately preceding FREE timeout. Accepted review used `opencode-go/deepseek-v4-flash`, session `ses_04224ca47ffe5qAwwHGtxlR7i7`, router exit 0, router-reported cost `0.0010424344 USD` (observed step-finish aggregate `0.0206550400 USD`), reviewed diff `ec96796d3541710a36ac8121e40ffd888737c7c926f191a28034482cedbfd556`, mutation count 0, verdict PASS, findings NONE.
- **IAsup adjudication and frozen technical identity:** PASS accepted. The reviewer's canonicalization concern is `REJECTED_INCORRECT` because IAsup independently reproduced the specified path-framed binary-diff hash before and after review; the reviewer's no-test limitation is `ACCEPTED_NONBLOCKING` because review was intentionally read-only and IAsup owns the hermetic gate. Pre-documentation full diff SHA-256: `ec96796d3541710a36ac8121e40ffd888737c7c926f191a28034482cedbfd556`. Frozen non-governance diff SHA-256: `13b0af13e653641d75d2466d7d8d69090e655a18e28bb678a7090dbe0e2ecab0`.
- **Mode and next exact action:** direct IAsup `openai-codex/gpt-5.6-sol` performed physical reconciliation and deterministic gates; independent Flash performed review. Next: focused B3/B1/B2/governance gates, freeze the final 16-path publication diff, obtain zero-mutation documentary addendum, then selectively stage/commit/push and verify. B4, Phase 5 and Phase 6 are NOT AUTHORIZED; migration v4 is PROHIBITED.
