## Current authoritative state — PHASE 3-B11 SINGLE-INIT CUTOVER IMPLEMENTED IN WORKTREE / FINAL GATES GREEN / MANDATORY FREE REVIEW UNUSABLE / HARD STOP BEFORE STAGING / NO COMMIT / NO PUSH — PHASE 3-B10-R1 CLOSED / ACCEPTED — PHASE 3-B10 CLOSED / ACCEPTED — PHASE 3-B9: CLOSED / ACCEPTED — PHASE 3-B8: CLOSED / ACCEPTED — PHASE 3-B7: CLOSED / ACCEPTED — PHASE 3-B6: CLOSED / ACCEPTED — PHASE 3-B5-R1: CLOSED / ACCEPTED — PHASE 3-B5: CLOSED / ACCEPTED — PHASE 3-B4: CLOSED / ACCEPTED — PHASE 3-B3: CLOSED / ACCEPTED — PHASE 3-B2: CLOSED / ACCEPTED — PHASE 3-B1: CLOSED / ACCEPTED — PHASE 3-B ASSESSMENT: CLOSED / ACCEPTED — PHASE 3-A: CLOSED / ACCEPTED — PHASE 2: CLOSED / ACCEPTED — Architecture refactor Phase 1: CLOSED / ACCEPTED — Historical snapshot custody: OPEN / SOURCE PRESERVED / LEVEL 3 NOT EXECUTED / SECURITY-COMPLETE CUSTODY NOT CLAIMED (2026-07-31)

### PHASE 3-B11 — canonical single-init cutover and transaction postcondition reconciliation (2026-07-30)

- **Status/baseline:** IMPLEMENTED IN WORKTREE / LOCALLY VERIFIED / INDEPENDENT FREE REVIEW, COMMIT AND PUSH PENDING. B10/B10-R1 are CLOSED / ACCEPTED. Starting workspace `D:\OneDrive\Programação\SGAA_clean_baseline`, branch `refactor/architecture-safety-net`, HEAD/upstream/live remote `e63e1a66b9d2ebad7253a0efd2e0a367b89b8b8a`, divergence 0/0, index empty and no Git operation active before B11 continuation.
- **Ownership:** `app.db.init_db` is the sole defining initializer. `main.py` imports and compatibility-exports the same object; local `main.init_db`, `app.db._init_db_impl`, `_get_main_db_helpers`, `_sync_database_from_main`, every lazy key/retrieval, and every `app.db → main` edge are absent. `app.db` directly owns preferred-matrix selection, app/cloud/turma settings helpers and module-local logger.
- **Runtime/callers:** canonical init explicitly binds the active Flask app into `app.backup_settings` before connection acquisition. The corrected qualified lexical inventory is exactly 72 `main.init_db(...)` expressions and 5 `app_db.init_db(...)` expressions, all resolved to canonical runtime owner `app.db.init_db`; three additional bare `init_db(...)` calls are separately resolved through `from app.db import init_db` in development startup, restore and `tools/seed_demo_data.py::seed`. Lexical caller count is distinct from the sole defining owner. Seed passed repeated/idempotent execution without importing `main`.
- **Transaction adjudication:** B11 introduced no commit, rollback, BEGIN, savepoint, RELEASE or transaction split. B10 `app.db._init_db_impl`, B10 `main.init_db`, and B11 `app.db.init_db` each have exactly one final `conn.commit()` and no body-owned transaction SQL. `apply_early_schema_migrations` remains isolated and intentionally durable; `ensure_usuario_access_schema` retains its clean-vs-nested savepoint contract; clean app-settings CREATE can be SQLite statement-autocommitted; all later bootstrap work remains caller-owned until the one final commit.
- **Failure postconditions:** focused tests now distinguish rollback inside early migration, durability after successful v1/v2, durability after the outermost access savepoint, clean-connection app-settings structural durability, caller-visible versus observer-visible state, final migration metadata visibility, failed final commit, FK restoration, close rollback, exception propagation and no continuation. Production correction was rejected because no B11-introduced boundary or excess persistence was found.
- **Current evidence:** corrected caller/registry targeted gate `52 passed in 8.46s`; final cutover/fault injection rerun `15 passed in 3.85s`; established affected schema/migration/backup/restore/seed/ownership/isolation lane `206 passed in 29.22s`. The preserved pre-correction hermetic checkpoint is `913 passed / 17 D73H deselected / 348.40s / exit 0`; the final post-correction hermetic gate passed `913 / 17 / 416.66s / exit 0`, with zero failures/errors and all 25 slowest durations captured. Routes remain 131 and RBAC unmapped remains 0. The AST registry extractor accepts the real annotated assignment and proves `SCHEMA_VERSION == 3`, exact contiguous unique v1/v2/v3 names, callable entries, no gap, no duplicate and no v4.
- **Exact current 14-path manifest:** `app/db.py`, `main.py`; tests `test_phase3_final_init_cutover.py` (new), `test_phase3_schema_startup_transaction_contract.py`, `test_residual_shared_helpers.py`, `test_backup_settings_ownership.py`, `test_db_schema_maintenance.py`, `test_db_connection_ownership.py`, `test_activity_versioning_leaf_schema_ownership.py`; canonical contract, Documentation Index, Architecture Ledger, this state and `AGENT_HANDOFF.md`. No Phase 4, migration v4, route, RBAC, UI, API or template path.
- **Physical safety:** pre-continuation reconciliation proved canonical `database.db` 544768 bytes, SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`, WAL/SHM absent; protected residual 17420 bytes, SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`, ignored/untracked/index-absent. Canonical SQLite opens remain 0.
- **Execution/routing/review:** direct IAsup `openai-codex/gpt-5.6-sol`, high, because deterministic adjudication depends on accumulated evidence. The one effective review call used the no-fallback wrapper fixed to `opencode` / `opencode/deepseek-v4-flash-free`, read-only prompt, and timed out after 600s without stdout/usable report or wrapper metadata; session ID, cost and verdict are unavailable. A preceding Windows/MSYS path error occurred before the wrapper/model frontier and was recovered without a model call. Post-failure reconciliation proved reviewer mutation zero, no remaining process, identical 14-path manifest and diff SHA-256 `27e6c3ef2e769809008a8de81709b29a37caed693f1ce18a96820d18b9b80dff`.
- **Next action:** HARD STOP. Staging, commit and push remain prohibited. Any new FREE attempt, paid fallback or other recovery requires a new explicit supervisor authorization. Phase 4 remains unauthorized.

### PHASE 3-B10 — activity-versioning core schema migration v3, self-transactional rebuild retirement, direct core ownership, and two-entry lazy bridge (2026-07-30)
