...
### PHASE 4-B7-P — Arquivos/Alertas neutral shared-owner prerequisite — CLOSED / ACCEPTED

- **Current authority/status:** PHASE 4-B4.2: CLOSED / ACCEPTED. PHASE 4-B5 CLOSED / ACCEPTED. PHASE 4-B6-P: CLOSED / ACCEPTED. PHASE 4-B6-R1: CLOSED / ACCEPTED. PHASE 4-B6: CLOSED / ACCEPTED. PHASE 4-B7-A: **CLOSED / ACCEPTED**. PHASE 4-B7-P: **CLOSED / ACCEPTED**. PHASE 4-B7 (route/blueprint extraction): **NOT AUTHORIZED**. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION; it is not closed. Phase 5: NOT AUTHORIZED. Phase 6: NOT AUTHORIZED. Migration v4: PROHIBITED. This unit is only the neutral shared-owner prerequisite; no Arquivos/Alertas/Reportes route moved and `app/views/admin/arquivos_alertas_reportes.py` remains absent.
- **Accepted technical commit:** `1c82a1954250aa5e6654349ce77a50d60f03fe8f`, subject `Extract B7 shared neutral owners`, parent `b6d6e2295e2beeba046cfe1f4c1614f667261ad2` (`Record acceptance of Phase 4-B6`); protected `main` `340fc7c91c6bc9b50e884adcb5915f9e29a0bfe1`. Technical publication: **COMPLETE**. Post-publication bounded verification: **COMPLETE**. External supervisor technical acceptance: **GRANTED**.
- **Canonical partition:** `app.db_maintenance` = 2 (`ensure_admin_arquivos_table`, `ensure_admin_alertas_table`); `app.admin_files` = 1 (`get_admin_arquivo`); `app.admin_alerts` = 1 (`list_active_admin_alertas`). Aluno lazy map reduced 5 → exactly 2 (`get_student_request_update_alert`, `mark_student_request_updates_seen`). Zero route movement.
- **Routing/reviewer deviation (preserved truthfully):** the ordered IAsup/IAexec router (`openai-codex/gpt-5.6-sol` / DeepSeek V4 Flash) was not invocable in the implementing harness; implementation was performed directly by the acting Claude Code session, disclosed rather than silently substituted. The external supervisor subsequently performed an independent repository review as part of this closeout. Deviation accepted as nonblocking.
- **Governance closeout subject:** `Record acceptance of Phase 4-B7-P`; identity resolves through Git history. This R3 closeout changes exactly six governance paths; no production, test, snapshot, or database path is changed.
- **Next exact action:** none for B7-P. **PHASE 4-B7 (route/blueprint extraction) remains NOT AUTHORIZED.** No later cohort is named or authorized by this closeout.

## Historical authoritative state — PHASE 4-B7-P IMPLEMENTED / AWAITING SUPERVISOR REVIEW (historical phase-time record, 2026-08-04)

### PHASE 4-B7-P — Arquivos/Alertas neutral shared-owner prerequisite (implementation record, superseded by R3 acceptance above)

- **Historical phase-time authority/status:** PHASE 4-B4.2: CLOSED / ACCEPTED. PHASE 4-B5 CLOSED / ACCEPTED. PHASE 4-B6-P: CLOSED / ACCEPTED. PHASE 4-B6-R1: CLOSED / ACCEPTED. PHASE 4-B6: CLOSED / ACCEPTED. PHASE 4-B7-A: DIAGNOSIS COMPLETE / SHARED_OWNER_PREREQUISITE_REQUIRED (accepted). PHASE 4-B7-P: IMPLEMENTED / AWAITING SUPERVISOR REVIEW (phase-time; superseded by CLOSED / ACCEPTED above). PHASE 4-B7 (route/blueprint extraction): NOT AUTHORIZED. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION; it is not closed. Phase 5: NOT AUTHORIZED. Phase 6: NOT AUTHORIZED. Migration v4: PROHIBITED. This unit is only the neutral shared-owner prerequisite; no Arquivos/Alertas/Reportes route moved and `app/views/admin/arquivos_alertas_reportes.py` remains absent.
- **Baseline / parent:** branch `refactor/architecture-safety-net`; required entry `HEAD` `b6d6e2295e2beeba046cfe1f4c1614f667261ad2` (`Record acceptance of Phase 4-B6`); protected `main` `340fc7c91c6bc9b50e884adcb5915f9e29a0bfe1`. Both confirmed matching before any mutation.
- **Exact 4 shared symbols moved, neutral owners 2/1/1:** `app.db_maintenance` owns `ensure_admin_arquivos_table` and `ensure_admin_alertas_table` (2, alongside the pre-existing `ensure_reportes_table`); new `app.admin_files` owns `get_admin_arquivo` (1); new `app.admin_alerts` owns `list_active_admin_alertas` (1). `main` re-exports all four by identity with zero local bodies. All four moved bodies are AST-equivalent to entry baseline `b6d6e2295e2beeba046cfe1f4c1614f667261ad2:main.py`; only name resolution of the `ensure_*` dependency inside `get_admin_arquivo`/`list_active_admin_alertas` changed, through a normal import.
- **Aluno consumer resolution:** `app/views/aluno.py` now imports `ensure_admin_arquivos_table` directly from `app.db_maintenance`, `get_admin_arquivo` directly from `app.admin_files`, and `list_active_admin_alertas` directly from `app.admin_alerts`. The residual lazy map in `aluno._get_main_helpers()` is reduced from 5 keys to exactly 2: `get_student_request_update_alert`, `mark_student_request_updates_seen` (both Requisições-domain, out of scope for B7). No new `app -> main` edge; no cycle.
- **Preserved unchanged (verified AST-identical to entry baseline):** `main.uploaded_file` (`/uploads/<path:filename>`) and `main.admin_dashboard` — neither was touched; both continue resolving the canonical imported identities. `ensure_reportes_table` ownership and `REPORTE_CATEGORY_OPTIONS` neutral ownership are unchanged; Reportes has zero B7-P delta.
- **Zero route movement:** all 12 Arquivos/Alertas/Reportes route handlers (5 Arquivos, 4 Alertas, 3 Reportes; 13 route-method pairs) remain main-local, byte/AST unchanged.
- **Pre-existing residual debt (not touched in B7-P, recorded only):** `_best_effort_remove_admin_arquivo_file` (main-local) deletes via `os.path.normpath(os.path.join(upload_root, rel_path))` + `os.path.isfile`, without the `startswith(base)` containment check used by `uploaded_file`; low-risk because `rel_path` is always server-generated by `save_upload`, never raw user input. The phantom `sgaa_canonical_db_guard` governance debt (referenced only in documentary evidence labels, no physical module) is likewise unrepaired and uninvoked in this phase, per standing instruction.
- **TDD chronology:** RED (new `tests/test_phase4_arquivos_alertas_shared_owners.py`, 11 collected): 7 failed / 4 passed, exit 1 — all seven failures attributable exclusively to the not-yet-implemented prerequisite (owner modules absent, main still owned the four bodies, aluno still had the 5-key lazy map); the four passes were pre-true invariants (zero route movement, `uploaded_file`/`admin_dashboard`/Reportes unchanged). GREEN after implementation: 11 passed, exit 0.
- **Scope-expansion correction (supervisor-authorized, PHASE 4-B7-P-R2):** the full default suite surfaced two additional frozen assertions of the identical pre-B7-P 5-key aluno lazy map, outside the originally named 13-path pool: `tests/test_db_schema_maintenance.py` (`EXPECTED_ALUNO_LAZY_KEYS_AFTER_VERSIONING_EXTRACTION`, historical residue from the B2 versioning-subsystem extraction) and `tests/test_phase4_versioning_subsystem.py` (`REMAINING_ALUNO_MAIN_HELPERS`). Neither file references arquivos/alertas beyond this one shared constant; neither was flagged by the B7-A diagnosis. Classification: `PRE_REVIEW_SCOPE_EXPANSION / EXPLICITLY_AUTHORIZED / ALUNO_LAZY_MAP_INVARIANT_RECONCILIATION / NO_RETROACTIVE_GENERIC_AUTHORITY`. Supervisor explicitly authorized adding both to the mutable pool via in-session confirmation; both received the identical one-for-one 5-key → 2-key correction, no other change.
- **Full default suite result (PHASE 4-B7-P-R2 environmental waiver):** `1103 passed / 3 failed / 17 deselected`, 329.53s, exit 1. **NOT claimed as GREEN / NOT claimed as 0 failed.** The exact three failures — `tests/test_phase3_final_init_cutover.py::test_seed_tool_uses_factory_owner_without_main_and_is_idempotent`, `tests/test_pytest_runtime_isolation.py::TestSubprocessImportMain::test_import_main_uses_runtime_root`, `tests/test_pytest_runtime_isolation.py::TestMainNoOverwrite::test_import_main_preserves_upload_folder` — were independently reproduced, identical failure fingerprints, on a disposable git-worktree checkout of the unmodified entry baseline `b6d6e2295e2beeba046cfe1f4c1614f667261ad2` under the same interpreter. Root cause is a Windows console/subprocess-temp-file UTF-8 encoding defect unrelated to arquivos/alertas; neither failing file imports or references any B7-P symbol (`ensure_admin_arquivos_table`, `ensure_admin_alertas_table`, `get_admin_arquivo`, `list_active_admin_alertas`) or `admin_arquivos`/`admin_alertas` tables. Classification: `PRE_EXISTING_BASELINE_REPRODUCED / ENVIRONMENTAL_ENCODING_FAILURE / UNRELATED_TO_B7_P / ACCEPTED_NONBLOCKING_RESIDUAL / NO_RETROACTIVE_GREEN_CLAIM`. Neither failing test file was modified by B7-P and neither is added to the mutable pool; that environmental debt remains separate, unrepaired, out of scope.
- **B7-P-specific/affected focused gates (all GREEN, 0 failed / 0 errors / exit 0):** `tests/test_phase4_arquivos_alertas_shared_owners.py` (11 passed), `tests/test_phase4_requisicoes_shared_owners.py` (9 passed), `tests/test_residual_shared_helpers.py`, `tests/test_admin_arquivos.py`, `tests/test_admin_reportes.py`, `tests/test_admin_aux_sections.py`, `tests/test_csrf_inventory_audit.py` (69 passed combined); `tests/test_route_inventory_snapshot.py`, `tests/test_rbac_requirement_coverage.py`, `tests/test_ref_0c_d_r1_route_complete_actor_matrix.py` (36 passed combined).
- **Global invariants (unchanged):** routes 131; endpoints 130; business route-method pairs 160; RBAC unmapped 0 (`tests/_artifacts/rbac_unmapped_routes_baseline.json` byte-identical, zero git diff); route inventory byte-identical (`tests/_artifacts/route_inventory_baseline.json` zero git diff); message catalog 536 (`utils.messages._message_catalog()` measured directly); no template/static/JS delta; no schema/semantic delta.
- **Physical custody:** canonical `database.db` before and after the full suite: 544768 bytes, SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9` — unchanged; WAL/SHM/journal absent both times.
- **Agent routing disclosure:** the order specified IAsup `openai-codex/gpt-5.6-sol` and IAexec DeepSeek V4 Flash (FREE-first, fallback to normal). This session's harness has no router tool to invoke that provider; all diagnosis and implementation were performed directly by the acting Claude Code session. This deviation is disclosed rather than silently substituted; no escalation to a Pro/Luna/GPT tier occurred.
- **Canonical contract / next exact action:** `docs/refactor/PHASE4_ARQUIVOS_ALERTAS_SHARED_OWNER_CONTRACT.md`. PHASE 4-B7-P is implemented and frozen locally; publication and external supervisor acceptance are the next steps. PHASE 4-B7 (blueprint route extraction) remains NOT AUTHORIZED until a separate order.

## Historical authoritative state — PHASE 4-B6-R3 CLOSED / ACCEPTED — GOVERNANCE CLOSEOUT PUBLISHED (2026-08-04)

### PHASE 4-B6 — Alunos/Turmas/Cursos admin blueprint — CLOSED / ACCEPTED

- **Current authority/status:** PHASE 4-B4.2: CLOSED / ACCEPTED. PHASE 4-B5 CLOSED / ACCEPTED. PHASE 4-B6-P: **CLOSED / ACCEPTED**. PHASE 4-B6-R1: **CLOSED / ACCEPTED** as incorporated supervisor correction. PHASE 4-B6: **CLOSED / ACCEPTED**. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION; it is not closed. Phase 5: **NOT AUTHORIZED**. Phase 6: **NOT AUTHORIZED**. Migration v4: **PROHIBITED**. Arquivos / Alertas / Reportes are not authorized.
- **Baseline/publication boundary:** branch `refactor/architecture-safety-net`; accepted technical commit `3d9660a99e6944ff94a3991a353ecf3aaf300987`, subject `Extract admin alunos turmas cursos blueprint`; protected `main` `340fc7c91c6bc9b50e884adcb5915f9e29a0bfe1`. Technical publication: **COMPLETE**. Post-publication verification: **COMPLETE**. External supervisor acceptance: **GRANTED**. Governance closeout: published by this R3 commit.
- **Canonical B6 owner:** `app.views.admin.alunos_turmas_cursos` owns exactly 17 global legacy endpoints / 24 route-method pairs and exactly 10 route-private helpers. `main.py` has zero local B6 handler/helper bodies and re-exports all 27 moved symbols by identity. Factory default registration and explicit exact-cohort opt-out use the accepted legacy registrar. RBAC remains exactly 6 view / 13 edit / 5 full. Zero `app -> main` edge.
- **B6-P preservation:** direct canonical owners remain `app.academics` (3), `app.user_accounts` (5) and `app.web.request` (1); the accepted neutral-owner prerequisite is unchanged.
- **Original contradiction and R1 correction:** the original B6 order simultaneously required three handlers using `periodo_corrente` to move, required `periodo_corrente` to stay main-local for `_build_admin_dashboard_turma_cards`, and prohibited `app -> main`. Repository-wide templates had zero consumers. The supervisor authorized only removal of dead `periodo_corrente=periodo_corrente` kwargs from `admin_detalhes_curso`, `admin_turmas` and `admin_detalhes_turma`: exactly three removals, no fourth, no path/domain expansion. `periodo_corrente` remains main-local and byte/AST unchanged; `_build_admin_dashboard_turma_cards` still calls it; the B6 module has no binding.
- **Equivalence:** all ten helper bodies are AST-identical to `cab4c61...:main.py`; all 17 handlers are equivalent modulo route decorators and exactly the three R1 deletions. Route/rule/method/endpoint semantics and production expectations are unchanged.
- **Truthful test chronology:** original RED collected 25 = 6 passed + 19 failed, 0 errors; primary GREEN 28 passed; hermetic CSRF regeneration 2 passed, second generation byte-identical, exact 11 B6 owner-only deltas; expanded focused lane 254 passed; initial full hermetic 1093 passed / 2 failed / 17 deselected / canonical opens 0. The exact two failures were stale cumulative CSRF contracts in Matrizes and Requisições and remain historical. R2 direct corrected-node lane: 2 passed / 0 failed / 0 errors / 1.08s / exit 0. R2 structural/global lane: 34 passed / 0 failed / 0 errors / 1.29s / exit 0. Pre-freeze full: collected 1112 = 1095 passed + exactly 17 deselected, 0 failed, 0 errors, 433.87s, exit 0, `CANONICAL_SQLITE_OPENS=0`. The selective-staging whitespace gate then exposed five copied trailing-space lines, and the post-review/full gate exposed the B1 ledger-row compatibility regression; the first recovery full truthfully yielded 1093 passed / 2 failed / 17 deselected (AST literal equivalence plus B1 governance). The correction kept the three SQL-string spaces through source `\x20`, removed only two syntactic trailing spaces, restored canonical `| Fase 4 |` governance wording, passed the two focused nodes, and the repeated final full collected 1112 = 1095 passed + 17 deselected / 0 failed / 0 errors / 318.06s / exit 0 / `CANONICAL_SQLITE_OPENS=0`.
- **Exact CSRF partitions:** B6 comparison `11`; Matrizes cumulative comparison `8 + 11 = 19`; Requisições cumulative comparison `5 + 8 + 11 = 24`; each partition is exact/exhaustive in shadow-off and shadow-on, with zero non-owner and zero unexpected-route delta. No `>=`, subset-only or generic owner broadening was introduced.
- **Global invariants:** routes 131; endpoints 130; business pairs 160; governed pairs 134; RBAC unmapped 0; actor 402 = 263 allowed + 139 denied; message catalog 536. Route inventory byte-identical: 20814 bytes, SHA-256 `6e32148cd1988d5e405c9d11bdbda285359f72d5fc7eca8bd5ef8da9b83049fa`.
- **Physical custody:** canonical `database.db` 544768 bytes / SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent; canonical opens 0. Protected residual 17420 bytes / SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`. The final full left candidate manifest, index, route inventory, database, residual and runtime roots unchanged.
- **Exact candidate ceiling:** 10 technical/test paths + exactly 6 governance paths = absolute 16; path 17 hard stop. The last two technical paths are the explicitly authorized cumulative-contract corrections `tests/test_phase4_matrizes_blueprint.py` and `tests/test_phase4_requisicoes_blueprint.py`.
- **Canonical contract / next exact action:** `docs/refactor/PHASE4_ALUNOS_TURMAS_CURSOS_BLUEPRINT_CONTRACT.md`. No B6 action remains open. PHASE 4-B6 is CLOSED / ACCEPTED. Governance closeout complete. Do not begin another cohort.

## Historical authoritative state — PHASE 4-B6-P CLOSED / ACCEPTED (2026-08-04)

### PHASE 4-B6-P — Alunos/Turmas/Cursos neutral shared-owner prerequisite (published, recovered and externally accepted)
...
- **Exact 9 shared symbols moved, neutral owners 3/5/1:**
  - `app.academics` owns `build_turma_aluno_matricula`, `resequence_turma_aluno_matriculas`, `resequence_turma_aluno_matriculas_for_ids` (3).
  - `app.user_accounts` owns `_access_defaults_map`, `_default_password_for_user_type`, `create_usuario_with_default_access`, `create_usuario_with_default_password`, `normalize_usuario_access_for_user_type` (5).
  - `app.web.request` owns `_is_ajax_request` (1).
- **Identity re-exports:** `main` re-exports all nine by identity, with zero local bodies/wrappers/duplicates. All nine moved bodies are **AST-equivalent to parent `1bf5173949b0bf0bdce15d1b87d6ed15d535158c:main.py`** with no redesign.
- **Consumer resolution:** `app.views.core` imports `normalize_usuario_access_for_user_type` directly from `app.user_accounts`; the residual lazy map in `core._get_main_helpers` remains exactly `aluno_url` / `get_db_connection` / `logger`. The `login` flow `normalize_usuario_access_for_user_type(...) -> conn.commit() -> refreshed SELECT` is preserved. Acesso consumers and `_admin_access_denied_response` (uses the canonical `_is_ajax_request`) are preserved.
...
- **Remain local:** `periodo_corrente` remains local in `main` and is AST-identical to parent.
- **Import isolation:** isolated imports of the three owners (`app.academics`, `app.user_accounts`, `app.web.request`) do not import `main`; no new owner → `main` edge or cycle. `app.auth`/`app.db_maintenance` do not import `app.user_accounts`.
...
- **B6-P-R1 literal record —** `PRE_REVIEW_SCOPE_EXPANSION / EXPLICITLY_AUTHORIZED / MESSAGE_SCANNER_OWNER_RECONCILIATION / NO_RETROACTIVE_GENERIC_AUTHORITY`. R1 added `utils/messages.py` only to production scope and added exactly `PROJECT_ROOT / "app" / "academics.py"` to `_iter_backend_files()`; no recursive `app/**/*.py`, no `app/user_accounts.py`, no `app/web/request.py`, no hashing/normalization/sink/frontend/template/static/DB/editable/display/context changes. Regression RED before fix: catalog 535, key `msg_4642b1608cf6a126` absent; GREEN after fix: catalog 536, key present, exact default `"Turma sem código para gerar matrícula."`, source `app/academics.py`, no duplicate. Scanner provenance preserved literally: the moved `ValueError` message usage/source owner changed from `main.py` to `app/academics.py` (because the raising `build_turma_aluno_matricula` body moved there), while key `msg_4642b1608cf6a126` and the exact default text remained unchanged — the message itself did not change. Discrimination was an in-memory monkeypatch filtering `app/academics.py`, proving 535/key absent, then restoring the real callable/cache; final bytes retain the entry.
- **B6-P-R3 literal record —** `PRE_REVIEW_SCOPE_EXPANSION / ONE_FOR_ONE_TEST_POOL_SUBSTITUTION_PLUS_FOUR_ADDITIONS / STALE_GOVERNANCE_TEST_RECONCILIATION / MESSAGE_SCANNER_ALLOWLIST_RECONCILIATION / NO_RETROACTIVE_GENERIC_AUTHORITY`. `tests/test_residual_shared_helpers.py` lost mutation authority and remained unchanged gate-only; five paths were explicitly added; the final exact mutable test pool is the six test paths in the actual dirty manifest. First full result exposed four preexisting stale governance assertions plus one R1-caused scanner allowlist assertion: 1061 passed, 5 failed, 17 deselected, 410.35s, canonical opens 0, physical state identical. Exact five-node recovery passed 5/5 in 1.06s. Coupled five-file lane passed 77/77 in 11.26s. Fresh final full: collected 1083 = 1066 passed + exactly 17 deselected, 0 failed, 0 errors, 328.77s, exit 0, `CANONICAL_SQLITE_OPENS=0`, physical before/after candidate snapshot identical.
- **Earlier focused qualification:** collected/passed 206, failed 0, errors 0, 117.65s, exit 0. The first failed harness attempt (206 passed but exit 1) was a recoverable basetemp path translation defect (MSYS `/c` interpreted by native Windows Python as `D:/c`); rerun with native `C:/` path passed with identical physical state. Exact R1 CSRF nodes passed 2/2, snapshot bytes unchanged, B6-P CSRF delta `[0,0]`.
...
- **Fresh invariants after full GREEN:** routes 131; endpoints 130; business pairs 160; governed pairs 134; RBAC unmapped 0; actor 402 = 263 allowed + 139 denied; message catalog 536. Route inventory byte-identical: 20814 bytes, SHA-256 `6e32148cd1988d5e405c9d11bdbda285359f72d5fc7eca8bd5ef8da9b83049fa`. CSRF shadow-off and shadow-on byte-identical and equal to each other: each 288509 bytes, SHA-256 `4b16f1b4a1e672807398f33fb626f073e29ada5e9453675eabe5c824a7eec769`. Canonical `database.db` 544768 bytes, SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent; canonical SQLite opens 0. Protected residual 17420 bytes, SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`.
- **Authorized ceiling after R3:** production 6, tests 6, governance 6, total 18; path 19 hard stop. Actual non-governance dirty manifest is exactly 12 paths: production `app/academics.py`, `app/user_accounts.py`, `app/web/request.py`, `app/views/core.py`, `main.py`, `utils/messages.py`; tests `tests/test_phase4_alunos_turmas_cursos_shared_owners.py`, `tests/test_phase3_schema_startup_transaction_contract.py`, `tests/test_phase4_atividades_blueprint.py`, `tests/test_phase4_configuracoes_blueprint.py`, `tests/test_phase4_requisicoes_shared_owners.py`, `tests/test_phase4_versioning_subsystem.py`. Adding the exact six governance documents yields exact 18 changed paths. `tests/test_residual_shared_helpers.py` remains unchanged/read-only gate.
- **Canonical contract:** `docs/refactor/PHASE4_ALUNOS_TURMAS_CURSOS_SHARED_OWNER_CONTRACT.md`. Governance synchronization changes exactly six governance paths and no production, test, snapshot or database path.
...
## Historical authoritative state — PHASE 4-B5 CLOSED / ACCEPTED (2026-08-03)

### PHASE 4-B5 — Matrizes admin blueprint extraction (published)

- **Status/authority:** PHASE 4-B4.2: CLOSED / ACCEPTED. PHASE 4-B5-A: DIAGNOSIS COMPLETE / SHARED_OWNER_PREREQUISITE REQUIRED. PHASE 4-B5-P: CLOSED / ACCEPTED and immutable prerequisite history. PHASE 4-B5: **CLOSED / ACCEPTED** — technical publication COMPLETE, post-publication verification COMPLETE, external supervisor acceptance GRANTED. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION; it is not closed. Phase 5 and Phase 6 are NOT AUTHORIZED. Migration v4 is PROHIBITED. No later cohort is named or authorized as current work by this block.
...
- **Published technical commit:** `2a122357a79080fa66aa19c00ed5ff8533308f41`, subject `Extract admin matrices blueprint`, parent `ef874b9d14b02656a0f26ea885024a280d49682e`. Publication: COMPLETE. Post-publication verification: COMPLETE. Published manifest: exactly 17 paths. The former pre-publication candidate state (unstaged / uncommitted / unpushed; independent review COMPLETE / PASS; external supervisor review PENDING) is historical / superseded and is preserved only below as phase-time evidence.
...
- **Final canonical state:** `PHASE4_B5_CLOSED / ACCEPTED / MATRIZES_BLUEPRINT_ESTABLISHED / EXACT_10_LEGACY_ENDPOINTS / EXACT_12_ROUTE_METHOD_PAIRS / EXACT_21_CORRECTED_HELPERS / RBAC_3_VIEW_7_EDIT_2_FULL / B5_P_ADMIN_ACCESS_OWNER_PRESERVED / ZERO_APP_TO_MAIN_EDGE / ROUTES_131 / ENDPOINTS_130 / BUSINESS_PAIRS_160 / GOVERNED_PAIRS_134 / RBAC_UNMAPPED_ZERO / ACTOR_402_263_139 / MESSAGE_CATALOG_536 / ROUTE_INVENTORY_BYTE_IDENTICAL / CSRF_EXACT_8_OWNER_ONLY_DELTAS / FULL_HERMETIC_1050_17 / INDEPENDENT_REVIEW_PASS / TECHNICAL_COMMIT_2A122357 / PUBLICATION_VERIFIED / SECTION_22_PROCESS_DEVIATION_RECORDED / GOVERNANCE_CLOSEOUT_PUBLISHED`. PHASE 4 remains OPEN / INCREMENTAL IMPLEMENTATION. PHASE 4-B5 is CLOSED / ACCEPTED; Phase 5 and Phase 6 are NOT AUTHORIZED; Migration v4 is PROHIBITED; no later cohort is named or authorized as current work. The Section-22 pre-implementation reconciliation gate was NOT executed before the first B5 production mutation (process deviation recorded truthfully, not retroactively satisfied). The authorized governance closeout subject is `Record acceptance of Phase 4-B5`; its identity resolves through Git history.

### Section 22 — permanent historical process-deviation record

The Section-22 pre-implementation reconciliation gate was **NOT executed before the first
B5 production mutation** as required by the original order. This is a historical process
deviation. The B5-R4 hard stop detected the missing record before publication. The B5-R5
external-supervisor waiver superseded only the pre-push documentary blocker and authorized
one normal fast-forward publication of the frozen technical commit unchanged.
...
This closeout resolves the governance debt by recording it truthfully, not by rewriting
history. It does not claim that Section 22 was satisfied retroactively, that the deviation
did not happen, or that the original ordering requirement was met.

## Historical authoritative state — PHASE 4-B5-P CLOSED / ACCEPTED (2026-08-02)

### PHASE 4-B5-P — Matrizes neutral admin-access shared-owner prerequisite

- **Status/authority:** PHASE 4-B4.2: CLOSED / ACCEPTED. PHASE 4-B5-A: DIAGNOSIS COMPLETE / SHARED_OWNER_PREREQUISITE REQUIRED. PHASE 4-B5-P: CLOSED / ACCEPTED. PHASE 4-B5: NOT AUTHORIZED FOR IMPLEMENTATION. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION; it is not closed. Phase 5 and Phase 6 are NOT AUTHORIZED. Migration v4 is PROHIBITED. This is only the neutral shared owner prerequisite; no Matrizes route moved and `app/views/admin/matrizes.py` remains absent.
...
- **Ownership:** new canonical owner `app/admin_access.py` defines exactly `_fetch_user_access_overrides`, `_build_access_scope_groups_for_level`, `_load_admin_access_context`, `_get_current_admin_access_context` and `_admin_can`. `main.py` has zero local bodies for those five and directly imports/re-exports all five by identity. All five moved bodies are AST-identical to parent `a0b56896252a276e562da3842d3d61b078bd9f27:main.py` with no redesign. `_persist_user_access_overrides` and `_parse_access_overrides_from_form` remain local in `main.py`.
...
- **Import graph:** `app.admin_access -> app.auth, app.db, app.db_maintenance` and Flask `g`/`session`; `app.admin_access -> main = 0`; `app.auth -> app.admin_access = 0`; no cycle among these modules. Existing unrelated lazy `app.views.core`/`aluno -> main` edges remain baseline and are not caused by B5-P. Isolated `app.admin_access` import creates no Flask app/file/network/SQLite/migration side effect. Transaction neutrality preserved: no commit/rollback/close/connection replacement added.
...
- **TDD/recovery evidence:** corrected pre-production RED `20 failed / 2.01s / exit 1` (`app.admin_access` absent, `main` still owned the five bodies). First core GREEN attempt `2 failed / 23 passed / 3.88s` exposed two latent contradictions owned by the new test, corrected without production mutation: canonical grouped ordering follows `ACCESS_RESOURCE_GROUPS`, not `ACCESS_RESOURCE_ORDER`; decorated consumers require `__wrapped__` test introspection. Recovered core lane `25 passed / 3.78s`. Focused mandatory/access/admin-context/route/RBAC/CSRF/message lane `168 passed / 59.40s`. First full hermetic run `1 failed / 1024 passed / 17 deselected / 399.46s`: sole deterministic stale assumption at `tests/test_phase4_requisicoes_blueprint.py:432` compared historical B4.2 CSRF snapshots against moving HEAD (zero deltas). Mutation froze; user explicitly authorized exactly that one extra test path in B5-P-R1; direct minimal correction introduced `B4_2_BASELINE_COMMIT=c587098152e97d125f41a2d26f2f414c10ae5676` and replaced only HEAD with that fixed reference. Classification verbatim: `PRE_REVIEW_SCOPE_EXPANSION / NOW_EXPLICITLY_RATIFIED / NO_RETROACTIVE_GENERIC_AUTHORITY`. R1 exact node `1 passed / 0.68s`; read-only comparison proves exactly five owner-only deltas per shadow-off/on snapshot, exact five Requisições POST routes, `main.<handler> -> app.views.admin.requisicoes.<handler>`, all other fields equal. A later full run was externally interrupted before result; not a physical failure or final result. B5-P-R2 SUPPLEMENTAL SCOPE AUTHORIZATION added and modified only `tests/test_phase4_requisicoes_shared_owners.py`: it reads accepted B4.1 governance from fixed commit `c587098152e97d125f41a2d26f2f414c10ae5676`, preserves B4.1 closure/publication/assertions, and separately proves current B4.2 CLOSED / ACCEPTED, B5-P IMPLEMENTED / AWAITING SUPERVISOR REVIEW, Phase 4 OPEN, Phase 5 and Phase 6 NOT AUTHORIZED, Migration v4 PROHIBITED; no unrelated assertion weakened. Classification verbatim: `PRE_REVIEW_SCOPE_EXPANSION / EXPLICITLY_AUTHORIZED_B5_P_R2 / FIXED_ACCEPTED_B4_1_BASELINE / NO_RETROACTIVE_GENERIC_AUTHORITY`. R2 exact node `1 passed / 1.02s`; affected governance aggregate `118 passed / 16.70s`. Final pre-publication hermetic after R2: total collected 1042 = 1025 passed + exactly 17 D73H deselected; 0 failed; 0 errors; 326.74s; exit 0. The earlier 1025 passed / 17 deselected / 306.41s run is only pre-R2 historical evidence. Historical first full hermetic `1 failed / 1024 passed / 17 deselected / 399.46s` (stale B4.2 CSRF baseline) is superseded by the B5-P-R1 fixed-baseline correction. Accepted final evidence: independent-review rerun `1025 / 17 / 0 / 0 / 367.27s`; post-publication focused `132 passed / 0 failed / 0 errors / 39.23s`. All pre-publication pending statements are historical/superseded.
- **Invariants:** routes 131; endpoints 130; business pairs 160; governed pairs 134; RBAC unmapped 0; actor 402 = 263 allowed + 139 denied; message catalog 536. Route inventory byte-identical: 20814 bytes, SHA-256 `6e32148cd1988d5e405c9d11bdbda285359f72d5fc7eca8bd5ef8da9b83049fa`. CSRF shadow-off and shadow-on byte-identical to HEAD and to each other: each 288349 bytes, SHA-256 `3a94e2e1ca587ffe966262717e2cdcde1251ef90b61202d81d4b681ae79a0056`; historical B4.2 five-owner-only deltas remain proven against fixed `c587098...` baseline. Canonical database: 544768 bytes, SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent; canonical SQLite opens 0. Protected residual: 17420 bytes, SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`.
- **Execution evidence:** Supervisor/IAsup: Hermes `openai-codex/gpt-5.6-sol`, direct adjudication and deterministic gates. TDD/implementation IAexec continuity session `ses_03ce1c465ffeb31XEcAo57hbC4`. Initial route `flash_free`; later explicit fallback to `opencode-go/deepseek-v4-flash` because FREE context exceeded policy threshold, trigger `FALLBACK_FREE_CONTEXT_TOO_LARGE`. Recorded router costs: RED correction `0.0008019144 USD`; GREEN `0.0009146312 USD`; test correction `0.0007943432 USD`. No silent escalation. B5-P-R1 direct IAsup correction rationale: the exact deterministic two-site fixed-reference test correction was already fully specified and independently proved; a fourth 234k-context delegated continuation would increase cost/context/risk without judgment value. Earlier B5-P governance authoring succeeded via `flash_normal`, `opencode-go/deepseek-v4-flash`, session `ses_03cb9cee8ffeyk6hJFuMcmRrgD`, exit 0, cost `0.0013047696 USD`, no fallback, exact six governance mutations. A later continuation attempt to that expired/nonexistent session failed with `FLASH_NORMAL_FAILED / Session not found / no session / no cost / no mutation`; recorded as a recoverable transport/session failure. The fresh explicit delegation then succeeded via `flash_normal`, `opencode-go/deepseek-v4-flash`, session `ses_03c9ae6a7ffe4XeucPqy9Iyrre`, exit 0, cost `0.0005911248 USD`, with no fallback and mutations limited to the same six governance paths. The R2 test correction was
      a direct IAsup Sol correction because it was deterministic, exact, tiny, and safer/cheaper than another context export. Independent read-only
      review is COMPLETE: provider `opencode`, model `opencode/deepseek-v4-flash-free`, session `ses_03c92c10affegAmZLZ63tmTjjA`, exit 0, cost 0,
      fallback none, mutation 0, blocking findings 0, nonblocking findings 3 (R2 no literal `NO ROUTE MOVEMENT` token; `B4_2_BASELINE_COMMIT`
      naming; runtime duration changed); external acceptance GRANTED by the PHASE 4-B5-P-R3 order.
...
- **Ownership:** `app.views.admin.requisicoes` owns the nine handlers plus `ALLOWED_EXCEL` and four route-private helpers. `main` preserves identity exports. The new owner consumes accepted neutral owners directly, uses `current_app.config` as the sole mechanical owner adaptation, and has zero direct or dynamic `app -> main` edge. AST adjudication proves all 13 moved function bodies unchanged modulo route-decorator removal and `app.config -> current_app.config`.
- **Compatibility:** factory default registers the exact cohort after accepted Phase-4 cohorts; opt-out removes exactly the nine endpoints; independent apps remain isolated. The supervisor expressly authorized the mechanical monkeypatch owner correction in `tests/test_admin_snapshot_diagnostics.py`. Two B1/B3 structural tests were advanced within the originally authorized pool to recognize the new owner without weakening their assertions.
- **Accepted evidence:** TDD RED `20 failed / 3 passed`; primary GREEN `138 passed`; complementary GREEN `107 passed`; first full lane `1001 passed / 4 failed / 17 deselected` remains historical. Final hermetic `1005 passed / 17 deselected / 0 failed / 0 errors / 362.33s`; index-visible `57 passed / 0 failed / 0 errors`; post-publication `56 passed / 1 deselected / 0 failed / 0 errors`. Routes 131; endpoints 130; governed pairs 134; RBAC unmapped 0; actor 402 = 263 allowed + 139 denied; message catalog 536. Post-commit CSRF comparison `HEAD^ -> HEAD`: `[5, 5]` owner-only deltas. Route inventory unchanged. Canonical SQLite opens: 0.
- **Accepted technical review:** logical `flash_free`; `opencode` / `opencode/deepseek-v4-flash-free`; session `ses_03ff5695bffeEBNIZRiSVZonYz`; exit 0; cost 0; fallback none; mutation 0; PASS / blocking NONE / scope EXACT / behavior PRESERVED. Hash-method finding: VALID / NONBLOCKING / DOCUMENTARILY CORRECTED / NO TECHNICAL IMPACT.
- **Accepted documentary addendum:** requested `flash_free`, selected `flash_normal`; `opencode-go` / `opencode-go/deepseek-v4-flash`; session `ses_03fce2ebbffejkf6IUdfPrsxF3`; exit 0; fallback `FALLBACK_FREE_BUDGET_EXHAUSTED`; router cost `0.0011911032 USD`; observed step-finish aggregate `0.0180531456 USD`; mutation 0; outside-scope reads 0; PASS / blocking NONE. Flash FREE itself did not complete this addendum.
- **Accepted identities:** raw complete `1b8435a9db10f8a2ae680f60c17a9ad0a723eed88066a834ca59255bf7b8cc0e`; Git-canonical complete `c362566627667ba684765ad3ea8fdeb9abf7678dd52e185cedd3ed8b08a891b4`; governance raw/canonical `19519783e02bf983f820d357e6c2b250db541581fae57f6988e64cf1900f544d` / `b44ae4231aaaeb022c2cfc2ca94f20be76a94dd7e82c51e97966866d294d0ceb`; production/test raw `f60ebdab5cd1e7aa2d98d9ade66534925c5a077326dff2e70b24c72e6390c037`. The raw worktree and Git-canonical content identities use the distinct algorithms documented in the B4.2 contract.
- **Physical safety:** canonical `database.db` remains 544768 bytes / SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent. Protected residual remains 17420 bytes / SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`.
- **Historical pre-publication state (superseded):** the former prepublication candidate state and its outstanding documentary, Git-publication and external-acceptance gates are phase-time statements only. RED/GREEN, first-failed-full-suite, review/fallback, hash-normalization and `tests/test_admin_snapshot_diagnostics.py` scope-expansion history remain preserved.
- **Governance closeout:** exactly six governance paths; authorized subject `Record acceptance of Phase 4-B4.2`; identity resolves through Git history. No technical path is changed by the closeout.
...
## Historical authoritative state — PHASE 4-B4.1 CLOSED / ACCEPTED (2026-08-01)

### PHASE 4-B4.1-R2 — external supervisor acceptance and canonical reconciliation

- **Status/authority:** PHASE 4-B4-A: CLOSED / ACCEPTED. PHASE 4-B4.1: CLOSED / ACCEPTED. B1, B2 and B3 remain CLOSED / ACCEPTED. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION. PHASE 4-B4.2: NOT AUTHORIZED. Phase 5: NOT AUTHORIZED. Phase 6: NOT AUTHORIZED. Migration v4: PROHIBITED. No B4.1 technical residual remains.
- **Published technical identity:** commit `73ebf0dc34681e74e778759af476e1cd2f981444` (`Extract requisition shared owners`), parent `185426daccc9f0eb0dba4497248100c1a88d15fa`; exact 20-path artifact = 7 production + 7 tests + 6 governance. Publication: COMPLETE. Post-publication verification: COMPLETE. The governance closeout uses authorized subject `Record acceptance of Phase 4-B4.1`; its identity is resolved through Git history.
- **Accepted expanded scope:** `app.settings` owns the eight B1 settings helpers; `app.requisitions` owns `auto_indefer_devolvidas`; `app.matrix_scope` owns exactly six matrix-scope symbols. Configurações and `main` preserve identity exports. Aluno imports `get_effective_matriz_for_turma` directly. Classification: `B4_1_SCOPE_EXPANSION / COHERENT_NEUTRALIZATION / SUPERVISOR_RECONCILED / INCLUDED_IN_ACCEPTED_B4_1_COMMIT / NO_GENERIC_RETROACTIVE_AUTHORITY`.
- **Accepted history and boundaries:** B2 established six Aluno lazy-main edges at B2-time. B4.1 removes only the matrix resolver edge and leaves exactly five: `ensure_admin_arquivos_table`, `get_admin_arquivo`, `get_student_request_update_alert`, `list_active_admin_alertas`, and `mark_student_request_updates_seen`. NO ROUTE MOVEMENT; exactly nine Requisições handlers / 12 route-method pairs remain in `main.py`; `app/views/admin/requisicoes.py` is absent.
- **Accepted gates:** final hermetic 984 passed / 17 deselected; index-visible 170 passed; post-publication 170 passed; routes 131; endpoints 130; governed pairs 134; RBAC unmapped 0; actor 402 = 263 allowed + 139 denied; message catalog 536; canonical SQLite opens 0.
- **Accepted reviews:** technical review `flash_free`, provider/model `opencode` / `opencode/deepseek-v4-flash-free`, session `ses_040d538bfffegBsAJQbLJrnqSV`, exit 0, cost 0, fallback none, mutation 0, PASS / material findings NONE. Documentary addendum `flash_free`, same provider/model, session `ses_040ae7625ffeHCGZxyXoMzoQmw`, exit 0, cost 0, fallback none, mutation 0, PASS / blocking and nonblocking findings NONE. Preserve `READ_SCOPE_DEVIATION / NON_MUTATING / PHYSICALLY_RECONCILED / NO_CANDIDATE_IMPACT`.
- **Accepted hashes:** technical reviewed tracked diff `f4b6cb00b4365cc7c20af5fcba1ac736ece1bab0ab9c6e0f89b19084799727f9`; complete pre-addendum `ddc73cb94786899595f3cce9d577bdbcca961a082c83866498a36ebd4687a23f`; non-governance `74649089be0699dff4440260bdb11793b4e5793550f17456893f4a281bd6096b`; final publication `bdd947a18df900aac691ce12683f393a82d8c8efc3e28a8c54c55226d7bf2d4a`; governance `301a6a891936338bd1a752ff0be65dd7855302d1e3a8e81f9bcbd94964b66a71`. “8 tests” remains `REJECTED_NON_MATERIAL / STALE_NARRATIVE_COUNT / NO_TECHNICAL_CONSEQUENCE`; exact count seven.
- **Physical safety:** canonical `database.db` 544768 bytes / SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent. Protected residual 17420 bytes / SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`.
- **Next exact action:** none for B4.1. B4.2 remains separately gated and is not authorized.

## Historical authoritative state — PHASE 4-B3 CLOSED / ACCEPTED (2026-08-01)

### PHASE 4-B3-R3 — external supervisor acceptance and canonical reconciliation

- **Status/authority:** PHASE 4-B3 CLOSED / ACCEPTED. PHASE 4-B1 and PHASE 4-B2 remain CLOSED / ACCEPTED; B2 commit `17e468ad938e873e1f9e9c303808ad31b9f3806b` remains published with post-publication verification COMPLETE. Technical B3 commit `50801b6bdddc4d2772853c13f4905c49e8c996cf` (`Extract admin activities blueprint`), parent `81cc6b10b893f1d34bd211a527e9fd12c3b6bbbe`, is published. Publication: COMPLETE. Post-publication verification: COMPLETE. PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION. PHASE 4-B4, Phase 5 and Phase 6 are NOT AUTHORIZED; migration v4 is PROHIBITED. No B3 residual or correction remains open.
- **Exact accepted artifact:** 16 paths = 6 production (`app/__init__.py`, `app/activity_catalog.py`, `app/uploads.py`, `app/views/admin/atividades.py`, `main.py`, `utils/messages.py`) + 4 tests/snapshots (`tests/_artifacts/csrf_inventory_shadow_off.json`, `tests/_artifacts/csrf_inventory_shadow_on.json`, `tests/test_phase4_atividades_blueprint.py`, `tests/test_phase4_configuracoes_blueprint.py`) + 6 governance (`AGENT_HANDOFF.md`, `PROJECT_STATE.md`, `docs/DOCUMENTATION_INDEX.md`, `docs/mapeamento/05_avaliacao_refactor.md`, `docs/refactor/ARCHITECTURE_REFACTOR_LEDGER.md`, `docs/refactor/PHASE4_ATIVIDADES_BLUEPRINT_CONTRACT.md`).
- **Accepted owners:** `app.views.admin.atividades` is the canonical owner of exactly 22 legacy global endpoints / 29 route-method combinations. `app.activity_catalog` is the canonical neutral shared owner of 14 helpers. `app.uploads` is the neutral `save_upload` owner. `main` retains compatibility identities. Matrizes/Requisições route cohorts remain unmoved.
- **Accepted compatibility/reconciliation:** route and RBAC semantics are unchanged; message inventory 536→536 with zero semantic/default/kind/key delta; each CSRF snapshot has exactly 15 owner-only deltas. Routes 131; endpoints 130; governed pairs 134; RBAC unmapped 0; actor matrix 402 = 263 allowed + 139 denied.
- **Accepted gates/reviews:** final hermetic 974 passed / 17 deselected; index-visible 300 passed; post-publication 300 passed. Technical review PASS / findings NONE. Documentary addendum PASS / findings NONE.
- **Accepted hashes:** technical reviewed hash `ec96796d3541710a36ac8121e40ffd888737c7c926f191a28034482cedbfd556`; final publication hash `c41ffe5b7328b6d5a986dbdc28f054fe89641496589003b4e5649ff88463cc19`; governance hash `af2906ef0fa9fef7fdd469dd4e967cd1c914b4bfb21fc2a132b8d74c2d8dfd27`; non-governance hash `13b0af13e653641d75d2466d7d8d69090e655a18e28bb678a7090dbe0e2ecab0`.
- **Review history:** provisional FREE `opencode/deepseek-v4-flash-free`, session `ses_0425bf1cbffeIxMwsDQn0etSEC`, cost 0: PROVISIONAL / UNUSABLE DELIVERY / NO VERDICT because the external bundle was inaccessible. Final FREE session `ses_0422f0e1cffepCXyNpg47dWpoq`, cost 0: TIMEOUT / NO ACCEPTED DELIVERY. Accepted final technical review: logical `flash_free`, selected `flash_normal`, router trigger `FALLBACK_FREE_BUDGET_EXHAUSTED`, antecedent the immediately preceding FREE timeout; effective `opencode-go/deepseek-v4-flash`, session `ses_04224ca47ffe5qAwwHGtxlR7i7`, router cost `0.0010424344 USD`, observed step-finish aggregate `0.0206550400 USD`, mutation 0, PASS / findings NONE; IAsup PASS ACCEPTED.
- **Documentary addendum:** first verdict REJECT due only to stale historical Documentation Index wording; correction was governance-only. Final effective model `opencode-go/deepseek-v4-flash`, session `ses_04203dca3ffe8OM83rhbHyjqYI`, reviewed hashes `c41ffe5b7328b6d5a986dbdc28f054fe89641496589003b4e5649ff88463cc19` / `af2906ef0fa9fef7fdd469dd4e967cd1c914b4bfb21fc2a132b8d74c2d8dfd27` / `13b0af13e653641d75d2466d7d8d69090e655a18e28bb678a7090dbe0e2ecab0`; final verdict PASS.
- **Historical pre-publication state (superseded):** PHASE 4-B3 IMPLEMENTED / AWAITING SUPERVISOR REVIEW; review, staging, commit, push and publication were pending at that phase-time. They are not current state.
- **Historical B2-time boundary (superseded):** PHASE 4-B3: NOT AUTHORIZED. The separate B3 order and accepted publication superseded only that phase-time prohibition.
- **Scope-expansion time semantics:** historical pre-publication classification `PRE_REVIEW_SCOPE_EXPANSION / MECHANICALLY_CAUSED_BY_CANONICAL_OWNER_MOVE / NOT_STAGED / NOT_COMMITTED / PUBLICATION_WITHHELD`. Current disposition `EXACT_DELTA_PROVED / SUPERVISOR_RECONCILED / INCLUDED_IN_ACCEPTED_B3_COMMIT / NO_GENERIC_RETROACTIVE_AUTHORITY`.
- **Physical safety:** canonical `database.db` remains 544768 bytes / SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent. Protected ignored residual remains 17420 bytes / SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`, ignored and index-absent.
- **Next exact action:** none for B3. B4 requires a new separate order.

## Historical authoritative state — PHASE 4-B2 CLOSED / ACCEPTED (2026-08-01)

### PHASE 4-B2-R3 — external supervisor acceptance and post-publication reconciliation

- **Status/authority:** PHASE 4-B2 CLOSED / ACCEPTED. Accepted technical commit `17e468ad938e873e1f9e9c303808ad31b9f3806b` (`Extract versioning subsystem ownership`), parent `2fbe4954106dc8d410f6495ca8bd4b1956b326d2`, is published. Post-publication verification: COMPLETE. Exact published manifest: 24 paths = 8 production + 10 tests + 6 governance. B1 remains CLOSED / ACCEPTED.
- **Canonical owner map:** `app.versioning.resolver` owns resolver/read-model bodies; `app.versioning.snapshots` owns snapshot feature flags, payload and caller-owned write; `app.versioning.shadow_reads` owns shadow-read event/sink/parse/filter/dedup/source behavior; `app.views.admin.versioning` owns exactly three diagnostic handlers. `main` contains no local body for the three primary runtime functions or three handlers and re-exports canonical objects by identity.
- **Aluno reverse edge:** `app/views/aluno.py` directly imports the snapshot and shadow-read functions. Its lazy-main map contains exactly six residual dependencies: `ensure_admin_arquivos_table`, `get_admin_arquivo`, `get_effective_matriz_for_turma`, `get_student_request_update_alert`, `list_active_admin_alertas`, `mark_student_request_updates_seen`.
- **Exact three routes:** GET `/admin/diagnostico/atividades-versionadas` → `admin_diagnostico_atividades_versionadas` (`atividades:view`); GET `/admin/diagnostico/atividades-versionadas/view` → `admin_diagnostico_atividades_versionadas_view` (`atividades:view`); GET `/admin/diagnostico/versioned-shadow-reads` → `admin_diagnostico_versioned_shadow_reads` (`banco_dados:view`). The accepted B1 registrar is reused; no namespaced alias or duplicate rule exists.
- **Logging R1 repair:** the extracted `__file__` expression initially relocated the default log root to `app/versioning`. R1 establishes `PROJECT_ROOT = Path(__file__).resolve().parents[2]`, preserves logger identity with `logging.getLogger("main")`, default dedicated `<repo>/logs/versioned_shadow_reads.log`, explicit fallback `<repo>/logs/app.log`, configured `APP_LOG_DIR`, handler-derived candidates, rotation ordering and dedicated/fallback precedence. Import creates no handler, directory, log, database or network connection.
- **Recovered full-suite evidence:** the first invalid full run was 949 passed / 2 failed / 17 deselected in 384.10s. Both failures were stale lazy-map expectations in `tests/test_db_schema_maintenance.py` and `tests/test_residual_shared_helpers.py`; they were corrected within the expanded authorized pool.
- **Accepted gates:** logging/B2 contract 14 passed; complete affected lane 241 passed in 106.86s; full hermetic 954 passed / 17 deselected / 343.75s / exit 0; index-visible 271 passed; post-publication 282 passed. Direct counts: routes 131, endpoints 130, governed pairs 134, RBAC unmapped 0, actor matrix 402 = 263 allowed + 139 denied. Route snapshot remains byte-identical.
- **Physical safety:** canonical `database.db` remains 544768 bytes / SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; WAL/SHM/journal absent. Protected ignored residual remains 17420 bytes / SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`, ignored and index-absent. Candidate/runtime-surface hashes were stable across the full gate; no pytest process remained.
- **Exact published manifest:** 24 paths: eight production; three original-pool tests; all seven R1-expanded mechanically necessary tests; six governance paths. `tests/test_activity_versioning_resolver.py` was authorized but omitted because it needed no semantic delta. No 25th/26th path was required. Full classification is canonical in `docs/refactor/PHASE4_VERSIONING_SUBSYSTEM_CONTRACT.md`.
- **Test-harness expansion:** `tests/conftest.py` plus `tests/test_pytest_runtime_isolation.py` support/prove pre-staging cached and non-ignored untracked candidate sandboxes without weakening isolation. The other five expanded tests reconcile canonical ownership, exact lazy-map and B1/RBAC invariants only. All seven are retained; none is omitted.
- **EOL:** normalized semantic comparisons found no EOL-only delta or whole-file EOL rewrite. `.gitattributes` is unchanged; no broad formatter/normalizer ran.
- **Technical review:** FREE `opencode/deepseek-v4-flash-free`, session `ses_0433de371ffefBa8J03FBmkoV4`, cost 0, exit 0 returned UNUSABLE DELIVERY / NO VERDICT; fallback `FALLBACK_FREE_UNUSABLE_DELIVERY`. Accepted review used `opencode-go/deepseek-v4-flash`, session `ses_043375c9affeYmMxqtbpNjNohl`, cost `0.01838004 USD`, exit 0, reviewed diff SHA-256 `a97275ac9f29cefcfd8ed4d3038ce37f552a886036481be0f7fd1c7f85a373b7`, mutation count 0, verdict PASS, material findings 0.
- **IAsup adjudication:** LOW matrix status-label finding REJECTED AS NON-MATERIAL / SEMANTIC EQUIVALENCE PROVED; baseline and extracted labels/fallback are equivalent.
...
- **Scratch nonconformity:** `baseline_main.py`, SHA-256 `2652d1213d7f0b5ac577ebddb528341448e9eb0afb8b41d051e5826a56d4af48`: EXTERNAL_REVIEW_SCRATCH / OUTSIDE_REPOSITORY / NOT_STAGED / NOT_COMMITTED / SELECTIVELY_REMOVED / NO_CANDIDATE_OR_INDEX_IMPACT. Do not recreate it.
...
- **Current boundaries / next exact action:** no B2 technical residual or open correction is demonstrated. PHASE 4 remains OPEN / INCREMENTAL IMPLEMENTATION. PHASE 4-B3: NOT AUTHORIZED. Phase 5: NOT AUTHORIZED. Phase 6: NOT AUTHORIZED. Migration v4: PROHIBITED. B3 requires a new separate authority.

## Historical authoritative state — PHASE 4-B1 CLOSED / ACCEPTED — PHASE 4 OPEN / INCREMENTAL IMPLEMENTATION (2026-07-31)

### PHASE 4-B1 — Configurações/Mensagens legacy-endpoint blueprint

- **Status/authority:** PHASE 4-B1 CLOSED / ACCEPTED. Accepted technical commit `cd8a76b2484abc376174332578ecd8be4b8206ea`, subject `Extract admin configuration blueprint`, parent `7f393c72ad3e9d70eae4c06ee41e0d74881e40f2`. The exact 13-path artifact is committed, published and post-publication verified; there is no open B1 correction.
- **Exact production scope:** `app/views/admin/configuracoes.py` owns exactly eight Configurações/Mensagens views plus the eight settings helpers. `app/views/admin/__init__.py` owns the reusable immutable legacy-route registrar. `app.create_app` registers the blueprint by default and exposes the explicit opt-out flag. `main.py` imports and re-exports every moved route/helper by identity; it neither wraps nor re-registers them.
- **Compatibility contract:** `docs/refactor/PHASE4_ADMIN_BLUEPRINT_COMPATIBILITY_CONTRACT.md`. Legacy global endpoint names, rules, methods, RBAC requirements, templates, redirects, CSRF behavior and persistence behavior are frozen. The registrar uses `Blueprint.record_once` and `state.app.add_url_rule`; collisions raise explicitly before any B1 route is added. Namespaced aliases and duplicate rules are prohibited.
- **Accepted physical/test evidence:** original B1 RED 15 failed / 6 passed; focused GREEN 21 passed; B1-R1 RED/GREEN 4 failed then 4 passed; two corrected CSRF nodes 2/2; targeted matrix 47 passed; full hermetic 939 passed / 17 deselected; post-publication 91 passed. Routes 131, endpoints 130, governed pairs 134 and RBAC unmapped 0. Canonical `database.db`, route/CSRF snapshots and protected residual remained byte-identical; WAL/SHM absent. Independent review route `flash_free`, effective `opencode/deepseek-v4-flash-free`, session `ses_0453cc57fffexsyJU43Oyi1wwz`, cost 0, no fallback/mutation, verdict PASS; no functional residual was demonstrated.
- **Exact accepted manifest:** `AGENT_HANDOFF.md`, `PROJECT_STATE.md`, `app/__init__.py`, `app/views/admin/__init__.py`, `app/views/admin/configuracoes.py`, `docs/DOCUMENTATION_INDEX.md`, `docs/mapeamento/05_avaliacao_refactor.md`, `docs/refactor/ARCHITECTURE_REFACTOR_LEDGER.md`, `docs/refactor/PHASE4_ADMIN_BLUEPRINT_COMPATIBILITY_CONTRACT.md`, `main.py`, `tests/test_csrf_inventory_audit.py`, `tests/test_phase4_configuracoes_blueprint.py`, `utils/messages.py` — exactly 13 paths.
- **Accepted discovery model:** deterministic filesystem-recursive repository-tree discovery under `app/views/**/*.py`; the implementation does not query or filter through the Git index. Clean-worktree and repository-boundary assumptions remain execution-protocol gates.
...
- **Formal decision/boundaries:** `PHASE 4-B1: CLOSED / ACCEPTED`; `PHASE 4: OPEN / INCREMENTAL IMPLEMENTATION`; `PHASE 4-B2: NOT AUTHORIZED`; `PHASE 5: NOT AUTHORIZED`; `PHASE 6: NOT AUTHORIZED`; `Migration v4: PROHIBITED`. Phase 4 is not closed. No auth, database, template, JavaScript/static, snapshot, migration, versioning, RBAC or runtime delta is accepted. `dashboard.py` and `admin_meus_dados` ownership remain unresolved.
- **Next exact action:** no B1 correction is open. PHASE 4-B2 requires a new separate order; no B2 implementation is authorized by this acceptance.

### Macro Phase 3 — formal data-access-consolidation acceptance closeout

- **Status and accepted commits:** Macro Phase 3, PHASE 3-B11, and PHASE 3-B11-R1 are CLOSED / ACCEPTED by external supervisor decision. Accepted technical commit: `c9009bf3d68950ad4e0499b65928603e84bee341` (`Unify database initialization ownership`), parent `e63e1a66b9d2ebad7253a0efd2e0a367b89b8b8a`. Accepted governance commit: `630d4eb448b992bdc3beb28752c30717989312bb` (`Record B11 publication and review closeout`), parent `c9009bf3d68950ad4e0499b65928603e84bee341`. B11 technical commit: COMMITTED AND PUSHED. B11 post-commit verification: COMPLETE.
- **Mandatory Phase 3 requirements satisfied:** (1) canonical database connection ownership is solely `app.db`; (2) `app.db.init_db` is the sole defining initializer and `main.init_db` is compatibility identity only; (3) versioned maintenance is exact v1/v2/v3 with `SCHEMA_VERSION == 3`, isolated destructive migration boundary, and retired recurring rebuild debt; (4) `app.db → main` dependencies and lazy bridge entries are zero; (5) canonical database, protected residual, route inventory, RBAC and hermetic/index-visible/post-publication gates remain accepted.
- **Final technical architecture:** defining init bodies 1 (`app.db` 1, `main` 0); `_init_db_impl`, `_get_main_db_helpers`, and `_sync_database_from_main` absent; qualified inventory 72 compatibility calls / 5 direct-owner calls plus three bare imported-owner calls. Deterministic transaction and failure postconditions remain unchanged and accepted. No migration v4 exists.
- **Optional repository-layer disposition:** OPTIONAL / NOT IMPLEMENTED / NOT A CLOSURE BLOCKER. No `app/repositories/` extraction is claimed complete, no open Phase 3 implementation remains, and this optional work is not silently transferred to Phase 4.
- **Tests and physical proofs:** final hermetic `913 passed / 17 deselected / 416.66s / exit 0`; index-visible 67 passed; route/RBAC `3 passed / routes 131 / unmapped 0`; B11 post-publication `212 passed / 42.37s`; latest governance/route/RBAC lane 12 passed. Canonical `database.db` remains 544768 bytes / SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`, WAL/SHM absent; protected ignored residual remains 17420 bytes / SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`, index-absent.
- **Accepted review:** verdict `APPROVE`; Material/Critical/High findings 0; accepted LOW intentional/tested `main` compatibility-import coupling. The complete routing/session/cost evidence and accepted INFO findings remain permanently recorded in the canonical contract. Review must not be repeated.
- **Reviewer nonconformity 1:** `SAME_SESSION_OUTPUT_RECOVERY / SECOND_INVOCATION_PROCESS_NONCONFORMITY / SAME_PROVIDER_MODEL_AND_SESSION / NO_NEW_TOOLS / NO_DIFF_CHANGE / NO_REVIEWER_SUBSTITUTION / USABLE_VERDICT_RECOVERED / ACCEPTED_WITH_DECLARATION`. First post-opt-in inference had no final text; textual continuation in the same session/model/provider recovered the verdict. No fallback, repository mutation, tool expansion, reviewer substitution, or candidate change occurred.
- **Reviewer nonconformity 2:** `EXTERNAL_TEMPORARY_FILE_MUTATION / OUTSIDE_REPOSITORY / NOT_STAGED / NOT_COMMITTED / NO_REPOSITORY_OR_INDEX_IMPACT / UNAUTHORIZED_BY_REVIEW_CONTRACT / ACCEPTED_WITH_DECLARATION`. Declared external artifact `/tmp/candidate.diff`; reported size 144049 bytes; reported SHA-256 `fcd1b62e141434dccaa89dabe9b604afe61977c96490674503b9410185627771`; preserved without inspection, alteration, or deletion under B11-R1.
- **Scope and supersession:** no production correction or functional residual was demonstrated. There is no open Phase 3 technical implementation. The prior blanket statement `Phase 4 remains NOT AUTHORIZED` is superseded only by the explicit B1 authority above; it remains historical for the Phase 3 closeout. No authority extends to B2, Phase 5, Phase 6, or migration v4.

### Historical / superseded recovery narrative — B11 pre-publication state (2026-07-30)

The first FREE review delivery timed out without usable output, so the phase correctly stopped before staging and withheld commit/publication at that time. The later accepted same-session recovery and explicit paid-Flash review record superseded that temporary restriction. Direct Git evidence now establishes the published B11 technical commit above; the earlier requirement for a new review attempt is historical and no longer governs B11-R1.

### Historical PHASE 3-B10 — activity-versioning core schema migration v3, self-transactional rebuild retirement, direct core ownership, and two-entry lazy bridge (2026-07-30)

- **Status/baseline:** COMMITTED AND PUSHED / POST-COMMIT VERIFIED / AWAITING SUPERVISOR REVIEW / GOVERNANCE CORRECTION REQUIRED. B10-R1 corrects the governance record to accurately reflect the actual 15-path commit manifest. B9 is CLOSED / ACCEPTED by the explicit B10 order.
- **Commit:** `8fe0345eab08e312f7e015730f70d02327e7eb5f`, subject `Version activity versioning core schema`, parent `37c8757e4ef41647c971ad8a974c853dec6ce4e7`, 15 files, 1726 insertions, 464 deletions. Published to `origin/refactor/architecture-safety-net`.
- **Core migration v3:** `SCHEMA_VERSION == 3`; v3 registry entry `normalize_activity_versioning_core` owned by `app.db_maintenance`. Migration v3 normalizes five legacy `atividade_versao` variants, preserves parent/child/self-reference data, and handles deterministic `numero_versao` assignment and `sqlite_sequence`. The early runner is the sole destructive migration boundary.
- **Self-transactional rebuild retired:** All seven legacy symbols removed from `main.py`. The public orchestrator `ensure_atividade_versioning_schema` is now solely defined in `app.db_maintenance`. `main.py` imports it by identity.
- **Lazy bridge:** Exactly two entries: `get_preferred_matriz_for_curso` and `logger`. `ensure_atividade_versioning_schema` is a direct import from `app.db_maintenance` in both `app.db` and `main.py`.
- **B8/B9/B7 isolation:** B8 four leaf tables, two leaf triggers, eight leaf indexes unchanged. B9 atividades v2 unchanged. B7 backup-settings unchanged. No B11 work.
- **Actual 15-path manifest:** `app/db_maintenance.py`, `app/db.py`, `main.py`; `tests/test_activity_versioning_core_migration_v3.py` (new), `tests/test_atividades_schema_migration_v2.py`, `tests/test_activity_versioning_leaf_schema_ownership.py`, `tests/test_backup_settings_ownership.py`, `tests/test_db_schema_maintenance.py`, `tests/test_phase3_schema_startup_transaction_contract.py`, **`tests/test_ref_0c_b1_p0_access_context_transactions.py`** (undocumented fifteenth path — mechanically required v3 expectation update, content correct, not preauthorized, process nonconformity, no production scope expansion); canonical contract, Documentation Index, Architecture Ledger, this state, `AGENT_HANDOFF.md`.
- **Test evidence:** Full hermetic: 898 passed, 17 D73H deselected, 0 failures in 302.78s. Focused: 49 passed. Contract gate: 163 passed. Regression: 352 passed. Post-commit: 231 passed. Routes 131; RBAC unmapped 0. Canonical database opens 0.
- **Canonical safety:** `database.db` 544768 bytes, SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`, WAL/SHM absent. Residual 17420 bytes, SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`.
- **Process nonconformity:** The published B10 governance documents recorded a 14-path manifest. The actual commit contains 15 paths. The fifteenth path (`tests/test_ref_0c_b1_p0_access_context_transactions.py`) changed only the final expected migration metadata (v2→v3, user_version 2→3). Content is accepted as mechanically required by the SCHEMA_VERSION change. B10-R1 corrects all governance records without reverting the path.
- **Next action:** supervisor review of B10 and B10-R1. PHASE 3-B11 remains unauthorized.

### Historical PHASE 3-B9 — versioned atividades schema rebuild (2026-07-27)

- **Status/baseline:** IMPLEMENTED / LOCALLY VERIFIED / INDEPENDENTLY REVIEWED / PUBLICATION PENDING AT COMMIT-TREE TIME. B8 is CLOSED / ACCEPTED by the explicit B9 order. Baseline workspace `D:\OneDrive\Programação\SGAA_clean_baseline`, branch `refactor/architecture-safety-net`, HEAD/upstream/live remote `42ad0b500fe26fb2a4f49a2f8655d0217233af75`, parent `d1947b0c11506045b8d52bd235bc7381a2ca22c9`, divergence 0/0 and clean before mutation.
- **Selected path/root cause:** PATH B — isolated early migration transaction. Deferred FK checking was rejected because `DROP TABLE atividades` can immediately cascade-delete `matrizes_atividades_itens` while leaving `foreign_key_check` empty. The recurring ten-column rebuild could also discard `documentos_json`.
- **Migration/target:** `app.db_maintenance` solely owns `SCHEMA_VERSION == 2`, contiguous v1/v2 registry, canonical eleven-column target, accepted-shape validation, lossless copy/rebuild and recorded-state validation. V2 name is `normalize_atividades_schema`; `user_version` follows the highest actually recorded migration.
- **Transaction/postconditions:** `apply_early_schema_migrations` rejects an existing transaction, records FK state, disables and verifies FK enforcement before `BEGIN IMMEDIATE`, encloses physical rebuild plus v2 metadata and user-version update in one bounded transaction, rolls back every injected stage failure, restores FK state and propagates exceptions before bootstrap. Exact rows in `requisicoes`, `matrizes_atividades_itens` and `atividade_legacy_map`, IDs, JSON values and `sqlite_sequence` are preserved.
- **Init/runtime delta:** both active init bodies call the early runner immediately after connection acquisition, create the same fresh eleven-column table, and retain unrestricted final migration application plus one final bootstrap commit. `main.ensure_atividades_schema_current`, its mapped-route call, the defensive ALTER/normalization sequence, and exactly one `app.db` lazy key/retrieval are retired. The remaining lazy order is activity-versioning, preferred-matrix, logger.
- **B8/B10 isolation:** B8 v1 helper SQL and all leaf table/trigger/index ownership remain unchanged. Activity-versioning core/rebuild/requisicoes compatibility remains in `main.py`; no B10 mutation occurred.
- **TDD/current validation:** pre-production RED was 20 expected failures; a supplemental contradiction RED proved `user_version=2` plus legacy physical schema did not hard-stop (`1 failed`, expected `RuntimeError`). The initial full hermetic run was 869 passed, 2 failed and 17 deselected; both failures were localized CSRF inventory/message-catalog collection caused by literal infrastructure strings passed directly to `RuntimeError`, with no route, CSRF-policy or snapshot drift. `SchemaMigrationStateError(RuntimeError)` plus the read-only contradiction gate resolved that defect without weakening inventory coverage. Focused migration is 23 passed, isolated CSRF 2 passed, critical aggregate 173 passed, and the pre-staging Windows `PYTHONUTF8=1`/`-B` full suite was 872 passed / 17 deselected. After selective staging exposed the new test to the Git-aware caller inventory, one post-staging contract node failed after 78 passes because the frozen manifest omitted `_run_init_on_temp_database` and still asserted 73 callers. The bounded authorized correction records 74; targeted rerun passed 1, the post-staging lane passed 79, and the required full rerun passed 873 with 17 deselected and zero failures/errors in 357.56s. All 14 authorized paths and protected physical invariants remained stable.
- **Routing/review:** Stage A FREE `opencode` / `opencode/deepseek-v4-flash-free`, session `ses_05ac5d405ffenqwkRrh7QeBAIo`, cost 0, was rejected as unusable; explicit Stage-A fallback `FALLBACK_FREE_UNUSABLE_DELIVERY` used `opencode-go` / `opencode-go/deepseek-v4-flash`, session `ses_05abb3bd1ffeut3C2R2SSaIuw7`, cost `0.0021259168`. Independent review attempted FREE first; after technically unusable/incomplete FREE transport and two recorded FREE uses, the final delta re-review explicitly used `FALLBACK_FREE_BUDGET_EXHAUSTED`, effective `opencode-go` / `opencode-go/deepseek-v4-flash`, session `ses_05a18eb14ffejH0B4Z7zROy427`, final-call cost `0.0005707464`, zero mutation, APPROVE / no findings. IAsup independently adjudicated and accepted the transaction, FK restoration, contradiction, JSON, child-preservation and exact lazy-delta evidence.
- **Current changed manifest:** production `app/db_maintenance.py`, `app/db.py`, `main.py`; tests `test_atividades_schema_migration_v2.py`, `test_activity_versioning_leaf_schema_ownership.py`, `test_backup_settings_ownership.py`, `test_ref_0c_b1_p0_access_context_transactions.py`, `test_phase3_schema_startup_transaction_contract.py`, `test_db_schema_maintenance.py`; canonical contract, Documentation Index, Architecture Ledger, this state and `AGENT_HANDOFF.md`—14 authorized paths. No fourth waiver path is used.
- **Canonical safety/next action:** canonical SQLite opens remain 0 by contract; `database.db` remains 544768 bytes with SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`, WAL/SHM absent, and the protected ignored residual remains 17420 bytes with SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`. Next: selective staging, bounded commit, feature-branch publication and post-publication verification. The commit tree intentionally contains no future SHA and records publication as pending. PHASE 3-B10 is not authorized.

### Historical PHASE 3-B8 — activity-versioning leaf schema ownership extraction (2026-07-27)

- **Status/baseline:** IMPLEMENTED / LOCALLY VERIFIED / INDEPENDENT REVIEW APPROVED / PUBLICATION PENDING. B7 is CLOSED / ACCEPTED by the explicit B8 order. Baseline workspace `D:\OneDrive\Programação\SGAA_clean_baseline`, branch `refactor/architecture-safety-net`, HEAD/upstream/live remote `d1947b0c11506045b8d52bd235bc7381a2ca22c9`, subject `Separate backup schema from runtime configuration`, parent `b328789f16cc4db0173e58d2d2454902565d0610`, divergence 0/0, tracked worktree clean, index empty and zero non-ignored untracked before mutation.
- **Owners/block classification:** `app.db_maintenance.ensure_atividade_versioning_leaf_tables` owns exactly `atividade_transicao`, `matriz_norma`, `matriz_atividade_versao_item`, and `atividade_legacy_map`; `ensure_atividade_versioning_leaf_triggers` owns exactly the INSERT then UPDATE AAC→AEU transition triggers; `ensure_atividade_versioning_leaf_indexes` owns exactly the eight accepted ordinary leaf indexes in baseline order. All three are pure caller-owned DDL.
- **Main isolation/order:** `main.ensure_atividade_versioning_schema(conn)` remains the one-argument public defining orchestrator. Migration/default-fix/hardening → matrix links → three core tables → leaf tables helper → six core triggers → leaf triggers helper → requisicoes compatibility ALTER block → seven core indexes → leaf indexes helper → requisicoes index is AST-equivalent to the accepted baseline after only the three block substitutions. `_VERSAO_NEW_DDL`, all six migration/rebuild predicates/delegates, core activity-version DDL/triggers/indexes and requisicoes compatibility statements remain AST-identical in `main`.
- **Lazy/import/transaction contract:** `app/db.py` is byte-identical to baseline and retains the exact four keys/retrievals: activities-current, activity-versioning, preferred-matrix, logger. Import direction is `main → app.db_maintenance`; importing the maintenance module does not import `main`. Each helper has one argument, executes only its accepted CREATE statements, owns no commit/rollback/savepoint/BEGIN/PRAGMA/executescript and preserves outer transaction ownership; caller rollback and injected failure paths are covered.
- **TDD/tests:** final expected RED was 16 failures with 3 baseline-only guards passing. GREEN focused passed 19 in 2.05s. Integrated ownership/Phase-B/idempotence/matrix/maintenance/residual/contract passed 126 in 15.24s; Git-aware isolation passed 15 in 8.69s; expanded 25-file versioning/matrix/requisition coverage passed 309 in 113.34s; route/RBAC passed 3 in 0.53s with direct counts 131/0; full hermetic passed 849 with 17 canonical D73H deselected in 287.05s. Independent review APPROVE is recorded below; post-publication gates remain pending.
- **Routing/adjudication:** Stage A used R4 `flash_free`, effective `opencode` / `opencode/deepseek-v4-flash-free`, session `ses_05af8e352ffetO5DM4E9LJ4n3d`, cost 0, exit 0, no fallback and no repository mutation. IAsup accepted its factual order/caller map but rejected its incorrect recommendation to move the `_needs_*` predicates. Independent review used the second FREE call, same provider/model, session `ses_05adebcf2ffe4xZqeQFq1LX6Fy`, cost 0, exit 0, no fallback/mutation; verdict APPROVE, accepted by IAsup.
- **Authorized/current manifest:** production `app/db_maintenance.py`, `main.py`; new focused test; executable Phase-3 contract test; canonical contract, Documentation Index, Architecture Ledger, this state and `AGENT_HANDOFF.md`—nine paths. `app/db.py` and existing functional activity-versioning tests remain unchanged.
- **Canonical safety:** canonical SQLite opens remain 0. Post-full proof retained `database.db` at 544768 bytes and SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`, WAL/SHM absent. The accepted ignored residual remains 17420 bytes, SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`, ignored/untracked; creation `2026-06-13 23:08:22.102134100 -0300` and last-write `2026-06-13 23:14:37.000000000 -0300` are preserved. Read/hash probes advanced only access/NTFS change metadata.
- **Next action:** create the single bounded commit, push only `refactor/architecture-safety-net`, then run the mandatory post-publication gates and physical/Git proof. PHASE 3-B9 is not authorized.

### PHASE 3-B7 — backup-settings responsibility separation and lazy-edge removal (2026-07-27)

- **Status:** CLOSED / ACCEPTED by the explicit B8 order. The historical B7 prohibition on B8 is superseded by that later authorization.
...
- **Historical next action:** B7 publication/review completed and was superseded by B8 authorization; current next action is recorded only in the B8 block above.
...
- **Ownership:** `app/db_maintenance.py` is the sole defining owner of
  `ensure_requisicao_alert_receipts_table`. Complete function AST, body, SQL, one-argument
  signature and `None` return annotation are identical to the starting-HEAD `main.py`
  definition. Normalized whole-module comparison proves zero unrelated delta in `main.py`,
  `app/db_maintenance.py` and `app/db.py`.
- **Schema/transactions:** exact `requisicao_alerta_receipts` table preserved with `id`,
  `requisicao_id`, `usuario_id`, `alert_kind`, `seen_at DEFAULT (datetime('now'))`, composite
  `UNIQUE(requisicao_id, usuario_id, alert_kind)`, cascading update/delete foreign keys to
  `requisicoes(id)` and `usuarios(id)`, and indexes `idx_req_alert_receipts_user_kind` and
  `idx_req_alert_receipts_req`. Parent tables may be absent at creation; repeat execution is
  idempotent; duplicate triples are rejected; both deletion cascades work. Caller transaction
  ownership remains intact with no commit, rollback or savepoint in the helper.
- **Compatibility/consumers:** `main.py` imports the owner and preserves
  `main.ensure_requisicao_alert_receipts_table is
  app.db_maintenance.ensure_requisicao_alert_receipts_table`. The complete pre-mutation
  `app/` inventory found only `app/db.py` as runtime consumer; it now imports the owner
  directly. All existing readers, writers and admin/student dashboard flows remain unchanged.
- **Lazy edge/order:** removed exactly `ensure_requisicao_alert_receipts_table` and its local
  `helpers[...]` retrieval from `app.db`; every other lazy key remains in original order. The
  direct call remains at the exact normalized-AST point in `_init_db_impl`; bootstrap order
  delta is zero.
- **Validation:** TDD RED produced 8 expected owner-absent failures. Focused receipts schema
  tests then passed 8/8. The first integrated lane passed 58 tests and exposed one historical
  B2 map expectation; after limiting that B2 contract to its own removed key, the focused
  ownership/schema/request-alert/admin-and-student-dashboard lane passed 59/59. Route/RBAC:
  3 passed. Git-aware runtime-isolation: 15 passed. Full hermetic suite: 756 passed, 17 D73H
  historical tests deselected, zero failures/errors in 406.48s.
- **Invariants:** behavior delta 0; routes 131→131; RBAC unmapped 0→0; schema-definition
  delta 0; migration delta 0; canonical database SQLite opens 0. `database.db` retained size
  544768 and SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`;
  no WAL/SHM. SQLite coverage used only `:memory:` or pytest-owned disposable databases
  outside the repository.
- **Execution/review:** direct IAsup `openai-codex/gpt-5.6-sol`, provider `openai-codex`,
  profile `ia-sup-sgaa`, medium. Direct mutation honored the order limiting IAexec to optional
  read-only review and minimized context/revalidation cost. FREE review used effective model
  `opencode/deepseek-v4-flash-free`, session `ses_05f20ebebffeCSTfszrEfFP4pZ`, cost 0,
  exit 0 and no fallback. Its `passed=false` verdict is rejected because it ran
  `git diff HEAD~1` instead of the authorized `HEAD`, falsely attributing already-published B2
  changes to B3; its positive exact-body/test-coverage observations were independently proven.
- **Exact manifest:** `app/db.py`, `app/db_maintenance.py`, `main.py`,
  ...
  `Extract requisition alert receipts schema ownership`; commit identity resolves through Git
  history.
- **Boundaries/residue:** repository file mutation was crossed. The historical B2 test
  expectation was corrected after physical reconciliation; no product mutation was repeated.
...
- **LEVEL 2 PHYSICAL RESTORATION (execution round R3): COMPLETE / LOCALLY VERIFIED /
  ...
  Level 3 NOT STARTED / NOT AUTHORIZED; Phase 2-A is separately authorized and locally
  implemented pending supervisor review; Phase 2-B onward is NOT STARTED / UNAUTHORIZED;
