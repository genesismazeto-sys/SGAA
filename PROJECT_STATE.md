## Current authoritative state — PHASE 4-B4.2 IMPLEMENTED / AWAITING_SUPERVISOR_REVIEW (2026-08-01)

### PHASE 4-B4.2 — Admin Requisições blueprint extraction

- **Status/authority:** PHASE 4-B4.2 is IMPLEMENTED / LOCALLY VERIFIED / INDEPENDENTLY REVIEWED / AWAITING_SUPERVISOR_REVIEW. B1, B2, B3, B4-A and B4.1 remain CLOSED / ACCEPTED. PHASE 4 remains OPEN / INCREMENTAL IMPLEMENTATION. Phase 5 and Phase 6 are NOT AUTHORIZED. Migration v4 is PROHIBITED.
- **Baseline and scope:** implementation began from clean `c587098152e97d125f41a2d26f2f414c10ae5676`, equal to upstream and live remote. Exactly the nine Admin Requisições handlers / 12 route-method pairs moved from `main.py` to canonical `app.views.admin.requisicoes`; RBAC remains `view=4`, `edit=5`, `full=3`. The accepted legacy registrar preserves global endpoint names; no alias, duplicate or namespaced route exists.
...
- **Test evidence:** TDD RED `20 failed / 3 passed`; primary GREEN lane `138 passed`; complementary route/RBAC/actor/versioning/Phase-3/request-alert lane `107 passed`. The first full lane exposed four stale governance assertions without physical drift (`1001 passed / 4 failed / 17 deselected`); the bounded documentary correction was focused-GREEN, and the final hermetic lane was `1005 passed / 17 deselected`. Routes 131; endpoints 130; governed pairs 134; RBAC unmapped 0; actor 402 = 263 allowed + 139 denied; message catalog 536 with zero semantic/default/kind/key delta. Each CSRF snapshot has exactly five owner-only `view_function` deltas and no other field delta.
- **Independent review:** Flash FREE `opencode/deepseek-v4-flash-free`, session `ses_03ff5695bffeEBNIZRiSVZonYz`, cost 0, exit 0, no fallback, mutation 0: PASS; blocking findings NONE; scope EXACT; behavior PRESERVED; governance COHERENT. One nonblocking reproducibility finding was adjudicated as valid and corrected by documenting the candidate-hash algorithm; technical reviewed content hash `a75677c343b393f884c690f06f29e637ac942a1fff52a4a0a4125dd2d4ee390a`.
- **Exact candidate manifest:** 16 paths = 3 production + 7 tests/snapshots + 6 governance, enumerated in `docs/refactor/PHASE4_REQUISICOES_BLUEPRINT_CONTRACT.md`. The independently reviewed technical candidate remains unstaged and unpublished.
...
- **Current boundaries / next exact action:** obtain a documentary addendum for the six-governance-path review record, then selectively stage, run index-visible gates, commit once and push fast-forward. Matrizes is not started. Phase 5, Phase 6 and migration v4 remain outside authority.
