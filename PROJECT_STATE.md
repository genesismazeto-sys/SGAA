## Current authoritative state — PHASE 4-B4.1 neutral shared-owner extraction (2026-08-01)

### PHASE 4-B4.1 — IMPLEMENTED / AWAITING SUPERVISOR REVIEW

- **Status/authority:** PHASE 4-B4-A: CLOSED / ACCEPTED. PHASE 4-B4.1: IMPLEMENTED / AWAITING SUPERVISOR REVIEW. B1, B2 and B3 remain CLOSED / ACCEPTED. PHASE 4 remains OPEN / INCREMENTAL IMPLEMENTATION. PHASE 4-B4.2: NOT AUTHORIZED. Phase 5: NOT AUTHORIZED. Phase 6: NOT AUTHORIZED. Migration v4: PROHIBITED.
- **Baseline/publication identity:** parent `185426daccc9f0eb0dba4497248100c1a88d15fa`, branch `refactor/architecture-safety-net`, baseline divergence `0/0`, protected `main` `340fc7c91c6bc9b50e884adcb5915f9e29a0bfe1`; bounded child subject `Extract requisition shared owners`.
- **Reconciled expansion:** `app/settings.py` is the accepted neutral owner of all eight B1 settings helpers; `app/matrix_scope.py` is the accepted dedicated neutral matrix-scope owner; `app/views/aluno.py` contains the one B4.1 mechanical reverse-edge reduction; `tests/test_phase3_schema_startup_transaction_contract.py` contains only a bounded governance-reader reconciliation. Classification: `B4_1_SCOPE_EXPANSION / COHERENT_NEUTRALIZATION / SUPERVISOR_RECONCILED`.
- **Canonical owners:** `app.settings` owns exactly `_normalize_optional_iso_date`, `get_app_settings`, `get_response_time_settings`, `save_app_settings`, `reset_response_time_metrics`, `save_return_response_settings`, `get_horas_settings`, and `save_horas_settings`; `app.requisitions` owns `auto_indefer_devolvidas`; `app.matrix_scope` owns `MATRIZ_STATUS_META`, `_matriz_status_label`, `_matriz_option_label`, `get_effective_matriz_for_turma`, `is_activity_allowed_for_turma_matrix`, and `get_allowed_activity_ids_for_turma_matrix`. Identity re-exports remain in Configurações/`main`; Aluno imports matrix scope directly.
- **Scope proof:** NO ROUTE MOVEMENT. EXACT 9 REQUISICOES ROUTES REMAIN IN MAIN.PY, with baseline-identical ASTs and endpoint bindings. `app/views/admin/requisicoes.py` is absent. No functional hardening, schema, migration, SQL, transaction, RBAC, CSRF, template, snapshot, upload or document behavior delta.
- **Catalog/transaction proof:** message catalog remains 536 keys; six settings usages change canonical owner only. The auto-indefer body retains exactly one conditional commit and no rollback. All moved function/metadata ASTs match baseline.
- **Exact artifact:** 20 paths = 7 production + 7 tests + 6 governance; the added test path is the mechanically required Macro Phase 3 current-checklist reader, with no production expansion. Canonical enumeration and gates are in `docs/refactor/PHASE4_REQUISICOES_SHARED_OWNER_CONTRACT.md`.
- **TDD/gates:** baseline focused 125 passed / canonical SQLite opens 0; RED 7 failed + 1 passed; governance RED 1 failed; owner GREEN 8 passed; affected technical lane 247 passed / canonical SQLite opens 0. First full: 981 passed / 3 failed / 17 deselected; repaired two stale governance readers and isolated one pre-existing randomized turma-code collision; recovered nodes 3 passed. Final full: 984 passed / 17 deselected / 419.95s / canonical SQLite opens 0.
- **Technical review and freeze:** reviewed tracked-diff SHA-256 `f4b6cb00b4365cc7c20af5fcba1ac736ece1bab0ab9c6e0f89b19084799727f9`; complete pre-addendum candidate SHA-256 `ddc73cb94786899595f3cce9d577bdbcca961a082c83866498a36ebd4687a23f`; non-governance SHA-256 `74649089be0699dff4440260bdb11793b4e5793550f17456893f4a281bd6096b`. Route `flash_free`; requested/effective model `opencode/deepseek-v4-flash-free`; session `ses_040d538bfffegBsAJQbLJrnqSV`; exit 0; cost 0; fallback none; mutation count 0; PASS; material findings NONE. The reviewer phrase “8 tests” is `REJECTED_NON_MATERIAL / STALE NARRATIVE COUNT / NO TECHNICAL CONSEQUENCE`; the exact manifest remains seven test paths.
- **Historical owner truth:** at B1-time, `app/views/admin/configuracoes.py` defined all eight settings helpers. At B4.1-time, their defining ownership moves together to `app.settings`; Configurações and `main` retain identity-compatible exports and all eight B1 routes/behavior remain accepted. At B2-time, Aluno had six residual lazy-main edges. B4.1 later removes only `get_effective_matriz_for_turma`, leaving exactly five: `ensure_admin_arquivos_table`, `get_admin_arquivo`, `get_student_request_update_alert`, `list_active_admin_alertas`, and `mark_student_request_updates_seen`.
- **Physical safety:** canonical `database.db` 544768 bytes / SHA-256 `a3a55e63427024476d85d1fce3e0a5efaedcd33624400b2e67a815217d570fe9`; sidecars absent. Protected residual 17420 bytes / SHA-256 `7388cfbc8f446410ef3c98ec0fa274c2c36ff4f3ef8ed2cd649bb1f3e1d3bb0e`.
- **Next exact action:** documentary Flash FREE addendum, exact selective commit/push and post-publication verification in this execution, then external supervisor review. Do not begin B4.2.
