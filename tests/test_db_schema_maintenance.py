...
def test_schema_status_is_prod1_v7():
    conn = _prod1()
    status = db_maintenance.get_schema_status(conn)
    assert status["schema_epoch"] == status["target_schema_epoch"] == "prod-1"
    assert status["schema_version"] == status["target_schema_version"] == 7
    assert status["latest_migration"]["name"] == "request_email_notifications"
