...
def test_get_matrix_aac_page_renders_new_activity_button_in_left_header(versioned_env):
    ...
    response = client.get("/admin/editar_matriz/1?tab=aac")
    assert response.status_code == 200

    html = response.get_data(as_text=True)
    assert re.search(r'aria-labelledby="transfer-available-heading".*?\+ Nova atividade', html, re.S)
    assert 'action="/admin/matrizes/1/atividades/nova/aac"' in html
...
def test_get_matrix_aeu_page_renders_new_activity_button(versioned_env):
    ...
    response = client.get("/admin/editar_matriz/2?tab=aea")
    assert response.status_code == 200

    html = response.get_data(as_text=True)
    assert "+ Nova atividade" in html
    assert 'action="/admin/matrizes/2/atividades/nova/aea"' in html
    ...
    with main.app.app_context():
        ...
        legacy_scope_link = conn.execute(
            ...
            SELECT COUNT(*) AS c
             ...
             WHERE matriz_id = 1 AND atividade_id = ?
            """,
            (atividade["id"],),
        ).fetchone()["c"]
        ...
        matrix_version_link = conn.execute(
            ...
            SELECT COUNT(*) AS c
             ...
             WHERE matriz_id = 1 AND atividade_versao_id = ?
            """,
            (versao["id"],),
        ).fetchone()["c"]
        assert matrix_version_link == 1

    reopened = client.get("/admin/editar_matriz/1?tab=aac")
...
def test_post_new_aeu_activity_creates_aeu_context(versioned_env):
    ...
    response = client.post(
        "/admin/matrizes/2/atividades/nova/aea",
        data={
            "nome": activity_name,
            "add_to_matrix": "1",
        },
...
def test_post_new_activity_requires_explicit_norm_when_multiple_compatible_norms(versioned_env):
    ...
    with main.app.app_context():
        ...
        conn.execute("INSERT INTO matriz_norma (matriz_id, norma_id) VALUES (2, ?)", (norma_id,))
        conn.commit()

    response = client.post(
        "/admin/matrizes/2/atividades/nova/aac",
        data={
            "nome": activity_name,
            "grupo_numero": "9",
...
def test_modal_form_contains_csrf_and_post_requires_it(versioned_env_csrf):
    ...
    page = client.get("/admin/editar_matriz/1?tab=aac")
    ...
    assert "/admin/editar_matriz/1?tab=aac" in (created.headers.get("Location") or "")
