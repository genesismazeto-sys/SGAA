...
# Non-UT10 handlers that must remain main-owned before and after the
# extraction: Alertas (4) + Reportes (3). Frozen substitute for the
# non-Arquivos half of test_b7p_zero_route_movement_all_twelve_handlers...
...
def test_green_8_non_ut10_handlers_remain_main_owned():
    source = MAIN_PATH.read_text(encoding="utf-8-sig")
    defined = _top_level_defs(source)
    for name in NON_UT10_MAIN_OWNED_HANDLERS:
        assert name in defined, (
            f"{name} must still be locally defined in main.py "
            "(Alertas/Reportes stay main-owned through UT-10)"
        )
