...
REQUISICAO_SNAPSHOT_SUPPORTED_SCHEMA = "prod-1-request-v1"
...
IDENTITY_FIELDS = (
    ...
    "norma_id", "codigo_normativo", "eixo", "matriz_id_efetiva", "schema_version",
)
...
def read_requisicao_snapshot_for_processing(row) -> RequisicaoSnapshotProcessingRead:
    ...
    code = _value(row, "codigo_normativo_snapshot")
    if raw in (None, "") or version_id in (None, "") or code in (None, ""):
    ...
    try:
        identity = {
            ...
            "norma_id": _positive_int(payload["norma_id"]),
    ...
    if identity["codigo_normativo"] != code:
        return _invalid("normative_code_identity_mismatch")
    return RequisicaoSnapshotProcessingRead(
...
def _load_rule(conn, version_id: int):
    return conn.execute(
        ...
        SELECT version.id AS atividade_versao_id,
               ...
               norma.codigo AS norma_codigo, norma.eixo AS norma_eixo
          FROM atividade_versao version
          JOIN atividade_base base ON base.id=version.atividade_base_id
          JOIN norma_atividade norma ON norma.id=version.norma_id
         WHERE version.id=?
        """, (version_id,),
    ).fetchone()
...
def prepare_versioned_requisicao_snapshot(conn, *, flow_origin: str, aluno_id, atividade_versao_id):
    ...
    if not row or row["versao_status"] != "ativa":
        raise RequisicaoSnapshotError(RequisicaoSnapshotError.user_message)
    if row["codigo_normativo"] != row["norma_codigo"] or row["eixo"] != row["norma_eixo"]:
        raise RequisicaoSnapshotError(RequisicaoSnapshotError.user_message)
    if not conn.execute(
        "SELECT 1 FROM matriz_norma WHERE matriz_id=? AND norma_id=?",
        (resolved["matriz_id_efetiva"], row["norma_id"]),
    ).fetchone():
        raise RequisicaoSnapshotError(RequisicaoSnapshotError.user_message)
    ...
    payload = {
        ...
        "norma_id": int(row["norma_id"]), "codigo_normativo": str(row["codigo_normativo"]),
    ...
    return PreparedRequisicaoSnapshot(int(atividade_versao_id), str(row["codigo_normativo"]), snapshot_json, payload)
