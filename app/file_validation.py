...
def _image_within_limits(image: Image.Image) -> bool:
    ...
    if pixels > MAX_IMAGE_PIXELS:
        return False
    return pixels * max(1, len(image.getbands())) * 2 <= MAX_DECODED_IMAGE_BYTES
...
def _valid_pdf(content: bytes) -> bool:
    ...
    try:
        ...
        if len(reader.pages) < 1:
            return False
        page = reader.pages[0]
        return page.get("/Type") == "/Page" and page.mediabox is not None
    except Exception:
        return False
...
def detect_supported_mime(content: bytes) -> str | None:
    ...
    if _valid_image(content, signature=b"\x89PNG\r\n\x1a\n", expected_format="PNG"):
        return "image/png"
...
__all__ = [
    "MAX_DECODED_IMAGE_BYTES", "MAX_IMAGE_DIMENSION", "MAX_IMAGE_PIXELS",
    "MIME_BY_EXTENSION", "detect_supported_mime",
]
