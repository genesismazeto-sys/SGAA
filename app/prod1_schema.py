...
PASSWORD_FOUNDATION_MARKER = "password_foundation"
...
# The credential table is the v11 form, the token table the v10 form; the
# token index is unchanged since v8.  Composed here so the head schema has
# exactly one authority per object.
_PASSWORD_FOUNDATION_HEAD_SQL = ";\n".join(
    (
...
__PASSWORD_FOUNDATION_V8_SCHEMA_OBJECTS__;
...
INSERT INTO schema_migrations(version,name,schema_epoch,details_json)
VALUES(8,'password_foundation','prod-1','{"schema_epoch":"prod-1","credential_state":"usuario_credenciais","password_tokens":"senha_tokens","legacy_backfill":"current_default_match"}');
INSERT INTO schema_migrations(version,name,schema_epoch,details_json)
VALUES(9,'access_status','prod-1','{"schema_epoch":"prod-1","access_status":"usuario_credenciais.acesso_ativo","backfill":"all_existing_credentials_active"}');
INSERT INTO schema_migrations(version,name,schema_epoch,details_json)
...
_PROD1_V7_SIGNATURE_SHA256 = "61df106683b3a382199c2bd7147a3d05ac3974acefcbb9c0a09f62a69f31da03"
_PROD1_V8_SIGNATURE_SHA256 = "a2e91848fa577ddf98c3c0fee4d8d292cf8e6385db2bb46e8f6d3dd8eee7b4ba"
...
def _validate_prod1_v7_schema(conn: sqlite3.Connection) -> None:
    """Recognize the frozen prod-1/v7 predecessor exactly."""
    if _user_version(conn) != 7:
        raise Prod1SchemaError("prod-1/v7 user_version mismatch")
    if _marker(conn) != [
        (1, BASELINE_MARKER, SCHEMA_EPOCH),
        (2, NORMA_REMOVAL_MARKER, SCHEMA_EPOCH),
        (3, MATRIX_VERSION_REMOVAL_MARKER, SCHEMA_EPOCH),
        ...
        raise Prod1SchemaError("prod-1/v7 migration marker mismatch")
    if _physical_schema_digest(conn) != _PROD1_V7_SIGNATURE_SHA256:
        raise Prod1SchemaError("prod-1/v7 physical schema contract mismatch")
    violations = conn.execute("PRAGMA foreign_key_check").fetchall()
    if violations:
        raise Prod1SchemaError(f"prod-1/v7 foreign key violations: {violations!r}")


def _validate_prod1_v8_schema(conn: sqlite3.Connection) -> None:
    """Recognize the frozen prod-1/v8 predecessor exactly."""
    if _user_version(conn) != 8:
        raise Prod1SchemaError("prod-1/v8 user_version mismatch")
    if _marker(conn) != [
        (1, BASELINE_MARKER, SCHEMA_EPOCH),
        (2, NORMA_REMOVAL_MARKER, SCHEMA_EPOCH),
        (3, MATRIX_VERSION_REMOVAL_MARKER, SCHEMA_EPOCH),
        ...
        raise Prod1SchemaError("prod-1/v8 migration marker mismatch")
    if _physical_schema_digest(conn) != _PROD1_V8_SIGNATURE_SHA256:
        raise Prod1SchemaError("prod-1/v8 physical schema contract mismatch")
    violations = conn.execute("PRAGMA foreign_key_check").fetchall()
    if violations:
        raise Prod1SchemaError(f"prod-1/v8 foreign key violations: {violations!r}")


def _validate_prod1_v9_schema(conn: sqlite3.Connection) -> None:
    """Recognize the frozen prod-1/v9 predecessor exactly."""
    if _user_version(conn) != 9:
        raise Prod1SchemaError("prod-1/v9 user_version mismatch")
    if _marker(conn) != [
        (1, BASELINE_MARKER, SCHEMA_EPOCH),
        (2, NORMA_REMOVAL_MARKER, SCHEMA_EPOCH),
        (3, MATRIX_VERSION_REMOVAL_MARKER, SCHEMA_EPOCH),
        ...
        raise Prod1SchemaError("prod-1/v9 migration marker mismatch")
    if _physical_schema_digest(conn) != _PROD1_V9_SIGNATURE_SHA256:
        raise Prod1SchemaError("prod-1/v9 physical schema contract mismatch")
    violations = conn.execute("PRAGMA foreign_key_check").fetchall()
    if violations:
        raise Prod1SchemaError(f"prod-1/v9 foreign key violations: {violations!r}")


def _validate_prod1_v10_schema(conn: sqlite3.Connection) -> None:
    """Recognize the frozen prod-1/v10 predecessor exactly."""
    if _user_version(conn) != 10:
        raise Prod1SchemaError("prod-1/v10 user_version mismatch")
    if _marker(conn) != [
        (1, BASELINE_MARKER, SCHEMA_EPOCH),
        (2, NORMA_REMOVAL_MARKER, SCHEMA_EPOCH),
        (3, MATRIX_VERSION_REMOVAL_MARKER, SCHEMA_EPOCH),
        ...
        raise Prod1SchemaError("prod-1/v10 migration marker mismatch")
    if _physical_schema_digest(conn) != _PROD1_V10_SIGNATURE_SHA256:
        raise Prod1SchemaError("prod-1/v10 physical schema contract mismatch")
    violations = conn.execute("PRAGMA foreign_key_check").fetchall()
    if violations:
...
def migrate_prod1_v7_to_v8(conn: sqlite3.Connection) -> dict[str, object]:
    from app.prod1_password_foundation_v8 import migrate_prod1_v7_to_v8 as migrate
    return migrate(conn)


def migrate_prod1_v8_to_v9(conn: sqlite3.Connection) -> dict[str, object]:
    from app.prod1_access_status_v9 import migrate_prod1_v8_to_v9 as migrate
    return migrate(conn)


def migrate_prod1_v9_to_v10(conn: sqlite3.Connection) -> dict[str, object]:
    from app.prod1_access_delivery_v10 import migrate_prod1_v9_to_v10 as migrate
    return migrate(conn)


def migrate_prod1_v10_to_v11(conn: sqlite3.Connection) -> dict[str, object]:
    from app.prod1_credential_pending_v11 import migrate_prod1_v10_to_v11 as migrate
    return migrate(conn)
